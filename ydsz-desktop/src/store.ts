...
import { resolveThreadBranchRegressionGuard } from "@njydsz/shared/git";
import { normalizeModelSlug } from "@njydsz/shared/model";
import { normalizeWorkspaceRootForComparison } from "@njydsz/shared/threadWorkspace";
...
import { deriveThreadSummaryMetadata } from "@njydsz/shared/threadSummary";
...
const PERSISTED_STATE_KEY = "ydsz-buddy:renderer-state:v8";
const LEGACY_PERSISTED_STATE_KEYS = [
  "ydsz-buddy:renderer-state:v4",
  "ydsz-buddy:renderer-state:v3",
  "ydsz-buddy:renderer-state:v2",
  "ydsz-buddy:renderer-state:v1",
] as const;
...
function readModelSessionFromThreadSession(
  ...
  return {
    ...
    runtimeMode: previousThread?.runtimeMode ?? incomingSession?.runtimeMode ?? "code",
      ...
      return applyThreadUpdate(
        ...
        (thread) => {
          ...
          const retainedTurnIds = new Set<string>(
            turnDiffSummaries.map((entry) => entry.turnId).filter((id) => id !== null) as string[],
          );
...
// 项目操作函数已提取到 store/projectOperations.ts
export {
  toggleProject,
  setProjectExpanded,
  setAllProjectsExpanded,
