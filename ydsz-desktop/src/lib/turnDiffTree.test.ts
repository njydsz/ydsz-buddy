...
import { buildTurnDiffTree, flattenTurnDiffTree, summarizeTurnDiffStats } from "./turnDiffTree";
...
describe("flattenTurnDiffTree", () => {
  it("returns empty array for empty tree", () => {
    expect(flattenTurnDiffTree([])).toEqual([]);
  });

  it("flattens a flat list of file nodes", () => {
    const tree = buildTurnDiffTree([
      makeFile("a.ts", 1, 0),
      makeFile("b.ts", 2, 0),
    ]);
    ...
    expect(flat.every((n) => n.kind === "file")).toBe(true);
  });

  it("performs depth-first traversal (dirs first, then files)", () => {
    const tree = buildTurnDiffTree([
      makeFile("src/a.ts", 1, 0),
      makeFile("src/b.ts", 2, 1),
      makeFile("docs/readme.md", 5, 0),
    ...
    const flat = flattenTurnDiffTree(tree);
    // 期望顺序：docs (directory), docs/readme.md (file), src (directory), src/a.ts, src/b.ts
    expect(flat).toHaveLength(5);
    const kinds = flat.map((n) => n.kind);
    expect(kinds.filter((k) => k === "directory")).toHaveLength(2);
    expect(kinds.filter((k) => k === "file")).toHaveLength(3);
  });
});
