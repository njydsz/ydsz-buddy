 ...
 * Git worktree 对账查询选项。
 *
 * 调用 `git.reconcileWorktrees` 扫描 `git worktree list --porcelain`,
 * 同步后端 ManagedWorktreeService 内存注册表(注册孤儿、移除悬空),
 * 返回对账后的活跃 worktree 列表。
 *
 * 使用场景:
 * - 启动时对账:workspace 打开后调用,恢复崩溃前的注册表状态
 * - 用户手动 `git worktree remove` 后刷新
 *
 * @param input.cwd - 仓库工作目录
 */
export function gitReconcileWorktreesQueryOptions(cwd: string | null | undefined) {
  return queryOptions({
    queryKey: ["git", "reconcile-worktrees", cwd ?? ""] as const,
    queryFn: async () => {
      if (!cwd) {
        return { registered: 0, removed: 0, worktrees: [] };
      }
      const api = ensureNativeApi();
      return api.git.reconcileWorktrees({ cwd });
    },
    // 对账是廉价操作(单次 git 命令),30s 缓存避免短时间重复调用
    staleTime: 30_000,
  });
}
