 ...
 * @file 图标聚合模块
 *
 * 本模块将 ydsz 应用中分散在多个图标库（react-icons）的图标统一聚合导出，
...
import { type FC, type SVGProps, type JSX } from "react";
...
function adaptIcon(Component: TablerIcon): LucideIcon {
  return function AdaptedIcon(props: SVGProps<SVGSVGElement>) {
    return <Component {...(props as unknown as JSX.IntrinsicElements["svg"])} />;
  };
}
...
export const CoinIcon = adaptIcon(IconCoin);
...
export const SkipBackIcon = adaptIcon(IconPlayerSkipBack);
export const SkipForwardIcon = adaptIcon(IconPlayerSkipForward);
...
export const SendIcon = adaptIcon(IconSend);
...
export const StarOffIcon = adaptIcon(IconStarOff);
export const SparklesIcon = adaptIcon(IconSparkles);
export const CalendarIcon = adaptIcon(IconCalendar);
...
// HMR 保护：本模块被 80+ 组件引用，HMR 级联会同时重载所有依赖模块，
// 导致主线程长时间阻塞、窗口"未响应"。改为 decline → 强制整页刷新，
// 整页刷新比 80+ 模块的 HMR 级联更快更可靠。
if (import.meta.hot) {
