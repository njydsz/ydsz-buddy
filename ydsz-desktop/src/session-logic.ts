/**
 * @file 会话逻辑模块
 * @description 从线程状态推导会话相关信息，包括工作日志条目、待批准请求、用户输入问题等。
 *              提供 Provider 选项配置和会话状态相关的辅助函数。
 */
...
} from "@njydsz/shared/subagents";
import { summarizeToolRawOutput } from "@njydsz/shared/toolOutputSummary";
...
export function deriveActiveTaskListState(
  ...
  if (currentTurnTaskList) {
    if (
      currentTurnTaskList.turnId &&
      settledTurnIds.has(currentTurnTaskList.turnId) &&
      currentTurnTaskList.tasks.every((task) => task.status === "completed")
    ) {
      // Current turn settled with every task completed — hide the list.
    } else {
