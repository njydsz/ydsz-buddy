/**
 * @fileoverview "What's New" 更新日志条目数据
 * @description 在更新后对话框和设置中的"版本历史"视图里渲染的精选更新日志。
 *              由 `useWhatsNew`、`WhatsNewDialog` 和 `ChangelogAccordion` 消费。
 * @layer 静态数据层
 *
 * @remarks
 * 编写指南:
 * - 新版本添加在前面,这样文件阅读时最新的在前(UI 也会排序,但保持源文件整洁便于 PR 审查)。
 * - `version` 必须与 `apps/web/package.json#version` 完全一致。逻辑使用 semver 比较版本,
 *   只有当安装的构建在此有精选条目时才显示对话框。
 * - `date` 原样渲染 —— 选择你想要的格式(如 `"Apr 18"`、`"2026-04-18"`),
 *   只需在版本间保持一致。
 * - 每个功能特性需要一个 `id`(稳定、每个版本唯一)、一个简短 `title`、
 *   一个营销 `description`,以及可选的 `image`
 *   (相对于 `apps/web/public` 的绝对路径,如 `/whats-new/0.0.29/foo.png`)
 *   加上 `details` 用于在图片下方显示更长的技术说明。
 */
...
export const WHATS_NEW_ENTRIES: readonly WhatsNewEntry[] = [
  {
    ...
    features: [
      {
        id: "cross-project-parallel-sessions",
        title: "跨工程并行任务面板",
        description:
          "侧栏新增「运行中的任务」聚合面板,实时展示所有工程下正在运行的会话,按工程分组并按最近活跃排序,点击即可跳转。无需切换工作区即可纵览全局并行进度,对标 CodeX / OpenCode 的多 session 并行可视能力。",
      },
      {
        id: "rollback-drawer-activation",
        title: "多 turn 回滚抽屉激活",
        description:
          "消息上下文菜单与事件时间线的回滚入口现统一打开 RollbackDrawer,提供 diff 预览与二次确认,替代原有的朴素 confirm 弹窗,降低误操作风险。",
      },
      {
        id: "mcp-ssh-settings-mounted",
        title: "MCP 服务器与 SSH 远程配置入口",
        description:
          "设置页「高级」分区正式挂载 MCP 服务器管理与 SSH 远程连接面板,无需打开工作区即可查看配置说明,打开工作区后可直接在设置内完成 MCP / SSH 的增删改与连接测试。",
      },
    ],
  },
  {
    ...
    features: [
      {
        ...
        description: "云顶数字 Buddy 创建 includes Claude Opus 4.8 in the Claude model picker.",
      },
    ],
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "The provider picker no longer treats unknown provider status as usable. Providers stay in a checking state until ydsz-buddy has confirmed that the local runtime is available and authenticated.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "ydsz-buddy can now launch and monitor Kilo Code sessions alongside Codex, Claude, Cursor, OpenCode, and Gemini, with health checks, settings, mentions, handoff, and model compatibility wired through the app.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "云顶数字 Buddy 创建 tracks the newer OpenCode SDK session events, keeps titles fresher, and has much deeper coverage around OpenCode startup, output, and recovery flows.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "云顶数字 Buddy 创建 serves generated files through a dedicated local-image endpoint with MIME checks, workspace-aware path resolution, and Codex generated_images allowlists for both the normal home and desktop overlay home.",
      },
      {
  ...
  },
  {
    ...
    features: [
      {
        id: "cursor-provider",
        title: "Cursor is now a first-class ydsz-buddy provider",
        description:
          "Run Cursor CLI sessions directly from 云顶数字 Buddy now ACP-backed startup, model discovery, existing-chat resume, handoff, and provider health checks alongside Codex and OpenCode.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "The Git actions menu can now commit current work and push it from ydsz-buddy, keeping the common release and handoff flow closer to the chat.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "The Git menu now includes branch creation with ydsz-buddy-style names, and PR creation can recover from GitHub duplicate-PR responses by reusing the existing open pull request.",
      },
      {
        id: "legacy-import-recovery",
        title: "Legacy T3 imports heal themselves",
        description:
          "A new migration reconciles older imported T3 Code databases whose migration history skipped ydsz-buddy schema changes, preventing missing-column crashes after import.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "OpenCode todo events now flow into ydsz-buddy as active task updates, with a compact banner option for keeping current work visible without taking over the chat.",
      },
      {
      ...
      },
      {
        ...
        description:
          "OpenCode sessions now report context usage more consistently, giving ydsz-buddy better runtime visibility as conversations grow.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "Cursor is now available as a provider, with ACP sessions, model discovery, existing chats, handoff, shortcuts, and git text generation wired into ydsz-buddy.",
      },
      {
      ...
      },
      {
        ...
        description:
          "Toast notifications picked up feature-flag wiring, giving ydsz-buddy a safer way to roll notification changes forward or back.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "云顶数字 Buddy 创建 handles messy branch switches with clearer recovery actions, recreated stashes, unpublished branch publishing, and stronger checks around conflicts and local work.",
      },
      {
      ...
      },
      {
        ...
        description:
          "The desktop app now has app-level back and forward navigation controls, making it easier to move around 云顶数字 Buddy nowout losing your place.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "GPT-5.5 is now in the model picker with the right default reasoning behavior, so you can move new Codex sessions onto the latest model directly from ydsz-buddy.",
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "云顶数字 Buddy 创建 shows a proper splash screen while the desktop backend spins up, so launch feels intentional instead of looking briefly stalled.",
      },
      {
    ...
    features: [
      {
        ...
        description:
          "The new theme pack editor lets you tune UI colors directly in ydsz-buddy, with shared theme tokens keeping the sidebar, composer, transcript, and controls in sync.",
      },
      {
    ...
    features: [
      {
        ...
        title: "⏪ Steering messages are clearly marked",
        description:
      ...
      },
      {
        ...
        description:
          "If the updater never answers, 云顶数字 Buddy 创建 times out and recovers instead of hanging on a permanent Checking status.",
      },
    ],
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        description:
          "Point ydsz-buddy at your own Codex, Claude, or Gemini binary when your setup lives outside the default install path.",
      },
      {
