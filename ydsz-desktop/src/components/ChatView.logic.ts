...
import { normalizeModelSlug } from "@njydsz/shared/model";
import { buildYdszBuddyBranchName } from "@njydsz/shared/git";
import { isGenericChatThreadTitle } from "@njydsz/shared/chatThreads";
import { isGenericTerminalThreadTitle } from "@njydsz/shared/terminalThreads";
import {
...
export const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = "ydsz-buddy:last-invoked-script-by-project";
export const DISMISSED_PROVIDER_HEALTH_BANNERS_KEY = "ydsz-buddy:dismissed-provider-health-banners";
...
export function describeVoiceRecordingStartError(error: unknown): string {
  ...
  if (errorName === "NotAllowedError" || errorName === "PermissionDeniedError") {
    return "Microphone access was denied. Enable it in macOS Privacy & Security > Microphone for ydsz-buddy, then try again.";
  }
...
/**
 * Review 模式 diff 优先布局：判断是否需要自动打开 diff 面板
 *
 * 设计目标：
 * - 进入 review 模式时自动把 diff 面板拉到前面，确保 diff 优先可见
 * - 兼容 `panelState` 与 URL 搜索两种 diff 状态来源
 * - 用户主动从 review 切到其他模式时不做任何面板调整（尊重用户意图）
 * - 同一模式下重复点击 mode 切换器（mode === currentMode）也无需处理
 *
 * 返回值说明：
 * - `open`：需要触发 open 流程（onToggleDiffPanel 或 navigate）
 * - `noop`：diff 已经是打开状态，无需操作
 * - `skip`：不是 review 模式，不做面板动作
 */
export type ReviewModeDiffAction =
  | { kind: "open" }
  | { kind: "noop" }
  | { kind: "skip" };

export function resolveReviewModeDiffAutoOpen(input: {
  nextMode: ProviderInteractionMode;
  currentMode: ProviderInteractionMode;
  diffIsOpen: boolean;
}): ReviewModeDiffAction {
  if (input.nextMode === input.currentMode) {
    return { kind: "skip" };
  }
  if (input.nextMode !== "review") {
    return { kind: "skip" };
  }
  if (input.diffIsOpen) {
    return { kind: "noop" };
...
export function buildSuggestedWorktreeName(input: {
  ...
  return buildYdszBuddyBranchName(input.associatedWorktreeBranch ?? input.title);
}
...
export function shouldRenderTerminalWorkspace(options: {
  ...
  runtimeMode?: "work" | "code" | string | null;
}): boolean {
  if (options.runtimeMode === "work") return false;
  return (
