...
import { ParallelSessionsPanel } from "./ParallelSessionsPanel";
...
function useThreadSortLabels(): Record<SidebarThreadSortOrder, string> {
  const messages = useMessages();
  return {
    updated_at: messages.sidebar.sortRecentlyActive,
    created_at: messages.sidebar.sortCreatedAt,
...
type DebugFeatureFlagsWindow = Window & {
  ydszbuddyShowFeatureFlags?: () => void;
  ydszbuddyHideFeatureFlags?: () => void;
};
...
export function YdszBuddyWordmark() {
  return (
    <img
      alt="云顶数字 Buddy"
  ...
  return (
    <Menu>
      <Tooltip>
        <TooltipTrigger
          render={
            <MenuTrigger className="inline-flex size-7 cursor-pointer items-center justify-center rounded-md text-muted-foreground/70 transition-colors hover:bg-(--sidebar-accent) hover:text-foreground" />
          }
        >
      ...
      <MenuPopup
        align="end"
        side="bottom"
        className="min-w-44 rounded-lg border-(--color-border) bg-(--color-background-elevated-primary-opaque) shadow-lg"
      >
        <MenuGroup>
  ...
  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        ...
        className="group/sidebar-primary-action h-8 gap-2.5 rounded-lg px-2 font-system-ui text-(length:--app-font-size-ui,12px) font-normal text-foreground/89 transition-colors hover:bg-(--sidebar-accent) data-[active=true]:bg-(--sidebar-accent-active) data-[active=true]:text-(--sidebar-accent-foreground)"
      ...
      >
        ...
        {shortcutParts.length > 0 ? (
          <span
            className={cn(
              "ml-auto",
              shortcutLabel ? "opacity-100" : "opacity-0 transition-opacity group-hover/sidebar-primary-action:opacity-100 group-focus-visible/sidebar-primary-action:opacity-100",
            )}
          >
            <KbdGroup>
...
function SidebarSegmentedPicker({
  ...
  return (
    <div className="px-3 pb-1">
      <div className="inline-flex w-full rounded-md bg-(--color-background-elevated-secondary) p-0.5">
          ...
          return (
            <button
              ...
              className={cn(
                ...
                active
                  ? "bg-(--composer-surface) text-(--color-text-foreground) shadow-xs"
...
export default function Sidebar() {
  ...
  useEffect(() => {
    ...
    debugWindow.ydszbuddyShowFeatureFlags = showFeatureFlags;
    debugWindow.ydszbuddyHideFeatureFlags = hideFeatureFlags;
    window.addEventListener("storage", updateVisibility);
    updateVisibility();

    return () => {
      window.removeEventListener("storage", updateVisibility);
      if (debugWindow.ydszbuddyShowFeatureFlags === showFeatureFlags) {
        delete debugWindow.ydszbuddyShowFeatureFlags;
      }
      if (debugWindow.ydszbuddyHideFeatureFlags === hideFeatureFlags) {
        delete debugWindow.ydszbuddyHideFeatureFlags;
      }
    };
  }, []);
  ...
  const [chatSectionExpanded] = useState(
    () => readSidebarUiState().chatSectionExpanded,
  );
  const [chatThreadListExpanded] = useState(
  ...
  const terminalWorkspaceOpen = shouldRenderTerminalWorkspace({
    ...
    terminalOpen,
    runtimeMode: routeThreadSummary?.runtimeMode,
  });
  ...
  const handleSidebarViewChange = useCallback(
    (view: "threads" | "workspace") => {
      if (view === "workspace") {
        ...
        navigateToWorkspace(fallbackWorkspaceId);
        return;
      }

      // Work mode: always land on the Work home page for a predictable toggle.
      void navigate({ to: "/" });
    },
    [navigate, navigateToWorkspace, workspacePages],
  );
  ...
  const confirmAndDeleteThread = useCallback(
    async (threadId: ThreadId) => {
      ...
      // 显示带撤销按钮的 Toast
      // 注意:由于后端不支持软删除,撤销机制通过 archive → 延迟 delete 实现。
      // 但当前 deleteThread 已直接执行 thread.delete,无法撤销。
      // 此处仅作为通知,后续需要后端支持软删除后才能实现真正的撤销。
      toastManager.add({
        type: "info",
        title: messages.sidebar.threadDeletedToastTitle,
        description: messages.sidebar.threadDeletedToastDescription(thread.title),
        timeout: 4000,
      });
    },
    [appSettings.confirmThreadDelete, deleteThread, sidebarThreadSummaryById, messages.sidebar],
  );
  ...
  // 为屏幕阅读器构造拖拽公告文本：根据 id 在 sortedProjects 中查找标题
  const getProjectAnnouncementLabel = useCallback(
    (
      id: string | number,
      projects: ReadonlyArray<{ id: string; name?: string; title?: string }>,
    ) => {
      const found = projects.find((p) => p.id === String(id));
      return found?.title ?? found?.name ?? String(id);
    },
    [],
  );
  ...
  const commitProjectRename = useCallback(
    (projectId: ProjectId, nextName: string, previousLocalName: string | null) => {
      ...
      // 乐观更新本地状态
      renameProjectLocally(projectId, resolvedName);
      cancelProjectRename();
      // 同步到后端，确保跨设备/重启后名称不丢失
      const api = readNativeApi();
      if (!api) return;
      void api.orchestration
        .dispatchCommand({
          ...
          commandId: newCommandId(),
          projectId,
          ...(resolvedName ? { title: resolvedName } : {}),
        })
        .catch((error) => {
          console.error("Failed to sync project rename to server", { projectId, error });
          toastManager.add({
            type: "error",
            title: messages.sidebar.projectRenameSyncError,
          });
        });
    },
    [cancelProjectRename, renameProjectLocally, messages.sidebar.projectRenameSyncError],
  );
  ...
  function renderThreadArchiveAction(
    ...
    if (isPendingConfirmation) {
      return (
        <button
          type="button"
          aria-label={messages.sidebar.confirmArchive}
        ...
        >
          <span>{messages.sidebar.confirm}</span>
        </button>
      );
    }
    ...
    return (
      <div key={thread.id} className="group/thread-row relative w-full">
        <div
          ...
          data-thread-item
          className={cn(
            "grid h-8 w-full items-center gap-x-1.5 rounded-md px-2 text-left text-(length:--app-font-size-ui,12px) transition-colors cursor-pointer",
            ...
            isActive
              ...
              : "text-foreground/89 hover:bg-(--sidebar-accent)",
          )}
        ...
        >
          ...
          <div className="flex min-w-0 max-w-12 shrink items-center justify-end">
            {projectLabel ? (
              <span className="truncate text-right text-(length:--app-font-size-ui-meta,10px) text-muted-foreground/38">
                {projectLabel}
              </span>
            ) : null}
          </div>
  ...
  function renderThreadRow(
    ...
    const toggleButtonClassName = isHighlighted
      ? "border-(--color-border) bg-(--color-background-button-secondary) text-(--color-text-foreground-secondary) hover:bg-(--color-background-button-secondary-hover) hover:text-(--color-text-foreground)"
      : "border-(--color-border-light) bg-(--color-background-elevated-secondary) text-(--color-text-foreground-secondary) hover:border-(--color-border) hover:bg-(--color-background-button-secondary-hover) hover:text-(--color-text-foreground)";
          ...
          >
            ...
            {renamingThreadId === thread.id ? (
              <input
                ...
                className="min-w-0 flex-1 truncate rounded-md border border-ring bg-transparent px-1.5 py-0.5 text-(length:--app-font-size-ui,12px) outline-none"
                value={renamingTitle}
              ...
              <span
                className={cn(
                  "min-w-0 flex-1 truncate text-(length:--app-font-size-ui,12px)",
      ...
      <div className="group/collapsible">
        <div className="group/project-header relative">
          <SidebarMenuButton
            ...
            size="sm"
            className={`h-9 gap-2 rounded-md px-2 py-0.5 text-left text-(length:--app-font-size-ui,13px) font-medium transition-[padding] duration-150 ease-out hover:bg-(--sidebar-accent) group-hover/project-header:bg-(--sidebar-accent) group-hover/project-header:pr-19 group-hover/project-header:text-(--sidebar-accent-foreground) group-focus-within/project-header:pr-19 ${
            ...
            <div className="flex min-w-0 flex-1 items-baseline gap-2 overflow-hidden">
              {isRenamingProject ? (
                <input
                  ...
                  className="min-w-0 flex-1 rounded-md border border-ring bg-transparent px-1.5 py-0.5 text-(length:--app-font-size-ui,12px) font-normal text-foreground outline-none"
                ...
                <>
                  <span className="truncate font-system-ui text-(length:--app-font-size-ui,13px) font-medium text-foreground/90">
                    {project.name}
                  </span>
                  {project.localName ? (
                    <span className="shrink-0 truncate text-(length:--app-font-size-ui,12px) text-muted-foreground/40">
                      {project.folderName}
                    </span>
                  ) : null}
                </>
            ...
            <TooltipTrigger
              render={
                <SidebarMenuAction
                  ...
                  showOnHover
                  className="top-2 right-7.5 size-5 p-0 text-muted-foreground/70 transition-colors hover:text-foreground"
                  onClick={(event) => {
                    event.preventDefault();
                    event.stopPropagation();
            ...
            <TooltipTrigger
              render={
                <SidebarMenuAction
                  ...
                  showOnHover
                  className="top-2 right-13.5 size-5 p-0 text-muted-foreground/70 transition-colors hover:text-foreground"
                  onClick={(event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    void handleNewThread(project.id, {
          ...
          <Tooltip>
            <TooltipTrigger
              render={
                <SidebarMenuAction
                  ...
                  showOnHover
                  className="top-2 right-1.5 size-5 p-0 text-muted-foreground/70 transition-colors hover:text-foreground"
                  onClick={(event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    void handleNewThread(project.id, {
            ...
            <TooltipPopup side="top">
              {newThreadShortcutLabel
                ? `${messages.sidebar.newChat} (${newThreadShortcutLabel})`
              ...
              {hasHiddenThreads && !isThreadListExpanded && (
                <SidebarMenuSubItem className="w-full">
                  <SidebarMenuSubButton
                    ...
                    data-thread-selection-safe
                    size="sm"
                    className="h-7 w-full translate-x-0 justify-start rounded-lg pr-2 pl-8 text-left text-(length:--app-font-size-ui,12px) text-muted-foreground/79 hover:bg-(--sidebar-accent)"
                    onClick={() => {
        ...
        .then((nextState: DesktopUpdateState) => {
          ...
          if (nextState.status === "up-to-date") {
            toastManager.add({
              ...
              description: `ydsz-buddy ${nextState.currentVersion} is already the newest version.`,
            });
            return;
          }
  ...
  const sidebarBrand = (
    <div className="flex h-[44px] min-w-0 items-center justify-between px-4">
      {brandWordmark}
      <SidebarTrigger
        className="size-7 shrink-0 text-muted-foreground/75 hover:text-foreground"
        aria-label="Toggle thread sidebar"
  ...
  return (
    <>
      ...
      <SidebarContent className="gap-0 font-system-ui">
        ...
        {isOnSettings ? (
                        ...
                        return (
                          <SidebarMenuSubItem key={item.id} className="w-full">
                            <SidebarMenuSubButton
                              ...
                              size="sm"
                              isActive={isActive}
                              className="group/settings-nav-item h-7.5 w-full justify-start gap-2 rounded-lg px-2 py-0.5 text-(length:--app-font-size-ui,12px) font-normal hover:bg-(--sidebar-accent)"
                              onClick={() => {
                                void navigate({
                                  to: "/settings",
                                  search: (previous) => ({
                            ...
                            >
                              ...
                              <span
                                className={cn(
                                  "truncate text-(length:--app-font-size-ui,12px) leading-5",
          ...
          <>
            ...
            <SidebarSegmentedPicker activeView={isOnWorkspace ? "workspace" : "threads"} onSelectView={handleSidebarViewChange} />
            ...
            <SidebarGroup className="px-1.5 pt-1 pb-1.5">
              <SidebarMenu className="gap-0.5">
                <SidebarPrimaryAction
                  ...
                  shortcutLabel={newThreadShortcutLabel ?? newChatShortcutLabel}
                />
                <SidebarPrimaryAction
                  icon={SearchIcon}
            ...
            <SidebarGroup className="px-1.5 py-1.5">
                        ...
                        <TooltipTrigger
                          render={
                            <button
                              ...
                              className="inline-flex size-7 items-center justify-center rounded-md text-muted-foreground/70 transition-colors hover:bg-(--sidebar-accent) hover:text-foreground disabled:cursor-default disabled:opacity-45"
                      ...
                      <TooltipTrigger
                        render={
                          <button
                            ...
                            className="inline-flex size-7 items-center justify-center rounded-md text-muted-foreground/70 transition-colors hover:bg-(--sidebar-accent) hover:text-foreground"
                ...
                {shouldShowProjectPathEntry && (
                  <div className="mb-2.5 px-1">
                    <button
                      type="button"
                      className="flex h-8 w-full items-center justify-center gap-2 rounded-lg bg-(--color-background-elevated-secondary) px-2 text-(length:--app-font-size-ui,12px) font-normal text-(--color-text-foreground-secondary) transition-colors hover:bg-(--color-background-button-secondary-hover) hover:text-(--color-text-foreground) disabled:opacity-50"
                ...
                {isManualProjectSorting ? (
                  <DndContext
                    ...
                    accessibility={{
                      announcements: {
                        onDragStart: ({ active }) =>
                          `开始重新排序项目 ${getProjectAnnouncementLabel(active.id, sortedProjects)}`,
                        onDragOver: ({ active, over }) => {
                          if (!over) return undefined;
                          return `项目 ${getProjectAnnouncementLabel(active.id, sortedProjects)} 移动到 ${getProjectAnnouncementLabel(over.id, sortedProjects)} 之上`;
                        },
                        onDragEnd: ({ active, over }) => {
                          if (!over) {
                            return `项目 ${getProjectAnnouncementLabel(active.id, sortedProjects)} 拖拽已取消`;
                          }
                          return `项目 ${getProjectAnnouncementLabel(active.id, sortedProjects)} 已放置到 ${getProjectAnnouncementLabel(over.id, sortedProjects)} 位置`;
                        },
                        onDragCancel: ({ active }) =>
                          `项目 ${getProjectAnnouncementLabel(active.id, sortedProjects)} 拖拽已取消`,
                      },
                      screenReaderInstructions: {
                        draggable:
                          "要重新排序项目，请按下空格键进入拖拽模式，使用上下方向键移动位置，回车确认，Esc 取消。",
                      },
                    }}
                  >
                ...
                {projectEmptyState === "loading" && (
                  ...
                  >
                    <div className="text-center text-(length:--app-font-size-ui,12px) text-muted-foreground/58">
                ...
                {projectEmptyState === "empty" && (
                  <div className="px-2 pt-4 text-center">
                    <div className="text-(length:--app-font-size-ui,12px) text-muted-foreground/79">
                      {messages.sidebar.noProjectsYet}
                    </div>
                    <div className="mx-auto mt-1 max-w-45 text-(length:--app-font-size-ui-meta,10px) leading-4 text-muted-foreground/58">
                      {messages.sidebar.noProjectsYetDescription}
                    </div>
                    <button
                      type="button"
                      className="mx-auto mt-3 inline-flex h-8 items-center justify-center gap-2 rounded-lg bg-(--color-background-elevated-secondary) px-3 text-(length:--app-font-size-ui,12px) font-normal text-(--color-text-foreground-secondary) transition-colors hover:bg-(--color-background-button-secondary-hover) hover:text-(--color-text-foreground) disabled:opacity-50"
                      onClick={() => {
                        if (isDesktop) {
                          void handlePickFolder();
                          return;
      ...
      <SidebarSeparator />
      <SidebarFooter className="gap-1.5 p-2 font-system-ui">
        {isOnSettings ? null : <SidebarAccountBar />}
...
function SidebarSearchPaletteController(props: {
  ...
  ).filter((_, index) => supportsThreadImport(importProviderCapabilityQueries[index]?.data));
