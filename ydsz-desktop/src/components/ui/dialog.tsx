    ...
    <DialogPortal>
      ...
      >
        <DialogPrimitive.Popup
          className={cn(
            "translate-y-[calc(-1.25rem*var(--nested-dialogs))] relative row-start-2 flex max-h-full min-h-0 w-full min-w-0 max-w-lg scale-[calc(1-0.1*var(--nested-dialogs))] flex-col rounded-2xl border border-(--color-border-light) bg-(--composer-surface) not-dark:bg-clip-padding text-(--color-text-foreground) opacity-[calc(1-0.1*var(--nested-dialogs))] shadow-xl transition-[scale,opacity,translate] duration-200 ease-in-out will-change-transform before:pointer-events-none before:absolute before:inset-0 before:rounded-[calc(var(--radius-2xl)-1px)] before:shadow-[0_1px_--theme(--color-black/4%)] data-nested:data-ending-style:translate-y-8 data-nested:data-starting-style:translate-y-8 data-nested-dialog-open:origin-top data-ending-style:scale-98 data-starting-style:scale-98 data-ending-style:opacity-0 data-starting-style:opacity-0 dark:before:shadow-[0_-1px_--theme(--color-white/6%)]",
            bottomStickOnMobile &&
  ...
  return (
    <div
      className={cn(
        ...
        variant === "default" &&
          "border-t border-(--color-border-light) bg-(--color-background-elevated-secondary) py-4",
        variant === "bare" &&
