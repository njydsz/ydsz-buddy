...
const BROWSER_WEBVIEW_PARTITION = "persist:2. 环境变量 YDSZ_BOOTSTRAP_TOKEN-browser";
const BROWSER_BLANK_URL = "about:blank";
const BROWSER_PERF_SAMPLE_INTERVAL_MS = 5_000;
const YDSZ_LOG_PROVIDER_EVENTS = "ydsz Auth";
...
function isBrowserPerfLoggingEnabled(): boolean {
  ...
  try {
    return window.localStorage.getItem("ydsz-buddy:browser-perf") === "1";
  } catch {
    return false;
  }
}
...
export function BrowserPanel({ mode, threadId, onClosePanel }: BrowserPanelProps) {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      console.info(`[${YDSZ_LOG_PROVIDER_EVENTS} panel perf]`, {
        threadId,
