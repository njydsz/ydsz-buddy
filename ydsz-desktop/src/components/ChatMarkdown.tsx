...
import { splitTextWithReviewReferences } from "../lib/reviewFileReferences";
...
import { ReviewFileReferenceLink } from "./chat/ReviewFileReferenceLink";
...
interface ChatMarkdownProps {
  ...
  /**
   * 是否启用 Review 模式行级引用（`path:line` 渲染为可点击链接）
   *
   * - 默认 false：保持向后兼容
   * - Review 模式下开启：AI 输出中的 `src/foo.ts:42` 等引用变为可点击，
   *   点击在用户偏好的编辑器中跳转到对应行
   */
...
interface SuspenseShikiCodeBlockState {
  highlighter: DiffsHighlighter | null;
  promise: Promise<DiffsHighlighter> | null;
}
...
function SuspenseShikiCodeBlock({
  ...
  const stateRef = useRef<SuspenseShikiCodeBlockState>({ highlighter: null, promise: null });
  if (!stateRef.current.promise) {
    stateRef.current.promise = getHighlighterPromise(language);
    stateRef.current.promise.then((highlighter) => {
      stateRef.current.highlighter = highlighter;
    }).catch(() => {});
  }

  if (!stateRef.current.highlighter) {
    // eslint-disable-next-line @typescript-eslint/no-throw-literal
    throw stateRef.current.promise;
  }

  const highlighter = stateRef.current.highlighter;
...
function ChatMarkdown({
  ...
  const markdownComponents = useMemo<Components>(
    () => ({
      // 行级引用处理（Review 模式）
      // 优先用 text 组件拦截普通文本，再在 inlineCode 节点中递归处理。
      // 多个 segment 时穿插渲染引用链接；纯文本时直通。
      text({ node }) {
        // react-markdown v10 的 text 节点类型未暴露 value，这里通过 children 兜底提取
        const nodeValue = (node as unknown as { value?: unknown })?.value;
        const value = typeof nodeValue === "string" ? nodeValue : "";
        if (!value || !isReviewMode) {
          return <>{value}</>;
        }
        const segments = splitTextWithReviewReferences(value, cwd);
        ...
        return (
          <>
            {segments.map((segment, idx) =>
              segment.kind === "text" ? (
                <Fragment key={`txt-${idx}`}>{segment.text}</Fragment>
              ) : (
                <ReviewFileReferenceLink
                  key={`ref-${idx}`}
      ...
      code({ className: codeClassName, children }) {
        ...
        return (
          <code className={codeClassName}>
            {segments.map((segment, idx) =>
              segment.kind === "text" ? (
                <Fragment key={`code-txt-${idx}`}>{segment.text}</Fragment>
              ) : (
                <ReviewFileReferenceLink
                  key={`code-ref-${idx}`}
                  reference={segment.reference}
      ...
      pre({ node: _node, children, ...props }) {
        ...
        // mermaid 代码块走专用渲染器（懒加载 mermaid 库）
        const languageMatch = codeBlock.className
          ? CODE_FENCE_LANGUAGE_REGEX.exec(codeBlock.className)
          : null;
        if (languageMatch && languageMatch[1].toLowerCase() === "mermaid") {
          return (
            <CodeHighlightErrorBoundary fallback={<pre {...props}>{children}</pre>}>
              <MermaidBlock code={codeBlock.code} isStreaming={isStreaming} />
    ...
    [cwd, diffThemeName, isReviewMode, isStreaming, onImageExpand],
  );
