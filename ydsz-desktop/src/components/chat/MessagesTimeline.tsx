...
// Purpose: Renders the chat transcript rows and lets VirtualizedList own scrolling/follow behavior.
// Layer: Web chat presentation component
// Exports: MessagesTimeline

import { type MessageId, ThreadId, type TurnId } from "@ydsz-buddy/contracts";
import { resolveLatestTailUserMessageEditTarget } from "@njydsz/shared/conversationEdit";
import {
  memo,
  useCallback,
  useEffect,
...
import { StreamingAriaAnnouncer, StreamingProgressBar } from "./StreamingAriaAnnouncer";
import {
  ContextMenu,
  ContextMenuContent,
  ContextMenuItem,
...
interface MessagesTimelineProps {
  ...
  listRef?: MutableRefObject<VirtualizedListRef | null>;
  ...
  onMessagesClickCapture?: React.MouseEventHandler<HTMLDivElement>;
  onMessagesMouseUp?: React.MouseEventHandler<HTMLDivElement>;
  onMessagesPointerCancel?: React.PointerEventHandler<HTMLDivElement>;
  onMessagesPointerDown?: React.PointerEventHandler<HTMLDivElement>;
  onMessagesPointerUp?: React.PointerEventHandler<HTMLDivElement>;
  onMessagesScroll?: React.UIEventHandler<HTMLDivElement>;
  onMessagesTouchEnd?: React.TouchEventHandler<HTMLDivElement>;
  onMessagesTouchMove?: React.TouchEventHandler<HTMLDivElement>;
  onMessagesTouchStart?: React.TouchEventHandler<HTMLDivElement>;
  onMessagesWheel?: React.WheelEventHandler<HTMLDivElement>;
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const [pendingRevertMessageId, setPendingRevertMessageId] = useState<MessageId | null>(null);
  const fallbackListRef = useRef<VirtualizedListRef | null>(null);
  ...
  // P-5 aria-live: 找到当前流式消息,用于屏幕阅读器播报
  ...
  useEffect(() => {
    ...
    const frameId = window.requestAnimationFrame(() => {
      resolvedListRef.current?.scrollToBottom();
    });
    return () => {
  ...
  const handleListScroll = useCallback<NonNullable<MessagesTimelineProps["onMessagesScroll"]>>(
    (event) => {
      ...
      // VirtualizedList handles isAtBottom detection internally via onIsAtBottomChange
    },
    [onMessagesScroll],
  );
            ...
            <div>
              ...
              {showOverflowToggle && (
                <div className="mt-1.5 flex items-center justify-start gap-2 px-0.5">
                  ...
                  >
                    {isExpanded
                      ? messages.chat.timeline.showLess
                      : messages.chat.timeline.showMoreCount(hiddenCount)}
                  </button>
                </div>
              )}
            </div>
            ...
            <ContextMenu>
                      ...
                      >
                        ...
                        {userMessagePreview.collapsible && (
                          ...
                          >
                            {userMessageExpanded
                              ? messages.chat.timeline.showLess
                              : messages.chat.timeline.showMore}
                          </button>
                        )}
                      </div>
                    ) : null}
                    {!isEditingThisMessage && (
                      <div className="flex items-center justify-end gap-1.5 pr-0.5">
                        <div className="flex items-center gap-1 opacity-0 transition-opacity duration-200 focus-within:opacity-100 group-hover:opacity-100">
                          ...
                          {showEditUserMessage && (
                            <MessageActionButton
                              label={messages.chat.timeline.editMessage}
                              tooltip={messages.chat.timeline.editAndResend}
                          ...
                          <MessageActionButton
                            label={messages.chat.timeline.revertLabel}
                            tooltip={
                              canRevertAgentWork
                                ? messages.chat.timeline.revertTooltip
                                : messages.chat.timeline.undoUnavailable
                            }
                            disabled={!canRevertAgentWork || isRevertingCheckpoint || isWorking}
                            className="disabled:text-muted-foreground/35"
                            onClick={() => setPendingRevertMessageId(row.message.id)}
                          >
              ...
              <ContextMenuContent>
                ...
                >
                  ...
                  {messages.chat.timeline.revertLabel}
                </ContextMenuItem>
              </ContextMenuContent>
            </ContextMenu>
          );
                ...
                <div className="my-3 flex items-center gap-3">
                  ...
                  >
                    {row.completionSummary
                      ? messages.chat.timeline.responseWithSummary(row.completionSummary)
                      : messages.chat.timeline.response}
                  </span>
                  <span className="h-px flex-1 bg-border" />
                </div>
              )}
                      ...
                      inlineToolEntries.length > MAX_VISIBLE_INLINE_TOOL_ENTRIES && (
                        <div className="py-0.5">
                          ...
                          >
                            {inlineToolExpanded
                              ? messages.chat.timeline.showLess
                              : messages.chat.timeline.moreToolCalls(hiddenInlineToolCount)}
                          </button>
                        </div>
                      )}
                  </div>
                      ...
                      <div className="flex items-center justify-between gap-2 border-b border-(--color-border-light) px-3 py-2">
                        ...
                        >
                          {checkpointFiles.length === 1
                            ? messages.chat.timeline.oneFileChanged
                            : messages.chat.timeline.filesChanged(checkpointFiles.length)}
                        </span>
                        <div className="flex items-center gap-4">
                          <button
                            ...
                            aria-label={
                              fileChangesExpanded
                                ? messages.chat.timeline.collapseFiles
                                : messages.chat.timeline.expandFiles
                            }
                            onClick={(event) => {
                          ...
                          {canUndo && (
                            <button
                              ...
                              style={{ fontSize: chatTypographyStyle.fontSize }}
                              onClick={() => setPendingRevertMessageId(correspondingUserMessageId)}
                            >
                              Undo
      ...
      {row.kind === "proposed-plan" && (
        <div className="min-w-0 px-1 py-0.5">
          <ProposedPlanCard
            ...
            chatTypographyStyle={chatTypographyStyle}
            onApprove={onApproveProposedPlan}
            onReject={onRejectProposedPlan}
            onRevise={onReviseProposedPlan}
          />
        </div>
      )}
        ...
        >
          <span>
            {row.createdAt ? (
              <>
                {nowIso ? (
                  messages.chat.timeline.workingFor(
                    formatWorkingTimer(row.createdAt, nowIso) ?? "0s",
                  )
                ) : (
                  <>
                    {messages.chat.timeline.workingForPrefix}
                    <WorkingTimer createdAt={row.createdAt} />
                  </>
                )}
              </>
            ) : (
              messages.chat.timeline.working
            )}
          </span>
  ...
  return (
    <>
      {/* P-5 aria-live: 流式响应屏幕阅读器播报 + 进度条 */}
      <StreamingAriaAnnouncer
        streamingText={activeStreamingMessage?.text ?? null}
        isStreaming={Boolean(activeStreamingMessage)}
        justCompleted={!activeStreamingMessage && isWorking}
        streamingStartedAt={
          activeStreamingMessage && activeTurnStartedAt
            ? Date.parse(activeTurnStartedAt)
            : null
        }
      />
      {activeStreamingMessage && isWorking && (
        <div className="absolute inset-x-0 top-0 z-10">
          <StreamingProgressBar
            progress={activeTurnInProgress ? 15 : 5}
      ...
      <VirtualizedList
        ...
        estimateSize={90}
        followOutput={followLiveOutput}
        bottomThreshold={0.1}
        maintainScrollPosition
        onScroll={handleListScroll}
        onIsAtBottomChange={onIsAtEndChange}
        {...(onMessagesClickCapture
          ? { onClickCapture: onMessagesClickCapture as unknown as never }
          : {})}
      ...
      <RevertConfirmationDialog
        ...
        messageId={pendingRevertMessageId}
        messageText={
          pendingRevertMessageId
            ? (() => {
                const row = rows.find(
                  (r): r is Extract<MessagesTimelineRow, { kind: "message" }> =>
                    r.kind === "message" && r.message.id === pendingRevertMessageId,
                );
        ...
        onConfirm={() => {
          if (pendingRevertMessageId) {
            onRevertUserMessage(pendingRevertMessageId);
            setPendingRevertMessageId(null);
          }
        }}
        messages={messages}
        ...
        summaryLine={
          pendingRevertMessageId
            ? buildRevertSummaryLine(
                rows,
                pendingRevertMessageId,
                revertTurnCountByUserMessageId.get(pendingRevertMessageId),
              )
            : null
        }
      />
  ...
  return (
    ...
    >
      <textarea
        ...
        rows={1}
        aria-label={messages.chat.timeline.editMessage}
...
/**
 * 计算"回滚这条用户消息会带走多少条助手产物"。
 *
 * 用于在 RevertConfirmationDialog 顶部给用户一句 summary line，
 * 让用户对"撤销后会发生什么"有量化预期，而不是只看 200 字符预览。
 *
 * # 入参
 *
 * - `rows` —— 当前时间线行（message + work + tool-call 混合）
 * - `userMessageId` —— 被回滚的用户消息 ID
 * - `revertTurnCount` —— 该消息对应的可回滚 turn 数（来自 ES / checkpoint）
 *
 * # 返回
 *
 * - 形如"将删除 1 条助手消息 + 3 条工具调用"的中文 summary。
 * - 若 `revertTurnCount` 缺失或为 0，仅基于"该用户消息之后的消息行"做估算。
 */
export function buildRevertSummaryLine(
  ...
  for (const row of rows) {
    if (!sawUser) {
      if (row.kind === "message" && row.message.id === userMessageId) {
        sawUser = true;
      }
      continue;
    }
    if (row.kind === "message") {
      if (row.message.role === "assistant") {
        assistantCount += 1;
        // 行内挂载的工具/Work 日志(在 row 上,不在 message 上)
        toolCallCount += row.inlineWorkEntries?.length ?? 0;
      } else if (row.message.role === "user") {
        // 后续用户消息：通常会终止这段"将要回滚的对话段"
        break;
      }
    } else if (row.kind === "work") {
      workEntryCount += row.groupedEntries.length;
    } else if (row.kind === "proposed-plan" || row.kind === "working") {
      // proposal/working 也算要被回滚的产物
      workEntryCount += 1;
    }
  }
  const parts: string[] = [];
  if (assistantCount > 0) {
    parts.push(`${assistantCount} 条助手消息`);
  }
  if (toolCallCount > 0) {
    parts.push(`${toolCallCount} 条工具调用`);
  }
  if (workEntryCount > 0) {
    parts.push(`${workEntryCount} 条 Work 日志`);
  }
  if (parts.length === 0) {
    if (revertTurnCount && revertTurnCount > 0) {
      return `将回滚 ${revertTurnCount} 个 Turn 的所有产物`;
    }
    return "将回滚到该消息之前的状态";
  }
  const head =
    revertTurnCount && revertTurnCount > 0
      ? `将回滚 ${revertTurnCount} 个 Turn（含 `
      : "将删除 ";
  const tail = parts.join(" + ");
  const close = revertTurnCount && revertTurnCount > 0 ? " ）" : "";
...
function RevertConfirmationDialog({
  ...
  messages,
  /**
   * 倒计时窗口（秒）。在窗口期内可一键"撤销本次操作"。
   * 进度条会同步从满格到 0，到期后对话框自动关闭（视为取消）。
   *
   * 取值约束：
   * - 0 或负数：禁用倒计时与进度条
   * - 典型值：30（与 project memory 中"P1 enhancement 30s undo"约定一致）
   */
  undoWindowSeconds = 30,
  /**
   * 实际将要回滚的助手消息/产物数量（用于在对话框里给一句 summary line）。
   * 可选；不传则不渲染 summary 行。
   */
  summaryLine,
}: {
  ...
  // 30s 倒计时：用于在撤销确认后允许 30s 内的"取消撤销"(反悔)
  // 也用作对话框存在期间的"自动关闭"计时。
  const [remainingMs, setRemainingMs] = useState<number>(0);
  useEffect(() => {
    if (!open || undoWindowSeconds <= 0) {
      setRemainingMs(0);
      return;
    }
    ...
    // 依赖 onOpenChange 引用稳定性由调用方保证
  }, [open, undoWindowSeconds, onOpenChange]);

  const totalMs = Math.max(1, undoWindowSeconds * 1000);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader>
          <DialogTitle data-testid="revert-confirm-title">
            {messages.chat.timeline.revertConfirmTitle}
          </DialogTitle>
          <DialogDescription data-testid="revert-confirm-description">
            {messages.chat.timeline.revertConfirmDescription}
          </DialogDescription>
        </DialogHeader>
        <DialogPanel>
          {previewText && (
            <div
              ...
              data-testid="revert-confirm-preview"
            >
              <div className="mb-1.5 text-xs font-medium text-muted-foreground">
                {messages.chat.timeline.revertMessagePreview}
              </div>
              <div className="max-h-32 overflow-y-auto text-sm text-foreground/90">
                {previewText}
              </div>
            </div>
          )}
          {summaryLine && (
            <div
              ...
              data-testid="revert-confirm-summary"
            >
              {summaryLine}
            </div>
          )}
          <div className="mt-3 text-sm text-muted-foreground">
            {messages.chat.timeline.revertWarning}
          </div>
          {undoWindowSeconds > 0 && (
            <div
              className="mt-3 space-y-1.5"
              data-testid="revert-confirm-undo-window"
            >
              <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                <span>撤销确认窗口</span>
                <span className="tabular-nums" data-testid="revert-confirm-remaining">
                  剩余 {remainingSec}s
                </span>
              </div>
              <div
                ...
                data-testid="revert-confirm-progress"
              >
                <div
        ...
        <DialogFooter>
          <Button
            ...
            data-testid="revert-confirm-cancel"
          >
          ...
          <Button
            variant="destructive"
            onClick={onConfirm}
            data-testid="revert-confirm-submit"
          >
            <Undo2Icon className="mr-2 size-4" />
            {messages.chat.timeline.revertConfirmButton}
          </Button>
        </DialogFooter>
      </DialogPopup>
    </Dialog>
