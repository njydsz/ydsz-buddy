...
import { type ModelSlug, type ProviderKind, type ServerProviderStatus, PROVIDER_DISPLAY_NAMES } from "~/contracts";
...
const FAVORITE_MODEL_STORAGE_KEYS = {
  cursor: "ydsz-buddy:cursor-favourite-models:v1",
  kilo: "ydsz-buddy:kilo-favourite-models:v1",
  opencode: "ydsz-buddy:opencode-favourite-models:v1",
  pi: "ydsz-buddy:pi-favourite-models:v1",
} as const;
...
export const ProviderModelPicker = memo(function ProviderModelPicker(props: {
  ...
  // P3-8: 能力 chip 过滤(每个 provider 独立状态)
  // 用 provider id 作 key,展开 provider 时 init,关闭时保留
  // (避免每次重开都从头开始,符合大厂「状态保留」基线)
  ...
  const handleModelChange = (provider: ProviderKind, value: string) => {
    ...
    // Provider 切换时显示即时反馈 Toast
    if (provider !== props.provider) {
      const providerName = PROVIDER_DISPLAY_NAMES[provider] ?? provider;
      toastManager.add({
        type: "success",
        title: messages.providerFeedback.switchedTitle(providerName),
        description: messages.providerFeedback.switchedDescription,
        timeout: 3000,
      });
    }
