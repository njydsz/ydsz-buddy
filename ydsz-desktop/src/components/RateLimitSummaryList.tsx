...
export function RateLimitSummaryList({
  ...
  if (rows.length === 0) {
    return (
      <p className="text-(length:--app-font-size-chat-meta,10px) text-muted-foreground">
  ...
  return (
    <>
      {rows.map((row) => (
        <div
          key={row.id}
          className="flex items-center justify-between text-(length:--app-font-size-chat,12px)"
        >
          <span className="font-medium text-foreground">{row.label}</span>
          <span className="flex items-center gap-2 tabular-nums text-(length:--app-font-size-chat-meta,10px) text-muted-foreground">
