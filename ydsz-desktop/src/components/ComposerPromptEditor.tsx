...
function ComposerPromptEditorInner({
  ...
  return (
    <ComposerTerminalContextActionsContext.Provider value={terminalContextActions}>
      <div className="relative" aria-label="Message input">
        <PlainTextPlugin
          contentEditable={
            <ContentEditable
              className={cn(
                "font-system-ui block max-h-[200px] min-h-12 w-full overflow-y-auto whitespace-pre-wrap wrap-break-word bg-transparent text-(length:--app-font-size-chat,12px) leading-relaxed text-foreground focus:outline-none",
                className,
              )}
          ...
          placeholder={
            terminalContexts.length > 0 ? null : (
              <div className="font-system-ui pointer-events-none absolute inset-0 text-(length:--app-font-size-chat,12px) leading-relaxed text-muted-foreground/40">
                {placeholder}
              </div>
            )
          }
...
>(function ComposerPromptEditor(
  ...
  const initialConfig = useMemo<InitialConfigType>(
    () => ({
      namespace: "2. 环境变量 YDSZ_BOOTSTRAP_TOKEN-composer-editor",
      editable: true,
      nodes: [
        ComposerMentionNode,
        ComposerSkillNode,
