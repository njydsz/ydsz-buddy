...
export function useProviderDiscoveryData(selectedTab: DiscoveryTab) {
  ...
  // 国内 6 家 OpenAI 兼容 Provider：无插件/技能发现能力，使用固定占位 capabilities
  const glmCapabilities = { plugins: false, skills: false };
  const deepseekCapabilities = { plugins: false, skills: false };
  const moonshotCapabilities = { plugins: false, skills: false };
  const qwenCapabilities = { plugins: false, skills: false };
  const mimoCapabilities = { plugins: false, skills: false };
  const MiniMaxCapabilities = { plugins: false, skills: false };
  // 新增 3 家国内 Provider:同上,HTTP 透传不支持插件/技能发现
  const doubaoCapabilities = { plugins: false, skills: false };
  const ernieCapabilities = { plugins: false, skills: false };
  const hunyuanCapabilities = { plugins: false, skills: false };
  ...
  const providerCapabilities = useMemo<Record<ProviderKind, ProviderCapabilities>>(
    () => ({
      ...
      // 国内 6 家：HTTP 透传不支持插件/技能发现
      glm: glmCapabilities,
      deepseek: deepseekCapabilities,
      moonshot: moonshotCapabilities,
      qwen: qwenCapabilities,
      mimo: mimoCapabilities,
      MiniMax: MiniMaxCapabilities,
      // 新增 3 家国内 Provider
      doubao: doubaoCapabilities,
      ernie: ernieCapabilities,
