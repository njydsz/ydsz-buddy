 ...
 * @file 文件系统契约模块
 *
 * 本模块定义了 ydsz 工作区与文件系统交互的所有 Schema，涵盖目录浏览、文件读取、
...
export const FilesystemBrowseInput = Schema.Struct({
  partialPath: TrimmedNonEmptyString.pipe(Schema.maxLength(FILESYSTEM_PATH_MAX_LENGTH)),
  cwd: Schema.optional(TrimmedNonEmptyString.pipe(Schema.maxLength(FILESYSTEM_PATH_MAX_LENGTH))),
});
