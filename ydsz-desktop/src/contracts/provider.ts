 ...
 *
 * 本模块定义了 ydsz 工作区中 AI Provider（Claude / OpenAI / Codex / Cursor 等）
 * 的配置、密钥、模型、可用性等契约。
 *
 * ## 核心契约
 *
 ...
 * ## 协议设计
 *
 * - **密钥隔离**：API Key 存储在后端 `ydsz-auth::SecretStore`，前端不直接持有
...
export const ProviderSendTurnInput = Schema.Struct({
  ...
  attachments: Schema.optional(
    Schema.Array(ChatAttachment).pipe(Schema.maxItems(PROVIDER_SEND_TURN_MAX_ATTACHMENTS)),
  ),
