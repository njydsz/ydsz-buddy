 ...
 * @file 编排契约模块
 *
 * 本模块定义了 ydsz 工作区中编排层（Orchestration）的核心契约，
...
// Re-exported for backwards-compatibility with code that imports ProviderKind
// from "@ydsz-buddy/contracts" via "./orchestration". The canonical definition
// lives in "./baseSchemas".
export { ProviderKind };
...
export const ORCHESTRATION_WS_METHODS = {
  ...
  getTurnAiShareSnapshot: "orchestration.getTurnAiShareSnapshot",
...
// 国内 6 家 OpenAI 兼容 Provider：仅 provider + model 字段
// 暂不携带 options（HTTP 透传不需要），后端按 provider 路由
// 使用空 Struct 让 options 的类型为 `{} | undefined`，与 ModelSelection 联合兼容
const DomesticModelOptions = Schema.Struct({});
...
export const ModelSelection = Schema.Union(
  ...
  Schema.Struct({
    ...
    model: TrimmedNonEmptyString,
    options: Schema.optional(DomesticModelOptions),
  }),
  Schema.Struct({
  ...
  Schema.Struct({
    provider: Schema.Literal("moonshot"),
    model: TrimmedNonEmptyString,
    options: Schema.optional(DomesticModelOptions),
  }),
  Schema.Struct({
  ...
  // 新增 3 家国内 OpenAI 兼容 Provider:同上仅 provider + model + 空 options
  Schema.Struct({
    provider: Schema.Literal("doubao"),
    model: TrimmedNonEmptyString,
    options: Schema.optional(DomesticModelOptions),
  }),
  Schema.Struct({
    provider: Schema.Literal("ernie"),
    model: TrimmedNonEmptyString,
    options: Schema.optional(DomesticModelOptions),
  }),
  Schema.Struct({
    provider: Schema.Literal("hunyuan"),
    model: TrimmedNonEmptyString,
    options: Schema.optional(DomesticModelOptions),
  }),
);
...
// 全部 17 家 Provider 统一使用 HTTP API 模式
// 启动选项仅承载 API Key / Base URL 等连接参数
// 向后兼容：部分 Provider 仍支持本地二进制路径（binaryPath/homePath）
const HttpApiProviderStartOptions = Schema.Struct({
  apiKey: Schema.optional(TrimmedNonEmptyString),
  baseUrl: Schema.optional(TrimmedNonEmptyString),
});

// 扩展类型：支持 HTTP API 和本地二进制路径（向后兼容）
const LocalBinaryProviderStartOptions = Schema.Struct({
  apiKey: Schema.optional(TrimmedNonEmptyString),
  baseUrl: Schema.optional(TrimmedNonEmptyString),
...
export const CodexProviderStartOptions = LocalBinaryProviderStartOptions;
...
export const GlmProviderStartOptions = HttpApiProviderStartOptions;
export const DeepSeekProviderStartOptions = HttpApiProviderStartOptions;
export const MoonshotProviderStartOptions = HttpApiProviderStartOptions;
export const QwenProviderStartOptions = HttpApiProviderStartOptions;
export const MimoProviderStartOptions = HttpApiProviderStartOptions;
export const MiniMaxProviderStartOptions = HttpApiProviderStartOptions;
export const DoubaoProviderStartOptions = HttpApiProviderStartOptions;
export const ErnieProviderStartOptions = HttpApiProviderStartOptions;
export const HunyuanProviderStartOptions = HttpApiProviderStartOptions;
...
export const ProviderStartOptions = Schema.Struct({
  ...
  // 国内 9 家
  glm: Schema.optional(GlmProviderStartOptions),
  deepseek: Schema.optional(DeepSeekProviderStartOptions),
  moonshot: Schema.optional(MoonshotProviderStartOptions),
  qwen: Schema.optional(QwenProviderStartOptions),
  mimo: Schema.optional(MimoProviderStartOptions),
  MiniMax: Schema.optional(MiniMaxProviderStartOptions),
  doubao: Schema.optional(DoubaoProviderStartOptions),
  ernie: Schema.optional(ErnieProviderStartOptions),
  hunyuan: Schema.optional(HunyuanProviderStartOptions),
});
export type ProviderStartOptions = typeof ProviderStartOptions.Type;

// ydsz-buddy 0.3.0+: RuntimeMode expanded to Work/Code dimensions.
...
export const RuntimeMode = Schema.Literal(...RUNTIME_MODE_VALUES);
export type RuntimeMode = typeof RuntimeMode.Type;
export const DEFAULT_RUNTIME_MODE: RuntimeMode = "code";
// ydsz-buddy 0.2.0+: InteractionMode expanded to Chat/Plan/Agent/Review/Task.
// 旧版 "default" / "plan" 通过 Schema 的 filter 兜底为 "agent"。
const PROVIDER_INTERACTION_MODE_VALUES = ["chat", "plan", "agent", "review", "task"] as const;
export const ProviderInteractionMode = Schema.Literal(...PROVIDER_INTERACTION_MODE_VALUES);
...
export const TurnDispatchMode = Schema.Literal("queue", "steer");
export type TurnDispatchMode = typeof TurnDispatchMode.Type;
export const DEFAULT_TURN_DISPATCH_MODE: TurnDispatchMode = "queue";
/** Provider review target: uncommitted changes or a specific base branch */
...
export const ProviderUserInputAnswers = Schema.Record({ key: Schema.String, value: ProviderUserInputAnswer });
export type ProviderUserInputAnswers = typeof ProviderUserInputAnswers.Type;
export const ThreadHandoffBootstrapStatus = Schema.Literal("pending", "completed");
export type ThreadHandoffBootstrapStatus = typeof ThreadHandoffBootstrapStatus.Type;
export const ThreadEnvironmentMode = Schema.Literal("local", "worktree");
export type ThreadEnvironmentMode = typeof ThreadEnvironmentMode.Type;

export const OrchestrationMessageSource = Schema.Literal(
  "native",
  "handoff-import",
  "fork-import",
);
...
const ChatAttachmentId = TrimmedNonEmptyString.pipe(
...
export const ChatImageAttachment = Schema.Struct({
  ...
  mimeType: TrimmedNonEmptyString.pipe(Schema.maxLength(100), Schema.pattern(/^image\//i)),
...
export const ChatAssistantSelectionAttachment = Schema.Struct({
  ...
  text: TrimmedNonEmptyString.pipe(Schema.maxLength(CHAT_ASSISTANT_SELECTION_TEXT_MAX_CHARS)),
});
...
const UploadChatImageAttachment = Schema.Struct({
  ...
  mimeType: TrimmedNonEmptyString.pipe(Schema.maxLength(100), Schema.pattern(/^image\//i)),
  sizeBytes: NonNegativeInt.pipe(Schema.lessThanOrEqualTo(PROVIDER_SEND_TURN_MAX_IMAGE_BYTES)),
  dataUrl: TrimmedNonEmptyString.pipe(
...
export const OrchestrationMessage = Schema.Struct({
  ...
  source: Schema.optional(OrchestrationMessageSource).pipe(Schema.withDecodingDefault(() => "native")),
  createdAt: IsoDateTime,
  updatedAt: IsoDateTime,
});
...
export const OrchestrationProposedPlan = Schema.Struct({
  ...
  implementedAt: Schema.optional(Schema.NullOr(IsoDateTime)).pipe(Schema.withDecodingDefault(() => null)),
  implementationThreadId: Schema.optional(Schema.NullOr(ThreadId)).pipe(Schema.withDecodingDefault(() => null)),
  createdAt: IsoDateTime,
  updatedAt: IsoDateTime,
});
...
export const OrchestrationSession = Schema.Struct({
  ...
  runtimeMode: Schema.optional(RuntimeMode).pipe(Schema.withDecodingDefault(() => DEFAULT_RUNTIME_MODE)),
 ...
 * 文件级变更归属
 *
 * - "ai"    - 整文件由 AI 生成(turn 触发来源是 AI 模型)
 * - "user"  - 整文件由用户在 review 流程后手工改写
 * - "mixed" - 同文件内 AI / 用户改动混存
 */
export const OrchestrationCheckpointFileAuthor = Schema.Literal("ai", "user", "mixed");
export type OrchestrationCheckpointFileAuthor = typeof OrchestrationCheckpointFileAuthor.Type;
...
export const OrchestrationCheckpointFile = Schema.Struct({
  ...
  /**
   * 文件级 author;缺省时按 "ai" 兜底(legacy checkpoint / 非 review 模式)
   */
  author: Schema.optional(OrchestrationCheckpointFileAuthor),
});
...
export const OrchestrationThread = Schema.Struct({
  ...
  deletedAt: Schema.NullOr(IsoDateTime),
  handoff: Schema.optional(Schema.NullOr(ThreadHandoff)).pipe(Schema.withDecodingDefault(() => null)),
  messages: Schema.Array(OrchestrationMessage),
  proposedPlans: Schema.optional(Schema.Array(OrchestrationProposedPlan)).pipe(
...
const FanOutVariant = Schema.Struct({
  label: Schema.String,
  threadId: ThreadId,
  modelSelection: ModelSelection,
  runtimeMode: RuntimeMode,
...
const ThreadFanOutCommand = Schema.Struct({
  type: Schema.Literal("thread.fanout"),
  commandId: CommandId,
  sourceThreadId: ThreadId,
  projectId: ProjectId,
  prompt: Schema.String,
  variants: Schema.Array(FanOutVariant),
  createdAt: IsoDateTime,
});
...
const ThreadMessageEditAndResendCommand = Schema.Struct({
  ...
  threadId: ThreadId,
  messageId: MessageId,
  text: TrimmedNonEmptyString.pipe(Schema.maxLength(PROVIDER_SEND_TURN_MAX_INPUT_CHARS)),
  modelSelection: Schema.optional(ModelSelection),
  providerOptions: Schema.optional(ProviderStartOptions),
  assistantDeliveryMode: Schema.optional(AssistantDeliveryMode),
  runtimeMode: RuntimeMode,
...
export const ThreadTurnStartRequestedPayload = Schema.Struct({
  ...
  reviewTarget: Schema.optional(ProviderReviewTarget),
  assistantDeliveryMode: Schema.optional(AssistantDeliveryMode),
  dispatchMode: Schema.optional(TurnDispatchMode).pipe(Schema.withDecodingDefault(() => DEFAULT_TURN_DISPATCH_MODE)),
  runtimeMode: Schema.optional(RuntimeMode).pipe(Schema.withDecodingDefault(() => DEFAULT_RUNTIME_MODE)),
  interactionMode: Schema.optional(ProviderInteractionMode).pipe(
    Schema.withDecodingDefault(() => DEFAULT_PROVIDER_INTERACTION_MODE),
  ),
...
// === New event payloads for task tree / crash recovery / status line ===

export const ThreadTurnStartedPayload = Schema.Struct({
  threadId: ThreadId,
  turnId: TurnId,
...
export const ThreadTurnFailedPayload = Schema.Struct({
  threadId: ThreadId,
  turnId: TurnId,
...
export const ThreadToolCallStartedPayload = Schema.Struct({
  threadId: ThreadId,
  turnId: TurnId,
...
export const ThreadToolCallCompletedPayload = Schema.Struct({
  threadId: ThreadId,
  turnId: TurnId,
  toolCallId: TrimmedNonEmptyString,
...
export const ThreadFileReadPayload = Schema.Struct({
  threadId: ThreadId,
  turnId: TurnId,
  path: TrimmedNonEmptyString,
...
export const ThreadFileWrittenPayload = Schema.Struct({
  threadId: ThreadId,
  turnId: TurnId,
  path: TrimmedNonEmptyString,
  bytes: Schema.optional(NonNegativeInt),
...
export const ThreadDiffGeneratedPayload = Schema.Struct({
  threadId: ThreadId,
  turnId: TurnId,
  diffId: TrimmedNonEmptyString,
  generatedAt: IsoDateTime,
});

export const ThreadCommitCreatedPayload = Schema.Struct({
  threadId: ThreadId,
  turnId: TurnId,
...
export const OrchestrationEvent = Schema.Union(
  ...
  Schema.Struct({
    ...
    payload: ThreadTurnStartedPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
  ...
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.turn-failed"),
    payload: ThreadTurnFailedPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.tool-call-started"),
    payload: ThreadToolCallStartedPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.tool-call-completed"),
    payload: ThreadToolCallCompletedPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.file-read"),
    payload: ThreadFileReadPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.file-written"),
    payload: ThreadFileWrittenPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.diff-generated"),
    payload: ThreadDiffGeneratedPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.commit-created"),
    payload: ThreadCommitCreatedPayload,
  }),
);
...
const TurnCountRangeBase = Schema.Struct({
  fromTurnCount: NonNegativeInt,
  toTurnCount: NonNegativeInt,
});

export const TurnCountRange = TurnCountRangeBase.pipe(
  Schema.filter(
    (input) => input.fromTurnCount <= input.toTurnCount,
    { identifier: "OrchestrationTurnDiffRange" },
  ),
);

export const ThreadTurnDiff = Schema.Struct({
  ...TurnCountRangeBase.fields,
  threadId: ThreadId,
  diff: Schema.String,
});
...
/**
 * 全 workspace Turn AI 占比快照
 *
 * 与单线程 `useTurnAiShare(thread)` 不同,这是按 24h / 7d / 30d 三个
 * 窗口聚合整个 workspace 所有 thread 的 turn diff,作为"workspace
 * 整体 AI 生产占比"在桌面端 Sidebar / 移动端 ProviderUsage 展示。
 */
export const AiShareWindow = Schema.Struct({
  /** 时间窗口标签(24h / 7d / 30d) */
  window: Schema.String,
  /** AI 归属的净增行数 */
  aiLines: NonNegativeInt,
  /** 用户归属的净增行数 */
  humanLines: NonNegativeInt,
  /** 混合归属的净增行数 */
  mixedLines: NonNegativeInt,
  /** 全部归属行数 */
  totalLines: NonNegativeInt,
  /** AI 占比 0~1;无数据 = null */
  aiShare: Schema.NullOr(Schema.Number),
  /** 涉及的 turn 数 */
  turnCount: NonNegativeInt,
  /** 涉及的文件数(去重) */
...
export type AiShareWindow = typeof AiShareWindow.Type;

export const OrchestrationGetTurnAiShareSnapshotResult = Schema.Struct({
  windows: Schema.Array(AiShareWindow),
  /** 快照生成时间(UNIX ms) */
  generatedAtMs: NonNegativeInt,
  /** 数据是否为空 */
  isEmpty: Schema.Boolean,
});
export type OrchestrationGetTurnAiShareSnapshotResult =
  typeof OrchestrationGetTurnAiShareSnapshotResult.Type;
...
export const OrchestrationRpcSchemas = {
  ...
  getTurnAiShareSnapshot: {
    input: Schema.Void,
    output: OrchestrationGetTurnAiShareSnapshotResult,
  },
  replayEvents: {
