...
export const COMPOSER_DRAFT_STORAGE_KEY = "ydsz-buddy:composer-drafts:v1";
const COMPOSER_DRAFT_STORAGE_VERSION = 4;
const DraftThreadEnvModeSchema = Schema.Literal("local", "worktree");
export type DraftThreadEnvMode = typeof DraftThreadEnvModeSchema.Type;
const DraftThreadEntryPointSchema = Schema.Literal("chat", "terminal");
...
const PersistedQueuedComposerChatTurn = Schema.Struct({
  ...
  assistantSelections: Schema.optional(
    Schema.Array(
      Schema.Struct({
        id: Schema.String,
  ...
  modelSelection: ModelSelection,
  providerOptionsForDispatch: Schema.optional(ProviderStartOptions),
  runtimeMode: RuntimeMode,
  interactionMode: ProviderInteractionMode,
...
const PersistedComposerThreadDraftState = Schema.Struct({
  ...
  terminalContexts: Schema.optional(Schema.Array(PersistedTerminalContextDraft)),
  queuedTurns: Schema.optional(Schema.Array(PersistedQueuedComposerTurn)),
  modelSelectionByProvider: Schema.optional(
    Schema.Record({ key: ProviderKind, value: ModelSelection }),
  ),
  activeProvider: Schema.optional(Schema.NullOr(ProviderKind)),
...
const LegacyCodexFields = Schema.Struct({
  ...
  codexFastMode: Schema.optional(Schema.Boolean),
...
const LegacyStickyModelFields = Schema.Struct({
  stickyProvider: Schema.optional(ProviderKind),
  stickyModel: Schema.optional(Schema.String),
  stickyModelOptions: Schema.optional(Schema.NullOr(ProviderModelOptions)),
});
...
const PersistedDraftThreadState = Schema.Struct({
  ...
  runtimeMode: RuntimeMode,
  interactionMode: ProviderInteractionMode,
  entryPoint: Schema.optional(DraftThreadEntryPointSchema).pipe(Schema.withDecodingDefault(() => "chat")),
  branch: Schema.NullOr(Schema.String),
  worktreePath: Schema.NullOr(Schema.String),
  lastKnownPr: Schema.optional(Schema.NullOr(OrchestrationThreadPullRequest)),
  envMode: DraftThreadEnvModeSchema,
  isTemporary: Schema.optional(Schema.Boolean),
  promotedTo: Schema.optional(ThreadId),
});
type PersistedDraftThreadState = typeof PersistedDraftThreadState.Type;

const PersistedComposerDraftStoreState = Schema.Struct({
  draftsByThreadId: Schema.Record({ key: Schema.String, value: PersistedComposerThreadDraftState }),
  draftThreadsByThreadId: Schema.Record({ key: Schema.String, value: PersistedDraftThreadState }),
  projectDraftThreadIdByProjectId: Schema.Record({ key: Schema.String, value: Schema.String }),
  stickyModelSelectionByProvider: Schema.optional(
    Schema.Record({ key: ProviderKind, value: ModelSelection }),
  ),
  stickyActiveProvider: Schema.optional(Schema.NullOr(ProviderKind)),
});
...
const EMPTY_PERSISTED_DRAFT_STORE_STATE = Object.freeze<PersistedComposerDraftStoreState>({
  ...
  stickyModelSelectionByProvider: {} as Record<ProviderKind, ModelSelection>,
...
function makeModelSelection(
  ...
  switch (provider) {
    ...
    // 国内 6 家 OpenAI 兼容 Provider：暂不携带 options（HTTP 透传）
    case "glm":
    case "deepseek":
    case "moonshot":
    case "qwen":
    case "mimo":
    case "MiniMax":
    // 新增 3 家国内 Provider:同上,HTTP 透传不携带 options
    case "doubao":
    case "ernie":
    case "hunyuan":
...
function migratePersistedComposerDraftStoreState(
  ...
  return {
    ...
    draftThreadsByThreadId,
    projectDraftThreadIdByProjectId,
    stickyModelSelectionByProvider: stickyModelSelectionByProvider as Record<ProviderKind, ModelSelection>,
...
function partializeComposerDraftStoreState(
  ...
  for (const [threadId, draft] of Object.entries(state.draftsByThreadId)) {
    ...
    persistedDraftsByThreadId[threadId as ThreadId] = persistedDraft as DeepMutable<PersistedComposerThreadDraftState>;
  }
  return {
    ...
    stickyModelSelectionByProvider: state.stickyModelSelectionByProvider as Record<ProviderKind, ModelSelection>,
