 ...
 *
 * @description
 * TODO: 迁移期间临时使用 JSON 序列化；后续可接入 zod/effect 进行校验。
 */
 ...
 * 创建基于 JSON 的编解码器
 *
 * @returns 本地存储编解码器实例，使用 JSON 序列化/反序列化
 */
 ...
 *
 * @param key - 存储键名
 * @param _codec - 编解码器（当前仅作类型占位，实际使用 JSON 解析）
 ...
 * @param key - 存储键名
 * @param value - 要存储的值
 * @param _codec - 编解码器（当前仅作类型占位，实际使用 JSON 序列化）
 */
...
/** 本地存储变化事件的自定义事件类型名*/
const LOCAL_STORAGE_CHANGE_EVENT = "ydsz-buddy:local_storage_change";
 ...
 * @param _codec - 编解码器（当前仅作类型占位，实际使用 JSON 序列化）
 *
 * @returns 包含当前值和设置函数的元组 [value, setValue]
 * @returns.0 - 当前存储的值
 * @returns.1 - 设置值的函数，支持直接值或更新函数
 */
