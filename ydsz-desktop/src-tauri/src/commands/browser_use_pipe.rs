...
//! 允许外部 Codex CLI 通过命名管道与 ydsz-buddy 内嵌浏览器通信。
//!
...
async fn handle_connection(
    ...
    state: Arc<Mutex<ConnectionState>>,
    _cdp_listeners: Arc<Mutex<HashMap<String, Box<dyn Fn(serde_json::Value) + Send + Sync>>>>,
    ...
    match method {
        ...
        "getInfo" => {
            ...
            let mut info = serde_json::json!({
                "name": "ydsz-buddy In-app Browser",
                "version": "0.3.0",
                "type": "iab",
            });
        ...
        "createTab" => {
            ...
            let _tab_id = format!("tab-{}", state.next_tab_id);
