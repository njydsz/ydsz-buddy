...
//! - 定义语音识别的输入/输出数据结构
//! - 提供语音转文字的命令接口
//! - 转发到嵌入式 ydsz-server 的 `voice.transcribe` RPC 方法
//!
//! ## 核心功能
//!
...
//! ## 设计说明
//!
//! - 通过 `ydsz_server::ipc_voice_transcribe` 桥接调用嵌入式服务器的 ASR 引擎
//! - 实际 ASR 引擎可在服务端通过ydsz-server 端 配置（mock / openai_whisper / azure / local）
...
/// 语音转文字命令
///
/// 将 Base64 编码的音频数据转发到嵌入式 ydsz-server 的 `voice.transcribe` RPC。
...
pub async fn transcribe_voice(input: TranscribeVoiceInput) -> Result<TranscribeVoiceResult, String> {
    ...
    match ydsz_server::ipc_voice_transcribe(request).await {
        Ok(resp) => {
            let text = resp
                .get("text")
                .and_then(|v| v.as_str())
