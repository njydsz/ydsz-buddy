...
fn spawn_pty_reader(
    ...
    tokio::task::spawn_blocking(move || {
        // 输出节流策略：在 BUFFER_CAP 字节积累到阈值或 FLUSH_INTERVAL_MS 毫秒内有新数据时 flush。
        // 避免高频小 chunk (如 `yarn install` / `find /`) 造成 IPC 风暴卡死 WebView2 主线程。
        const BUFFER_CAP: usize = 16 * 1024;       // 16KB 软上限
        const FLUSH_INTERVAL_MS: u64 = 100;        // 100ms 兜底刷新间隔
        let mut buf = [0u8; 4096];
        let mut accumulated = String::with_capacity(BUFFER_CAP + 4096);
        let flush_threshold = BUFFER_CAP;
        let mut last_flush = std::time::Instant::now();
        loop {
            match reader.read(&mut buf) {
                Ok(0) => {
                    // PTY 关闭：flush 残留缓冲后退出
                    if !accumulated.is_empty() {
                        let payload = TerminalOutputPayload {
                            session_id: session_id.clone(),
                            output: std::mem::take(&mut accumulated),
                        };
                        if let Err(e) = app.emit("terminal-output", &payload) {
                            warn!("终端 {} 推送最终输出事件失败: {}", session_id, e);
                        }
                    }
                ...
                Ok(n) => {
                    ...
                    let should_flush_by_size = accumulated.len() >= flush_threshold;
                    let should_flush_by_time = last_flush.elapsed() >= std::time::Duration::from_millis(FLUSH_INTERVAL_MS);
