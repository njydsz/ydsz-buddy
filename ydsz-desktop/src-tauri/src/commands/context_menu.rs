...
///
/// 2026-06-26 修复：tauri-specta 的 `collect_commands!` 宏无法在泛型
/// 函数上推断出 `R: Runtime`，因此固定使用 `Wry` runtime（与
/// `Builder::<tauri::Wry>::new()` 一致）。原泛型实现保留为
/// `show_context_menu_impl`，避免行为变更。
#[tauri::command]
#[specta::specta]
pub async fn show_context_menu(
    ...
    show_context_menu_impl(app, items, position).await
}

/// 泛型实现，供 tauri::command 包装与未来多 runtime 场景共用。
pub async fn show_context_menu_impl<R: Runtime>(
    app: AppHandle<R>,
    items: Vec<ContextMenuItem>,
    position: Option<Position>,
) -> Result<Option<String>, String> {
