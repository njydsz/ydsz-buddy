...
Goal mutation accepted by one live agent. The matching `goal/change` session event has already committed. Listener failures are contained. Scope-filtered dispatch (`@njydsz/ydb-scope`): agent-scoped listeners receive only that agent.
 ...
 * @mode emit
 */
'goal/changed'(this: import('@njydsz/ydb-scope').Scoped<Agent>, payload: { agent: Agent; change: GoalChanged }): void
```
