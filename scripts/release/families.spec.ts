...
describe('release families', () => {
  it('names one tag for the whole dsh family and one per vendored package', () => {
    ...
    const cli = member('apps/cli', '@njydsz/ydb')
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const dsh = releaseFamily('dsh')
    const members = [member('apps/cli', '@njydsz/ydb'), { ...member('apps/web', '@njydsz/ydb-web-frontend'), version: '0.0.2' }]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@njydsz/ydb-consumer', { dependencies: { '@njydsz/ydb-library': 'workspace:^' } }),
      member('packages/a/library', '@njydsz/ydb-library'),
      member('packages/a/zebra', '@njydsz/ydb-zebra'),
    ]
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@njydsz/ydb-left', { dependencies: { '@njydsz/ydb-right': 'workspace:^' } }),
      member('packages/a/right', '@njydsz/ydb-right', { dependencies: { '@njydsz/ydb-left': 'workspace:^' } }),
    ]
  ...
  it('applies the harness payload policy to dsh and keeps upstream payloads for vendored packages', () => {
    const dsh = releaseFamily('dsh')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@njydsz/ydb-library')
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@njydsz/ydb', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@njydsz/ydb-library', { files: ['lib/index.js'] }),
