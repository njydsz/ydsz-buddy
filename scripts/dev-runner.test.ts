...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit REMI_CODE_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "REMI_CODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid REMI_CODE_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults REMI_CODE_HOME to ~/.remi-code when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.REMI_CODE_HOME, resolve(homedir(), ".remi-code"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          remiCodeHome: "/tmp/custom-remi-code",
          authToken: "secret",
          noBrowser: true,
        ...
        assert.equal(env.REMI_CODE_HOME, resolve("/tmp/custom-remi-code"));
        assert.equal(env.REMI_CODE_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://[::1]:4222");
        assert.equal(env.REMI_CODE_NO_BROWSER, "1");
        assert.equal(env.REMI_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.REMI_CODE_LOG_WS_EVENTS, "1");
        assert.equal(env.REMI_CODE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            REMI_CODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          remiCodeHome: undefined,
        ...
        assert.equal(env.REMI_CODE_MODE, "web");
        assert.equal(env.REMI_CODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.REMI_CODE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom remiCodeHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.REMI_CODE_HOME, resolve("/tmp/my-remi-code"));
      }),
    );
  });
