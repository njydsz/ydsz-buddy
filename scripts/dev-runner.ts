...
import { NetService } from "@remi-code/shared/Net";
...
const MODE_ARGS = {
  dev: [
    ...
    "dev",
    "--ui=tui",
    "--filter=@remi-code/contracts",
    "--filter=@remi-code/web",
    "--filter=remi-code",
    "--parallel",
  ],
  "dev:server": ["run", "dev", "--filter=remi-code"],
  "dev:web": ["run", "dev", "--filter=@remi-code/web"],
  "dev:desktop": [
    "run",
    "dev",
    "--filter=@remi-code/desktop",
    "--filter=@remi-code/web",
    "--parallel",
  ],
  "dev:win": [
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("REMI_CODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("REMI_CODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid REMI_CODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `REMI_CODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric REMI_CODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed REMI_CODE_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...baseEnv,
      REMI_CODE_PORT: String(serverPort),
      PORT: String(webPort),
      ELECTRON_RENDERER_PORT: String(webPort),
      VITE_WS_URL: `ws://[::1]:${serverPort}`,
      VITE_DEV_SERVER_URL: devUrl?.toString() ?? `http://localhost:${webPort}`,
      REMI_CODE_HOME: resolvedBaseDir,
    };

    if (host !== undefined) {
      output.REMI_CODE_HOST = host;
    }

    if (authToken !== undefined) {
      output.REMI_CODE_AUTH_TOKEN = authToken;
    } else {
      delete output.REMI_CODE_AUTH_TOKEN;
    }

    if (noBrowser !== undefined) {
      output.REMI_CODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else {
      delete output.REMI_CODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.REMI_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.REMI_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.REMI_CODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.REMI_CODE_LOG_WS_EVENTS;
    }

    if (mode === "dev" || mode === "dev:win") {
      output.REMI_CODE_MODE = "web";
      delete output.REMI_CODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read REMI_CODE_PORT_OFFSET/REMI_CODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("REMI_CODE_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv(
        "REMI_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
      ),
      logWebSocketEvents: readOptionalBooleanEnv("REMI_CODE_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.REMI_CODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.REMI_CODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to REMI_CODE_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("REMI_CODE_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to REMI_CODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("REMI_CODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to REMI_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("REMI_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to REMI_CODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("REMI_CODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to REMI_CODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("REMI_CODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to REMI_CODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("REMI_CODE_PORT")),
  ),
