...
import type { ToolSchema } from '@njydsz/ydb-llm'
import AgentRegistry from '@njydsz/ydb-agent'
import type { Agent } from '@njydsz/ydb-agent'
import { createScope } from '@njydsz/ydb-scope'
...
import GoalService from '@njydsz/ydb-goal'
import SystemPrompt from '@njydsz/ydb-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@njydsz/ydb-tools'
import LocalBashExecutor from '@njydsz/ydb-bash-local'
import * as BashEnvPlugin from '@njydsz/ydb-shell-env'
import { PwshLocalExecutor } from '@njydsz/ydb-pwsh-local'
...
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@njydsz/ydb-attachment'
import UserQuestionService from '@njydsz/ydb-user-questions'
import PlanModeController from '@njydsz/ydb-plan-mode'
import WebRuntime from '@njydsz/ydb-web'
import * as WebSearchExa from '@njydsz/ydb-web-search-exa'
import * as WebFetchLocal from '@njydsz/ydb-web-fetch-http'
import SubagentRuntime from '@njydsz/ydb-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@njydsz/ydb-subagent'
import * as ToolSubagentControl from '@njydsz/ydb-tool-subagent-control'
import * as ToolSubagentListAgents from '@njydsz/ydb-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@njydsz/ydb-tool-subagent-report'
import SkillRegistry from '@njydsz/ydb-skill'
import * as SkillFileSystem from '@njydsz/ydb-skill-filesystem'
import LocalJobRegistry from '@njydsz/ydb-jobs-local'
import * as ToolAskUser from '@njydsz/ydb-tool-ask-user'
import * as ToolBash from '@njydsz/ydb-tool-bash'
import * as ToolPwsh from '@njydsz/ydb-tool-pwsh'
import * as ToolBashPersistent from '@njydsz/ydb-tool-bash-persistent'
import CordisHostRunner from '@njydsz/ydb-cordis-host-runner'
import * as ToolCordis from '@njydsz/ydb-tool-cordis'
import * as ToolFs from '@njydsz/ydb-tool-fs'
import * as ToolFsSearch from '@njydsz/ydb-tool-fs-search'
import * as ToolStrReplaceEditor from '@njydsz/ydb-tool-str-replace-editor'
import TerminalSessionService from '@njydsz/ydb-terminal'
import * as ToolPty from '@njydsz/ydb-tool-terminal'
import * as ToolGoal from '@njydsz/ydb-tool-goal'
import * as ToolSchedule from '@njydsz/ydb-schedule'
import Lsp from '@njydsz/ydb-lsp'
import * as ToolLsp from '@njydsz/ydb-tool-lsp'
import * as ToolSkill from '@njydsz/ydb-tool-skill'
import * as ToolSessionQuery from '@njydsz/ydb-tool-session-query'
import * as ToolTasks from '@njydsz/ydb-tool-jobs'
import * as ToolTodo from '@njydsz/ydb-tool-todo'
import * as ToolSubagent from '@njydsz/ydb-tool-subagent'
import * as ToolWeb from '@njydsz/ydb-tool-web'
import VmWorkflowEngine from '@njydsz/ydb-workflow-worker-thread'
import * as ToolRalph from '@njydsz/ydb-tool-ralph'
import * as ToolWorkflow from '@njydsz/ydb-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@njydsz/ydb-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@njydsz/ydb-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@njydsz/ydb-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `DSH_*` environment comes from `@njydsz/ydb-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@njydsz/ydb-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@njydsz/ydb-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@njydsz/ydb-tool-bash-persistent',
  ...
  {
    pkg: '@njydsz/ydb-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@njydsz/ydb-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. `read_image` is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@njydsz/ydb-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@njydsz/ydb-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@njydsz/ydb-tool-ralph',
    dir: 'tool-ralph',
  ...
  {
    pkg: '@njydsz/ydb-tool-session-query',
  ...
  {
    pkg: '@njydsz/ydb-tool-subagent-control',
  ...
  {
    pkg: '@njydsz/ydb-tool-subagent-report',
