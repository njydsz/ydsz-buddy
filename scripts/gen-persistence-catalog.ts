...
const SESSION_PACKAGE = '@njydsz/ydb-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@njydsz/ydb-session/types'
 ...
 * top-level declaration (in `@njydsz/ydb-session`) and any declaration
 * merge inside a `declare module '@njydsz/ydb-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @njydsz/ydb-session/known-event-types',
    ' */',
    '',
    '/**',
