...
function checkManifest(
  ...
  if (owner.packageName === '@njydsz/ydb-invariants') return
  if (manifest.peerDependencies?.['@njydsz/ydb-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@njydsz/ydb-invariants must be a workspace:^ peerDependency',
    )
  }
  if (manifest.devDependencies?.['@njydsz/ydb-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@njydsz/ydb-invariants must also be a workspace:^ devDependency',
    )
  }
}
...
function checkBuild(
  ...
  if (owner.packageName !== '@njydsz/ydb-invariants'
