...
impl SqliteEventStore {
    ...
    /// Event type names follow the "aggregate.action" naming convention, e.g. "project.created"    fn extract_aggregate_info(event: &OrchestrationEvent) -> (String, String, String) {
        match event {
...
impl EventStore for SqliteEventStore {
    ...
    /// 5. Get and return the last inserted sequence number    fn append_event(&self, event: &OrchestrationEvent) -> PersistenceResult<Sequence> {
        ...
        // Serialize the event to JSON string        let payload_json = serde_json::to_string(event)?;
        // Get event occurrence time and convert to RFC3339 format
        let occurred_at = event.occurred_at().to_rfc3339();
        // Get the command ID that triggered this event (if any)        let command_id = event.command_id();
        ...
        // Get the auto-generated sequence number from the database        let sequence = self.client.last_insert_rowid()? as Sequence;
        Ok(sequence)
    }
    ...
    /// 4. Map each row to a `StoredEvent` struct    fn read_events(&self, from_sequence: Sequence, limit: usize) -> PersistenceResult<Vec<StoredEvent>> {
        let rows = self.client.query_map(
            ...
            |row| {
                // Map database row to StoredEvent struct                Ok(StoredEvent {
                    sequence: row.get(0)?,
                    event_id: row.get(1)?,
    ...
    /// The `COALESCE` function ensures that 0 is returned instead of NULL even when there is no data.    fn get_latest_sequence(&self) -> PersistenceResult<Sequence> {
...
mod tests {
    ...
    fn test_event_store_pagination_from_sequence() {
        ...
        // Read 3 events after sequence number 5        let page = store.read_events(5, 3).unwrap();
