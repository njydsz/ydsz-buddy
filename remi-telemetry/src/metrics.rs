...
impl MetricsCollector {
    ...
    /// ## 设计说明
    ///
    /// 将底层写入逻辑集中在单一方法中，而非在各便捷方法中重复写锁获取与 push 操作，
    /// 是为了确保所有指标类型的写入路径一致，便于未来添加统一的预处理逻辑
    /// （如指标采样、过滤、批量写入等），只需修改此方法即可。
    ///
    /// ## 参数
    ///
    ...
    ///
    /// 清空内部指标记录列表中的所有已记录数据。
    /// 注意：此操作**不会**重置收集器的启动时间（`start_time` 字段），
    /// 因此 [`MetricsCollector::uptime`] 仍从创建时刻开始计算。
    ///
    /// 这是因为 `start_time` 代表的是收集器的生命周期起点，而非数据的时间范围。
    /// 清除数据不应影响对服务运行时长的度量，否则会导致 `uptime()` 返回值
    /// 与实际服务运行时间不一致，影响基于 uptime 计算的指标采集速率（如 QPS）。
    ///
    /// ## 返回值
    ///
    /// - `Ok(())`：清除成功。
