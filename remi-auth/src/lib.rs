//! Remi Code 认证与授权模块。
//!
//! 本模块负责处理用户认证、会话管理、密钥存储、
//! 设置管理以及生命周期事件追踪。
//!
//! 持久化表（由 remi-persistence 迁移定义）：
//! - `auth_sessions` – 已签发的客户端会话
//! - `auth_pairing_links` – 一次性配对凭证
//! - `secrets` – 加密密钥存储
//! - `lifecycle_events` – 系统生命周期审计日志
//! - `settings` – 通用键值对设置
