 ...
 *
 * @description
 * 提供服务器设置（ServerSettings）的补丁合并功能，核心是 `applyServerSettingsPatch` 函数。
 * 该模块处理模型选择（ModelSelection）的特殊合并逻辑，确保在切换 AI 提供商时
 * 自动回退到该提供商的默认模型，而非保留旧提供商的模型名称。
 *
 * 核心功能：
 * - 深度合并服务器设置补丁（基于 `deepMerge`）
 * - 模型选择的智能合并：切换提供商时自动使用新提供商的默认模型
 * - 模型选项（options）的条件替换逻辑
 *
 * 合并策略：
 * - 普通字段使用深度合并（`deepMerge`）
 * - 模型选择字段使用特殊合并逻辑：
 *   - 若补丁指定了新的 provider 或 model，则视为"替换"操作
 *   - 切换 provider 但未指定 model 时，自动使用新 provider 的默认模型
 *   - options 在替换模式下仅使用补丁中的值，非替换模式下深度合并
 *
 ...
 *
 * // 仅修改模型，保留提供商
 * const patched2 = applyServerSettingsPatch(current, {
 ...
 * 判断模型选择补丁是否应触发"替换"操作
 *
 * 当补丁中明确指定了 `provider` 或 `model` 字段时，认为这是一次替换操作，
 * 需要重置模型选项（options）；否则视为部分更新，保留现有的 options。
 *
 * @param patch - 模型选择补丁对象
 * @returns 如果补丁包含 provider 或 model 字段则返回 true，表示应执行替换操作
 *
...
/**
 * 将补丁应用到服务器设置，返回合并后的新设置对象
 *
 * 该函数是服务器设置更新的核心入口，处理以下合并逻辑：
 *
 * 1. **普通字段**：使用 `deepMerge` 进行深度合并，patch 中的非 undefined 值覆盖 current 中的值
 * 2. **模型选择字段**（`textGenerationModelSelection`）使用特殊合并策略：
 *    - 若补丁指定了新的 `provider` 但未指定 `model`：
 *      - 如果新 provider 与当前不同且不是 "pi"，自动使用新 provider 的默认模型
 *      - 否则保留当前 model
 *    - 若补丁指定了 `model`：直接使用补丁中的 model
 *    - `options` 字段：替换模式下仅使用补丁中的 options；非替换模式下深度合并
 *
 * @param current - 当前的服务器设置对象
 * @param patch - 要应用的设置补丁（部分更新）
 * @returns 合并后的新服务器设置对象，类型与 `ServerSettings` 一致
 *
 ...
 * //   model: 'claude-sonnet-4-20250514'  // 自动使用 claudeAgent 的默认模型
 * // }
 * ```
 *
 * @example 仅更新模型选项
...
export function applyServerSettingsPatch(
  ...
  // 确定最终的 provider：优先使用补丁中的值，否则保留当前值
  const provider = selectionPatch.provider ?? current.textGenerationModelSelection.provider;
  // 确定最终的 model：
  // - 优先使用补丁中明确指定的 model
  // - 若补丁切换了 provider 且新 provider 不是 "pi"，则使用新 provider 的默认模型
  // - 否则保留当前 model
  const model =
  ...
  // 确定 options：替换模式下仅使用补丁中的 options，非替换模式下深度合并
