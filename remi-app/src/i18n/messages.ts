...
/**
 * 翻译消息类型定义： * 定义了所有 UI 字符串的结构，按键分组组织。 * 每个分组对应应用的一个功能模块或界面区域。 * 部分字段为函数类型，用于支持动态插值（如传入用户名、数量等）。 */
...
const zh: Messages = {
  ...
  skills: {
    ...
    localEmptyDescription:
      "Remi Claw 已扫描 ~/.claude/skills、~/.codex/skills 和 ~/.agents/skills。把一个含 SKILL.md 的技能目录放进任一目录后刷新即可",
  ...
  settings: {
    ...
    providers: {
      ...
      updates: {
        ...
        versionAdvisoryNoCommand:
          "检测到新版本，但 Remi Claw 未能为该安装识别出安全的一键更新命令",
      },
      tools: {
        title: "已安装的 CLI",
