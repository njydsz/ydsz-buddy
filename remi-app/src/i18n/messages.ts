/**
 * @file messages.ts
 * @description 集中管理所有面向用户的 UI 字符串，??React 组件树可以根据语言动态切换翻译内容，
 *              而无需修改调用方代码。消息按键（surface）分组，便于未来迁移到翻译工具时将其
 *              重新导出为扁平化的命名空??ID。模板使??${...} 语法进行插值?? * @module messages
 */

import type { Language } from "./language";

/**
 * 翻译消息类型定义?? * 定义了应用中所??UI 字符串的结构，按键分组组织?? * 每个分组对应应用的一个功能模块或界面区域?? * 部分字段为函数类型，用于支持动态插值（如传入用户名、数量等）?? */
...
const zh: Messages = {
  ...
  sidebar: {
    ...
    chooseProjectFolder: "选择项目文件??",
    openingFolderPicker: "正在打开...",
    addingProject: "正在添加...",
    loadingProjects: "正在加载项目",
    toggleSidebar: "切换线程侧边??",
    codeLabel: "代码",
    disposableChat: "一次性聊??",
    pendingApproval: "待审??",
    commandsHeading: "命令",
    ...
    thread: {
      ...
      archiveSuccessOne: "线程已归??",
      archiveSuccessMany: (count) => `已归??${count} 个线程`,
      archiveError: "归档线程失败",
      deleteEmpty: "没有可删除的内容",
      deleteWorktreeWarning: "线程已删除，但工作树清理失败",
      deleteSuccessOne: "线程已删??",
      deleteSuccessMany: (count) => `已删??${count} 个线程`,
      deleteError: "删除线程失败",
      pathUnavailable: "路径不可??",
      pathCopyUnavailable: "此线程没有可复制的工作区路径??",
      pathOpenUnavailable: "此线程没有可打开的工作区路径??",
      copyThreadId: "线程 ID 已复??",
  ...
  chat: {
    ...
    handoffError: "无法创建交接线程",
    refreshProviderStatus: "无法刷新提供方状??",
    deletedAction: (name) => `已删除操作??{name ?? "未知"}」`,
    deleteActionFailed: "无法删除操作",
    updateAccessModeFailed: "无法更新访问模式",
    tooManyAttachments: (max) => `每条消息最多附??${max} 个引用。`,
    browserAttachFailed: "无法附加应用内浏览器上下??",
    imagePreview: "展开的图片预??",
    ...
    voice: {
      authRequiredTitle: "请先??Codex 中登??ChatGPT 再使用语音??",
      authRequiredDescription: "语音功能需要已登录 ChatGPT ??Codex 会话??",
      authSessionTitle: "语音功能需要已登录 ChatGPT ??Codex 会话??",
      authSessionDescription: "请重新登??ChatGPT 后再录制语音??",
      planUnansweredTitle: "请先回答计划问题再录制语音??",
      planUnansweredDescription: "必须在回答计划问题后才能录制??",
      startFailedTitle: "无法开始录??",
      ...
      transcribeFailedTitle: "请重新登??ChatGPT",
      transcribeFailedDescription: "无法转写语音备注",
    },
    continueInNewWorktree: "在新工作树中继续",
    reviewLocalChanges: "查看本地未提交的变更",
    reviewBranchDiff: "查看当前分支相对基线??diff",
    composerPlaceholder: (providerName) => `??${providerName} 发送消息…`,
    stopGenerationAria: "停止生成",
    stopGenerationTitle: "停止当前回复。在 Mac 上，??Ctrl+C 中断??",
    ...
    timeline: {
      ...
      undo: "撤销",
      workingFor: (duration) => `已工??${duration}`,
      workingForPrefix: "已工??",
      working: "处理中??",
  ...
  chatRoute: {
    ...
    splitPaneEmptyTitle: "选择一个聊??",
    splitPaneEmptyProject: "项目",
  },
  composer: {
    placeholder: "输入任何内容，使??@ 引用文件/文件夹，或使??/ 查看可用命令",
    ...
    placeholderDisconnected: "请求后续修改或附加图??",
    moreAria: "更多输入控制",
    extrasAria: "输入扩展",
    modeLabel: "模式",
    buildLabel: "构建",
    ...
    contextWindowLabel: "上下文窗??",
    contextWindowPercent: (percent) => `已使??${percent}%`,
    sendMessage: "发送消??",
    sendingBusy: "发送中",
    sendingConnecting: "连接??",
    sendingTranscribing: "正在转写语音",
    sendingPreparingWorktree: "正在准备工作??",
    steer: "引导",
    deleteQueuedFollowUp: "删除排队的追??",
  ...
  skills: {
    ...
    emptySearchTitle: "没有匹配此搜索的技??",
    emptySearchDescription: "尝试其他关键词，或清空搜索查看全部技能??",
    unavailableTitle: "{provider} 暂不支持技??",
  ...
  settings: {
    ...
    nav: {
      ...
      providers: {
        label: "提供??",
        description: "选择可见的提供方、查??CLI 安装状态并更新提供方工具??",
      },
      advanced: {
        label: "高级",
    ...
    appearance: {
      heading: "外观",
      description: "主题、字体与时间格式??",
      themeAndTypographySection: "主题与排??",
    ...
    notifications: {
      ...
      testSuccessTitle: "测试通知已发??",
      testUnavailableTitle: "通知不可??",
    ...
    behavior: {
      ...
      terminalCloseConfirmationAria: "确认关闭终端标签??",
    },
    worktrees: {
      heading: "工作??",
      description: "查看并清理由 Remi Claw 创建的工作树??",
    ...
    models: {
      ...
      customModelResetLabel: "自定义模??",
      customAddPlaceholder: "添加自定义模型标??",
      customAddButton: "添加",
      customAddAria: "添加自定义模??",
    ...
    providers: {
      ...
      visibility: {
        ...
        statusAllVisible: "所有提供方均可??",
        statusCustomOrder: "自定义顺??",
        statusHidden: (count) => `已隐??${count} 个提供方`,
        statusHiddenOne: "已隐??1 个提供方",
      ...
      updates: {
        ...
        statusAvailablePlural: (count) => `??${count} 项可用更新`,
        manualUpdate: "手动更新",
        updateButton: "更新",
        updatingButton: "正在更新",
        commandLabel: "命令??",
      ...
      tools: {
        ...
        homePathDescription: "可选的自定??Codex 主目录与配置目录??",
        agentDirLabel: "Pi 代理目录",
        agentDirDescription: "可选的自定??Pi 代理目录，用于鉴权、模型、技能与命令??",
        ...
        binaryPathDescription: (command) => `留空将使??PATH 中的 \`${command}\`。`,
    ...
    changedSettingLabel: {
      ...
      darkThemePack: "深色主题??",
      lightThemePack: "浅色主题??",
      defaultProvider: "默认提供??",
      newThreadMode: "新线程模??",
      sidebarPosition: "侧边栏位??",
      ...
      gitWritingModel: "Git 写入模型",
      customModels: "自定义模??",
      providerInstalls: "提供方安??",
      providerVisibility: "提供方可见??",
      providerOrder: "提供方顺??",
      language: "语言",
    },
  ...
  dialog: {
    ...
    pullRequest: {
      ...
      description: "粘贴 GitHub Pull Request ??URL 或编号以关联到此线程??",
      placeholder: "https://github.com/owner/repo/pull/42 ??#42",
      open: "打开",
      cancel: "取消",
    },
    worktreeHandoff: {
  ...
  browser: {
    ...
    urlPlaceholder: "搜索或输??URL",
    actionsAria: "浏览器操??",
  },
  branchToolbar: {
    newWorktree: "新建工作??",
    handoffNewWorktree: "交接至新工作??",
    handoffLocal: "交接至本??",
  ...
  themeEditor: {
    copiedTitle: "主题已复??",
    copiedDescription: (variant) => `已复??${variant} 主题分享串。`,
  ...
  notification: {
    retention: {
      title: "正在清理旧聊天??",
      preparing: "正在准备后台清理??",
      progress: (purged, total) => `已移??${purged} / {total} 个聊天。`,
      progressSimple: (purged) => `已移??${purged} 个聊天。`,
      ...
      successTitle: "旧聊天已清理",
      successDescription: (purged) => `已从数据库移??${purged} 个聊天。`,
    ...
    providerUpdate: {
      ...
      failedTitleAll: "提供方更新失??",
      failedTitleSome: "部分提供方更新失??",
      successTitleOne: (providerName) => `${providerName} 已更新`,
      successTitleMany: (count) => `已更??${count} 个提供方`,
