 ...
 * @description 线程生命周期通知检测与通知文案构建逻辑层�? * 负责识别线程/终端的完成态与需关注态变更，并生成对应的通知文案�? * 本模块为纯逻辑层，不包�?UI 相关代码�? */
...
export interface CompletedThreadCandidate {
  ...
  /** 任务完成时间戳（ISO 8601 格式�?*/
  completedAt: string;
  /** 助手最新消息的摘要文本，若无则�?null */
  assistantSummary: string | null;
}

/**
 * 线程需要用户关注的候选项，用于生�?需要输�?类通知�? */
export interface ThreadAttentionCandidate {
  /** 关注类型：approval（审批请求）�?user-input（用户输入请求） */
...
/** 线程会话状态类型，复用 ThreadSession �?status 字段类型 */
type ThreadSessionStatus = ThreadSession["status"];

/**
 * 判断是否应显示线程完成通知�?Toast 提示�? * 仅在目标线程当前不可见时才显示通知，避免对已展示在屏幕上的线程重复提醒�? *
 * @param input.threadId - 待判断的线程 ID
 * @param input.visibleThreadIds - 当前屏幕上可见的线程 ID 集合
 * @returns 若线程不可见则返�?true，表示应显示 Toast
 */
...
/**
 * 判断线程会话状态是否属�?运行�?�? * �?"running"（运行中）和 "connecting"（连接中）均视为运行态�? *
 * @param status - 线程会话状态�? * @returns 若为运行中或连接中状态则返回 true
 */
...
/**
 * 从线程消息列表中提取最后一条助手消息的摘要�? * 摘要会去除多余空白字符并截断�?140 字符以内，避免在系统通知中展示过长内容�? *
 * @param thread - 线程对象，包含完整的消息列表
 * @returns 助手消息摘要文本（最�?140 字符），若无助手消息则返�?null
 */
function summarizeLatestAssistantMessage(thread: Thread): string | null {
  ...
  for (let index = thread.messages.length - 1; index >= 0; index -= 1) {
    ...
    // 去除首尾空白并将连续空白字符压缩为单个空�?    const trimmed = message.text.trim().replace(/\s+/g, " ");
    if (trimmed.length === 0) {
      continue;
    }
    // 超过 140 字符时截断并添加省略�?    return trimmed.length <= 140 ? trimmed : `${trimmed.slice(0, 137)}...`;
  }
  return null;
}

/**
 * 判断线程是否存在未完成的轮次（turn）�? * 满足以下任一条件即视为存在未完成轮次�? * 1. 最新轮次仍处于活跃状态（有实时交互）
 * 2. 最新轮次没有完成时间戳，且会话处于运行中状�? *
 * @param thread - 线程对象
 * @returns 若存在未完成轮次则返�?true
 */
function hadUnsettledTurn(thread: Thread | undefined): boolean {
  if (!thread) {
    return false;
  }
  // 检查最新轮次是否仍有实时交�?  if (hasLiveLatestTurn(thread.latestTurn, thread.session)) {
    return true;
  }
  // 轮次未完成且会话仍在运行
...
/**
 * 判断线程的完成通知是否已稳定（settled）�? * 稳定的条件：最新轮次有开始和完成时间戳，且会话的编排状态不�?"running"�? *
 * @param thread - 线程对象
 * @returns 若通知状态已稳定（可以安全发出完成通知）则返回 true
 */
function isCompletionNotificationSettled(thread: Thread | undefined): boolean {
  // 必须有轮次的开始和完成时间�?  if (!thread?.latestTurn?.startedAt || !thread.latestTurn.completedAt) {
    return false;
  }
  // 没有会话信息时视为已稳定
  if (!thread.session) {
  ...
  // 编排状态不再是运行中，说明已完全结�?  return thread.session.orchestrationStatus !== "running";
}

/**
 * 对比前后两次线程快照，收集新产生的已完成线程候选项�? * 通过比较连续快照中的状态变化，仅在检测到�?未完�?�?已完�?的转换时才生成候选项�? * 即使会话状态直接从运行态跳至就绪态也能正确捕获�? *
 * @param previousThreads - 上一次快照中的线程列�? * @param nextThreads - 当前快照中的线程列表
...
export function collectCompletedThreadCandidates(
  ...
  // 将上一次快照按线程 ID 建立索引，便于快速查�?  const previousById = new Map(previousThreads.map((thread) => [thread.id, thread] as const));
  ...
  for (const thread of nextThreads) {
    ...
    // 当前轮次尚未完成，跳�?    if (!completedAt) {
      continue;
    }
    // 通知状态尚未稳定（如编排仍在运行），跳�?    if (!isCompletionNotificationSettled(thread)) {
      continue;
    }
    ...
    // 通过以上所有过滤条件，确认为新产生的完成事�?    candidates.push({
      threadId: thread.id,
      projectId: thread.projectId,
      title: thread.title,
      completedAt,
...
/**
 * 解析指定终端在当前线程状态下的可视化状态�? * 优先级：attention（需立即关注�? running（运行中�? review（待审查�? idle（空闲）�? *
 * @param threadState - 线程的终端通知状态快照，可能�?undefined
...
function resolveTerminalNotificationState(
  ...
  // "attention" 优先级最高，表示终端需要用户立即关�?  if (threadState.terminalAttentionStatesById?.[terminalId] === "attention") {
    return "attention";
  }
  // 终端正在运行命令
...
/**
 * 解析指定终端的通知显示标题�?CLI 类型�? * 标题优先级：标题覆盖�?> 用户标签 > CLI 类型默认标题 > "Terminal"�? *
 * @param threadState - 线程的终端通知状态快照，可能�?undefined
 * @param terminalId - 目标终端 ID
...
function resolveTerminalNotificationTitle(
  ...
  // 按优先级依次尝试获取标题：覆盖标�?�?用户标签 �?CLI 默认标题 �?兜底�?  const title =
...
/**
 * 对比前后两次终端状态快照，收集新产生的已完成终端任务候选项�? * 仅当终端状态从�?"review" 变为 "review" 时，才认为任务完成并生成候选项�? *
 * @param previousByThreadId - 上一次快照中按线�?ID 索引的终端状�? * @param nextByThreadId - 当前快照中按线程 ID 索引的终端状�? * @returns 新产生的已完成终端任务候选项数组
 */
export function collectCompletedTerminalCandidates(
  ...
  for (const threadId of threadIds) {
    ...
    for (const terminalId of terminalIds) {
      ...
      // 仅当状态变�?"review" 且之前不�?"review" 时，才视为新完成的任�?      if (nextState !== "review" || previousState === "review") {
        continue;
      }
...
/**
 * 对比前后两次线程快照，收集新产生的需要用户关注的线程候选项�? * 包括新出现的审批请求和用户输入请求，通过比对请求 ID 去重，仅保留新增项�? * 结果按创建时间升序排列�? *
 * @param previousThreads - 上一次快照中的线程列�? * @param nextThreads - 当前快照中的线程列表
...
export function collectThreadAttentionCandidates(
  ...
  for (const thread of nextThreads) {
    ...
    // 收集上一次快照中已存在的审批请求 ID，用于去�?    const previousApprovalIds = new Set(
      derivePendingApprovals(previousThread.activities).map((approval) => approval.requestId),
    );
    // 收集上一次快照中已存在的用户输入请求 ID，用于去�?    const previousUserInputIds = new Set(
      derivePendingUserInputs(previousThread.activities).map((request) => request.requestId),
    );

    // 检查当前快照中的审批请求，筛选出新增�?    for (const approval of derivePendingApprovals(thread.activities)) {
    ...
    // 检查当前快照中的用户输入请求，筛选出新增�?    for (const request of derivePendingUserInputs(thread.activities)) {
  ...
  // 按创建时间升序排列，确保通知按时间顺序处�?  return candidates.toSorted((left, right) => left.createdAt.localeCompare(right.createdAt));
}

/**
 * 对比前后两次终端状态快照，收集新产生的需要用户关注的终端候选项�? * 仅当终端状态从�?"attention" 变为 "attention" 时，才认为需要关注并生成候选项�? *
 * @param previousByThreadId - 上一次快照中按线�?ID 索引的终端状�? * @param nextByThreadId - 当前快照中按线程 ID 索引的终端状�? * @returns 新产生的需关注终端候选项数组
 */
export function collectTerminalAttentionCandidates(
  ...
  for (const threadId of threadIds) {
    ...
    for (const terminalId of terminalIds) {
      const previousState = resolveTerminalNotificationState(previousThreadState, terminalId);
      const nextState = resolveTerminalNotificationState(nextThreadState, terminalId);
      // 仅当状态变�?"attention" 且之前不�?"attention" 时，才视为新的关注事�?      if (nextState !== "attention" || previousState === "attention") {
        continue;
      }
      const { cliKind, title } = resolveTerminalNotificationTitle(nextThreadState, terminalId);
      candidates.push({
...
/**
 * 构建线程任务完成通知的显示文案�? * 确保浏览�?Toast 和操作系统通知使用一致的文案内容�? *
...
export function buildTaskCompletionCopy(candidate: CompletedThreadCandidate): {
  ...
  // 标题为空时使用默认文�?  const threadLabel = normalizedTitle.length > 0 ? normalizedTitle : "Untitled thread";

  return {
    title: threadLabel,
    // 优先使用助手消息摘要，无摘要时使用默认完成文�?    body: candidate.assistantSummary || "Finished working.",
  };
}

/**
 * 构建线程需要用户关注时的通知显示文案�? * 包括审批请求和用户输入请求两种场景�? *
...
export function buildTerminalCompletionCopy(candidate: CompletedTerminalCandidate): {
  ...
  // 终端标题为空时使用默认�?  const terminalLabel = candidate.title.trim() || "Terminal";
  return {
...
/**
 * 判断是否应抑制当前可见线程的通知�? * 当应用窗口处于前台且目标线程正在可见区域时，抑制通知以避免打扰用户�? *
 * @param input.threadId - 待判断的线程 ID
 * @param input.visibleThreadIds - 当前可见的线�?ID 集合
 * @param input.windowForeground - 应用窗口是否处于前台
 * @returns 若应抑制通知则返�?true
 */
...
/**
 * 收集"需要用户输�?的线程候选项（collectThreadAttentionCandidates 的别名）�? * 使用语义化命名以提高调用方的代码可读性�? */
export const collectInputNeededThreadCandidates = collectThreadAttentionCandidates;

/**
 * 构建"需要用户输�?通知文案（buildThreadAttentionCopy 的别名）�? * 使用语义化命名以提高调用方的代码可读性�? */
export const buildInputNeededCopy = buildThreadAttentionCopy;

/**
 * 判断候选时间戳是否属于本次通知运行时的"新鲜"事件�? * 水合（hydration）过程可能会重放旧线程数据，只有在本通知运行时启动之后产生的时间�? * 才应被视为实时事件，避免对历史数据重复触发通知�? *
 * @param candidateTimestamp - 候选事件的时间戳（ISO 8601 格式字符串）
 * @param runtimeStartedAtMs - 通知运行时的启动时间（毫秒级时间戳）
 * @returns 若时间戳晚于运行时启动时间则返回 true，表示是新鲜事件�? *          若任一参数无法解析为有效数字也返回 true（保守策略）
 */
export function isNotificationRuntimeFreshTimestamp(
  ...
  // 若时间戳无法解析，采用保守策略视为新鲜事�?  if (!Number.isFinite(candidateMs) || !Number.isFinite(runtimeStartedAtMs)) {
    return true;
  }
