 ...
 * 应以单聊模式还是分屏面板模式呈现。导出分屏感知的激活解析器，
 * 供侧边栏点击、键盘导航和搜索流程共享使用。
 */
 ...
 * 线程命令激活结果类型。
 *
 * 描述侧边栏/搜索/键盘激活线程时应执行的操作：
 * - `ignore`：忽略激活（线程不存在或已是当前活跃线程）
 * - `single`：以单聊模式打开线程
 * - `split`：在分屏面板中打开线程
 */
...
/**
 * 解析侧边栏/搜索/键盘激活线程时应执行的操作。
 *
 * 调用方决定哪个分屏（如果有）是"首选"的。首选顺序为：
 * 当前活跃的分屏优先，其次按确定性归属规则查找持久化的分屏。
 *
 * 决策逻辑：
 * 1. 线程不存在 → 忽略
 * 2. 线程有首选分屏和面板 → 分屏模式
 * 3. 线程已是当前侧边栏活跃线程 → 忽略（避免重复激活）
 * 4. 其他情况 → 单聊模式
 *
 * @param input - 激活参数
 * @param input.threadId - 要激活的线程 ID
 * @param input.threadExists - 线程是否存在
 * @param input.activeSidebarThreadId - 当前侧边栏活跃线程 ID
 * @param input.preferredSplitViewId - 首选分屏视图 ID
 * @param input.splitPaneId - 首选面板 ID
 * @returns 激活结果，包含操作类型和相关信息
 *
 * @example
 * // 线程不存在时忽略
 ...
 *
 * @param input - 查找参数
 * @param input.activeSplitView - 当前活跃的分屏视图，无活跃分屏时为 null
 * @param input.splitViewsById - 所有分屏视图的映射表
 * @param input.threadId - 要查找的线程 ID
 * @returns 匹配的分屏视图 ID 和面板 ID，未找到时返回 null
 */
export function resolvePreferredSplitForCommand(input: {
  ...
  // 优先匹配源线程归属；若非源线程且存在多个匹配则放弃，避免不确定性
