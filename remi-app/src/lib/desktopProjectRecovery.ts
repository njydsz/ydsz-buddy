/**
 * @file 桌面项目恢复检测模块
 * @description 检测桌面启动快照中是否存在隐藏项目但线程行仍存在的情况。
 *              用于桌面启动修复路径的快照形状守卫。
 */

import type { OrchestrationReadModel, OrchestrationShellSnapshot } from "@remi-code/contracts";

/** 项目恢复快照类型 */
type ProjectRecoverySnapshot = OrchestrationReadModel | OrchestrationShellSnapshot;

/**
 * 检测是否存在活跃线程但缺少对应项目的情况
 * @param snapshot - 项目恢复快照
 * @returns 是否存在需要恢复的项目
 */
