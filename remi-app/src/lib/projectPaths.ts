 ...
 * @file 项目路径工具模块
 *
 * 本模块提供项目路径相关的工具函数，用于解析、规范化、比较项目路径，
 * 跨平台兼容 Unix（`/`）和 Windows（`\`）路径分隔符。
 *
 * ## 核心导出
 *
 * - `resolveProjectRoot`：解析项目根目录的绝对路径
 * - `normalizeProjectPath`：规范化路径（合并 `//`、解析 `..`）
 * - `isPathInside`：判断路径 A 是否在路径 B 内
 * - `getRelativePath`：计算两个路径的相对路径
 * - `pathEquals`：跨平台路径比较
 *
 * ## 使用场景
 *
 ...
 * ## 注意事项
 *
 * - 所有路径处理函数均经过 `path.posix` / `path.win32` 抽象
 * - 不存在的路径也能规范化（仅做字符串处理）
 * - 符号链接不会被解析（使用真实路径请调用 `fs.realpath`）
 */
