/**
 * @file 本地图片 URL 处理模块
 * @description 为 Markdown 图片预览和下载构建认证的本地图片 URL。
 *              依赖 wsHttpUrl（以便桌面请求携带附件使用的旧版启动令牌）
 *              和 @remi-code/shared/localImage（用于规范路由和扩展名白名单）。
 */

import {
  LOCAL_IMAGE_ROUTE_PATH,
  SUPPORTED_LOCAL_IMAGE_EXTENSION_REGEX,
} from "@remi-code/shared/localImage";
import { isWindowsAbsolutePath } from "@remi-code/shared/path";

import { resolveWsHttpUrl } from "./wsHttpUrl";

/**
 * 规范化 Markdown 图片路径（内部函数）
 * @param src - 原始图片路径
...
export function isLocalImageMarkdownSrc(src: string | undefined): src is string {
  ...
  // 将 Windows 绝对路径（如 C:\foo\bar.png）视为本地图片，
 ...
 * 构建本地图片 URL
 * @param input - 输入参数
 * @param input.src - 图片源路径
 * @param input.cwd - 当前工作目录
 * @param input.download - 是否为下载模式
 * @returns 构建后的本地图片 URL
 */
export function buildLocalImageUrl(input: {
  ...
  // 始终通过 WS 派生的 HTTP 源路由，以便桌面版本（自定义协议）
  // 包含附件已使用的相同旧版启动令牌；在 Web/开发环境（页面和服务器共享源）
  // 中，这会回退到相同的相对路径
