/**
 * @file 编辑器偏好设置模块
 * @description 管理用户上次使用的编辑器偏好，支持 localStorage 持久化。
 *              提供编辑器偏好的读取、持久化和自动打开功能。
 */
...
/**
 * 解析并持久化用户偏好的编辑器
 * 优先使用 localStorage 中存储的偏好，若不可用则选择可用编辑器列表中的第一个并持久化
 * @param availableEditors - 当前可用的编辑器 ID 列表
