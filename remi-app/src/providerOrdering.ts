 ...
 * @file Provider 排序管理
 *
 * 维护 Provider 选择器中的排序稳定性，确保设置页面、搜索和菜单中的顺序一致。
 * 提供默认排序、排序标准化、排序比较等工具函数。
 */
...
/**
 * Provider 的默认显示顺序。
 * 此顺序决定了侧边栏、选择器等 UI 中 Provider 的排列位置。
 */
...
/** 基于 DEFAULT_PROVIDER_ORDER 构建的 Provider 类型集合，用于快速查找 */
const PROVIDER_KIND_SET: ReadonlySet<ProviderKind> = new Set(DEFAULT_PROVIDER_ORDER);

/**
 * 判断给定字符串是否为有效的 ProviderKind 类型。
 *
 * @param value - 待判断的字符串
 * @returns 是否为有效的 ProviderKind
 */
...
/**
 * 标准化 Provider 排序列表。
 * 过滤无效值和重复项，并将用户未指定的 Provider 按默认顺序追加到末尾。
 *
 * @param providerOrder - 原始 Provider 排序列表
 * @returns 标准化后的完整 ProviderKind 排序数组
 */
...
/**
 * 按照指定排序比较两个 Provider 的先后顺序。
 * 不在排序列表中的 Provider 排在末尾，按默认顺序排列。
 *
 * @param providerOrder - 排序规则列表
 * @param left - 第一个 Provider
 * @param right - 第二个 Provider
 * @returns 负数表示 left 在前，正数表示 right 在前，0 表示相同
 */
