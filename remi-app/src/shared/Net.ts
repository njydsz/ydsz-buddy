 ...
 *
 * Node.js 的系统错误通常包含一个 code 属性（'EADDRINUSE'、'ECONNREFUSED' 等），
 * 该函数用于安全地检查并收窄错误类型。
 *
 * @param cause - 待检查的错误对象
 * @returns 如果 cause 是带 string 类型 code 属性的对象则返回 true
 */
 ...
 * 网络服务接口定义
 *
 * 描述 NetService 提供的所有网络辅助功能，包括：
 * - 检查指定主机和端口的可绑定性
 * - 检查环回地址（IPv4/IPv6）上的端口可用性
 * - 预留临时环回端口
 * - 查找可用端口（支持首选端口）
 */
...
/**
 * NetService - 启动阶段网络辅助工具的服务标识
 *
 * 基于 Effect ServiceMap 实现，提供依赖注入能力。
