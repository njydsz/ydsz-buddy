 ...
 *   - 切换 provider 但未指定 model 时，自动使用�?provider 的默认模�? *   - options 在替换模式下仅使用补丁中的值，非替换模式下深度合并
 *
 ...
 * @returns 如果补丁包含 provider �?model 字段则返�?true，表示应执行替换操作
 *
 ...
 * 1. **普通字�?*：使�?`deepMerge` 进行深度合并，patch 中的�?undefined 值覆�?current 中的�? * 2. **模型选择字段**（`textGenerationModelSelection`）使用特殊合并策略：
 *    - 若补丁指定了新的 `provider` 但未指定 `model`�? *      - 如果�?provider 与当前不同且不是 "pi"，自动使用新 provider 的默认模�? *      - 否则保留当前 model
 *    - 若补丁指定了 `model`：直接使用补丁中�?model
 *    - `options` 字段：替换模式下仅使用补丁中�?options；非替换模式下深度合�? *
...
export function applyServerSettingsPatch(
  ...
  // - 若补丁切换了 provider 且新 provider 不是 "pi"，则使用�?provider 的默认模�?  // - 否则保留当前 model
  const model =
  ...
  // 确定 options：替换模式下仅使用补丁中�?options，非替换模式下深度合�?  const options = shouldReplaceTextGenerationModelSelection(selectionPatch)
