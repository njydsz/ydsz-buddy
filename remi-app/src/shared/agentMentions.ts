 ...
 *
 * @description
 * 提供用户输入 `@alias(task)` 格式的内联代理指令解析功能。
 ...
 *
 * @property {string} alias - 代理别名（如 "reviewer"、"tester" 等）
 * @property {string} task - 任务描述（括号内的内容）
 * @property {string} raw - 原始提及文本（包含 `@alias(task)` 完整内容）
 ...
 *
 * @param text - 待解析的输入文本
 * @param provider - 代理提供商类型（"claudeAgent" 等）
 ...
 * @example
 * ```ts
 * const text = '@reviewer(审查代码) @tester(写测试)';
 * const invocations = parseAgentMentionInvocations(text, 'claudeAgent');
 *
 ...
 * @example 不匹配的提及会被忽略
 * ```ts
 * const text = '邮箱 user@example.com @invalid(未闭合)';
 * const invocations = parseAgentMentionInvocations(text, 'claudeAgent');
