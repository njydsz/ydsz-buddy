/**
 * @file NativeApi 入口模块
 * @description 提供获取原生 API 实例的统一入口�? *              按优先级依次尝试：window.nativeApi（桌面端注入）→ Tauri 桥接 �?WebSocket 实现�? *              确保在不同运行环境下都能获取到合适的 NativeApi 实例�? */
 ...
 * 4. 基于 WebSocket 的实�? * @returns NativeApi 实例，SSR 环境下返�?undefined
 */
...
/**
 * 确保 NativeApi 实例可用，不可用时抛出错�? * @returns NativeApi 实例
 * @throws 当无法获�?NativeApi 实例时抛出错�? */
