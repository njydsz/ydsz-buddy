...
// Depends on: nothing runtime â€?only types below.
//
 ...
 *
 * `image`, `imageAlt`, and `details` are optional â€?a release can still ship
 ...
 * omitted, the card falls back to a gradient + icon â€?so a release without a
 ...
 * dialog â€?it just sorts as the lowest possible value.
 */
 ...
 * list of releases to the user â€?both the post-update dialog and the
 ...
 * pass the same shape it already has â€?no parameter juggling.
 */
export interface WhatsNewInputs {
   ...
   * install â€?we silently mark the current version as seen instead of showing
 ...
 * - `silent-bootstrap`: first launch or no curated entry for this upgrade â€? *   no dialog, just record `nextLastSeenVersion` so we don't dump the
 ...
 * into the main view â€?we just confirm the current release has curated
...
export function resolveWhatsNewState(inputs: WhatsNewInputs): WhatsNewState {
  ...
  // surface anything â€?we only move the marker forward, never backward.
  ...
  if (!currentEntry) {
    // No curated notes for the installed build â€?silently advance so we
