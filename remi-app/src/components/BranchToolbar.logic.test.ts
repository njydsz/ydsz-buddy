  ...
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.peakcode/worktrees/feature-a",
  ...
  it("syncs stale local thread metadata to the concrete git checkout", () => {
    expect(
      shouldSyncLocalThreadBranch({
        envMode: "local",
        activeWorktreePath: null,
        activeThreadBranch: "peakcode/pi",
        currentGitBranch: "main",
  ...
  it("preserves associated worktree metadata during local branch-only syncs", () => {
    expect(
      resolveAssociatedWorktreeMetadataAfterWorkspacePatch({
        branch: "main",
        worktreePath: null,
        existingAssociatedWorktreePath: "/repo/.worktrees/peakcode-pi",
        existingAssociatedWorktreeBranch: "peakcode/pi",
        existingAssociatedWorktreeRef: "peakcode/pi",
      }),
    ).toEqual({
      associatedWorktreePath: "/repo/.worktrees/peakcode-pi",
      associatedWorktreeBranch: "peakcode/pi",
      associatedWorktreeRef: "peakcode/pi",
    });
  });
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        activeWorktreePath: "/repo/.peakcode/worktrees/feature-a",
        branch: {
          isDefault: false,
          worktreePath: "/repo/.peakcode/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.peakcode/worktrees/feature-b",
      nextWorktreePath: "/repo/.peakcode/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default branches", () => {
    expect(
      ...
      checkoutCwd: "/repo/.peakcode/worktrees/feature-a",
      nextWorktreePath: "/repo/.peakcode/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
