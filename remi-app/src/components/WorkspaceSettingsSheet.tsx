                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected
                        ? "border-(--color-border) bg-(--sidebar-accent) shadow-sm"
                        : "border-(--color-border-light) bg-card/50 hover:border-(--color-border) hover:bg-(--sidebar-accent)",
                    )}
                    aria-pressed={isSelected}
                    onClick={() => props.onSelectPreset(preset.id)}
                  >
                    <div className="relative h-24 overflow-hidden rounded-xl bg-linear-to-b from-muted/65 to-muted/35 p-0.5">
                      <WorkspaceLayoutPresetPreview presetId={preset.id} />
                      {isSelected ? (
                        <div className="absolute right-2 top-2 inline-flex size-6 items-center justify-center rounded-full bg-(--color-text-foreground) text-(--color-background-surface) shadow-sm">
