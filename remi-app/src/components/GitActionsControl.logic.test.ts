...
describe("resolveDefaultCreateBranchName", () => {
  it("uses peakcode as the default namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "fix toast copy");
    assert.equal(branch, "peakcode/fix-toast-copy");
  });

  it("keeps an existing peakcode namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "peakcode/refine-toolbar-actions");
    assert.equal(branch, "peakcode/refine-toolbar-actions");
  });

  it("preserves nested namespaces under peakcode", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "feature/refine-toolbar-actions");
    assert.equal(branch, "peakcode/feature/refine-toolbar-actions");
  });
...
describe("resolveLiveThreadBranchUpdate", () => {
  it("does not regress a semantic thread branch back to a temporary worktree branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      ...
      gitStatus: status({ branch: "peakcode/deadbeef" }),
    });
...
describe("shouldOfferCreateBranchPrompt", () => {
  const temporaryBranch = "peakcode/deadbeef";
