...
const BROWSER_WEBVIEW_PARTITION = "persist:remi-claw-browser";
const BROWSER_BLANK_URL = "about:blank";
const BROWSER_PERF_SAMPLE_INTERVAL_MS = 5_000;
const REMI_CLAW_BROWSER_LABEL = "Remi Claw browser";
...
function isBrowserPerfLoggingEnabled(): boolean {
  ...
  try {
    return window.localStorage.getItem("remi-claw:browser-perf") === "1";
  } catch {
    return false;
  }
}
...
export function BrowserPanel({ mode, threadId, onClosePanel }: BrowserPanelProps) {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      console.info(`[${REMI_CLAW_BROWSER_LABEL} panel perf]`, {
        threadId,
