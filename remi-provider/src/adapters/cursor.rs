...
//!
//! 本模块实现了 Cursor Provider 的适配器，通过 JSON-RPC over stdio 协议
//! 与 Cursor 进程进行交互。
//!
//! # 功能特性
//!
...
//! # 协议说明
//!
//! Cursor 使用 JSON-RPC over stdio 协议进行通信：
...
/// Cursor Provider 适配器
///
/// 通过 JSON-RPC over stdio 与 Cursor 进程通信。
/// 内部维护活跃会话列表和事件广播通道。
///
/// # 能力特性
///
/// Cursor 支持技能提及、技能发现、运行时模型列表和 Turn 转向，
/// 但不支持原生命令发现。
pub struct CursorAdapter {
    /// 活跃会话列表
    ///
    /// 键为 thread_id，值为对应的 JSON-RPC 客户端实例。
    /// 每个会话对应一个独立的 Cursor 子进程。
    sessions: Arc<RwLock<HashMap<String, Arc<JsonRpcClient>>>>,
    /// 事件广播发送器
    ///
    /// 将 Cursor 推送的通知转换为 `ProviderRuntimeEvent` 后广播给订阅者。
...
impl CursorAdapter {
    ...
    /// 启动 Cursor 进程并建立 JSON-RPC 连接
    ///
    /// 启动 Cursor CLI 子进程，传入模型和工作目录参数，
    /// 并启动后台事件监听线程。
    ///
    /// # 参数
    ///
    /// - `model`: 要使用的 Cursor 模型名称
    /// - `cwd`: 子进程的工作目录
    ///
    /// # 返回值
    ///
    ...
    async fn spawn_cursor_process(
        ...
        info!("启动 Cursor 进程: model={}, cwd={}", model, cwd);
    ...
    async fn listen_events(
        ...
        loop {
            match client.recv_notification().await {
                Some(notification) => {
                    debug!("收到 Cursor 通知: {}", notification.method);
...
///
/// 为 CursorAdapter 提供完整的 Provider 适配器接口实现，
/// 包括会话管理、消息发送、Turn 转向、事件流等核心功能。
#[async_trait]
impl ProviderAdapter for CursorAdapter {
    /// 获取 Provider 类型标识
    ///
    /// 返回 `ProviderKind::Cursor`，标识此适配器对应的 Provider 类型。
    ...
    /// 启动新的 Cursor 会话
    ///
    /// 启动 Cursor 子进程，初始化会话，并将客户端保存到会话列表。
    ///
    /// # 参数
    ///
    ...
    async fn start_session(&self, input: ProviderSessionStartInput) -> ProviderResult<ProviderSession> {
        ...
        let client = self.spawn_cursor_process(&input.model, &cwd).await?;
    ...
    /// 转向 Turn（重定向运行中的对话）
    ///
    /// 在 Turn 执行过程中重定向对话方向。Cursor 支持此功能。
    ///
    /// # 参数
    ///
    ...
    /// 订阅 Cursor Provider 事件流
    ///
    /// 创建新的事件接收器，用于接收 Cursor 推送的运行时事件。
    /// 多个订阅者可以同时接收相同的事件。
    ///
    /// # 返回值
    ///
