//! Kilo Provider 适配器实现
//!
//! 本模块实现了 Kilo Provider 的适配器，通过 JSON-RPC over stdio 协议
//! 与 Kilo AI 进程进行交互。
//!
//! # 功能特性
//!
...
impl KiloAdapter {
    ...
    /// 启动 Kilo 进程并建立 JSON-RPC 连接
    ///
    /// 启动 Kilo CLI 子进程，传入模型和工作目录参数，
    /// 并启动后台事件监听线程。
    ///
    /// # 参数
    ///
    /// - `model`: 要使用的 Kilo 模型名称
    /// - `cwd`: 子进程的工作目录
    ///
    /// # 返回值
    ///
    ...
    async fn spawn_kilo_process(
        &self,
        model: &str,
        cwd: &str,
    ) -> ProviderResult<JsonRpcClient> {
        info!("启动 Kilo 进程: model={}, cwd={}", model, cwd);
    ...
    async fn listen_events(
        ...
        loop {
            match client.recv_notification().await {
                Some(notification) => {
                    debug!("收到 Kilo 通知: {}", notification.method);
...
///
/// 为 KiloAdapter 提供基础的 Provider 适配器接口实现。
/// Kilo 不支持 Turn 转向、技能提及等高级功能。
#[async_trait]
impl ProviderAdapter for KiloAdapter {
    ...
    /// 启动新的 Kilo 会话
    ///
    /// 启动 Kilo 子进程，初始化会话，并将客户端保存到会话列表。
    ///
    /// # 参数
    ///
    ...
    async fn start_session(&self, input: ProviderSessionStartInput) -> ProviderResult<ProviderSession> {
        ...
        let client = self.spawn_kilo_process(&input.model, &cwd).await?;
    ...
    /// 订阅 Kilo Provider 事件流
    ///
    /// 创建新的事件接收器，用于接收 Kilo 推送的运行时事件。
    /// 多个订阅者可以同时接收相同的事件。
    ///
    /// # 返回值
    ///
