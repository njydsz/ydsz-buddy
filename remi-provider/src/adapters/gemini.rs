...
//!
//! 本模块实现了 Gemini Provider 的适配器，通过 JSON-RPC over stdio 协议
//! 与 Google Gemini 进程进行交互。
//!
//! # 功能特性
//!
//! - **会话管理**：支持创建、停止、列出会话
...
//! # 协议说明
//!
//! Gemini 使用 JSON-RPC over stdio 协议进行通信：
...
/// Gemini Provider 适配器
///
/// 通过 JSON-RPC over stdio 与 Google Gemini 进程通信。
/// 内部维护活跃会话列表和事件广播通道。
///
/// # 能力特性
///
/// Gemini 支持会话内模型切换和运行时模型列表，
/// 但不支持技能提及、技能发现、Turn 转向和原生命令发现。
pub struct GeminiAdapter {
    /// 活跃会话列表
    ///
    /// 键为 thread_id，值为对应的 JSON-RPC 客户端实例。
    /// 每个会话对应一个独立的 Gemini 子进程。
    sessions: Arc<RwLock<HashMap<String, Arc<JsonRpcClient>>>>,
    /// 事件广播发送器
    ///
    /// 将 Gemini 推送的通知转换为 `ProviderRuntimeEvent` 后广播给订阅者。
...
impl GeminiAdapter {
    ...
    /// 启动 Gemini 进程并建立 JSON-RPC 连接
    ///
    /// 启动 Gemini CLI 子进程，传入模型和工作目录参数，
    /// 并启动后台事件监听线程。
    ///
    /// # 参数
    ///
    ...
    async fn spawn_gemini_process(
        ...
        info!("启动 Gemini 进程: model={}, cwd={}", model, cwd);
    ...
    async fn listen_events(
        ...
        loop {
            match client.recv_notification().await {
                Some(notification) => {
                    debug!("收到 Gemini 通知: {}", notification.method);
...
///
/// 为 GeminiAdapter 提供基础的 Provider 适配器接口实现。
/// Gemini 不支持 Turn 转向、技能提及等高级功能。
#[async_trait]
impl ProviderAdapter for GeminiAdapter {
    ...
    /// 启动新的 Gemini 会话
    ///
    /// 启动 Gemini 子进程，初始化会话，并将客户端保存到会话列表。
    ///
    /// # 参数
    ///
    ...
    async fn start_session(&self, input: ProviderSessionStartInput) -> ProviderResult<ProviderSession> {
        ...
        let client = self.spawn_gemini_process(&input.model, &cwd).await?;
    ...
    /// 订阅 Gemini Provider 事件流
    ///
    /// 创建新的事件接收器，用于接收 Gemini 推送的运行时事件。
    /// 多个订阅者可以同时接收相同的事件。
    ///
    /// # 返回值
    ///
