...
//!
//! 本模块实现了 Gemini Provider 的适配器，通过 ACP (Agent Communication Protocol) 协议
//! 与 Gemini AI 服务进行交互。
//!
//! # 功能特性
//!
...
//! # 协议说明
//!
//! Gemini 使用 ACP (Agent Communication Protocol) 协议进行通信：
//! - 基于 HTTP/2 的双向流式通信
//! - 支持异步事件推送
//! - 提供标准化的消息格式
//!
//! # 当前状态
//!
//! 当前实现为占位逻辑，核心功能（如实际的 ACP 协议通信）尚未实现。
//! 需要后续集成 Gemini SDK 或实现自定义的 ACP 客户端。
//!
//! # 模块依赖
//!
...
//!
//! ```rust,ignore
//! use remi_provider::adapters::GeminiAdapter;
...
/// Gemini 适配器
///
/// 实现 `ProviderAdapter` trait，提供与 Gemini AI 服务的交互能力。
/// 内部维护会话列表和事件广播通道。
///
/// # 线程安全
///
...
impl ProviderAdapter for GeminiAdapter {
    /// 获取 Provider 类型标识
    ///
    /// 返回 `ProviderKind::Gemini`，标识此适配器对应的 Provider 类型。
    ...
    /// 获取适配器能力声明
    ///
    /// Gemini 适配器当前不支持高级特性（如模型切换、技能发现等）。
    ...
    /// 启动新的会话
    ///
    /// 创建并初始化一个新的 Gemini 会话。当前实现为占位逻辑，
    /// 需要后续集成 Gemini SDK 实现实际的会话启动。
    ///
    /// # 参数
    ///
    ...
    /// # TODO
    ///
    /// 需要实现 Gemini 会话启动逻辑（ACP 协议）
    async fn start_session(&self, input: ProviderSessionStartInput) -> ProviderResult<ProviderSession> {
        info!("GeminiAdapter: 启动会话 thread_id={}", input.thread_id);

        // TODO: 实现 Gemini 会话启动逻辑（ACP 协议）
        // 当前为占位实现，生成模拟的会话信息
    ...
    /// 发送 Turn（对话轮次）
    ///
    /// 将用户消息发送到 Gemini Provider，启动一个新的对话轮次。
    /// 当前实现为占位逻辑。
    ///
    /// # 参数
    ///
    ...
    /// # TODO
    ///
    /// 需要实现 Gemini Turn 发送逻辑
    async fn send_turn(&self, input: TurnInput) -> ProviderResult<ProviderTurnStartResult> {
        info!("GeminiAdapter: 发送 Turn thread_id={}", input.thread_id);

        // TODO: 实现 Gemini Turn 发送逻辑
    ...
    /// # TODO
    ///
    /// 需要实现 Gemini Turn 中断逻辑
    async fn interrupt_turn(&self, thread_id: &str, turn_id: Option<&str>) -> ProviderResult<()> {
        info!("GeminiAdapter: 中断 Turn thread_id={}, turn_id={:?}", thread_id, turn_id);

        // TODO: 实现 Gemini Turn 中断逻辑
        Ok(())
    }
    ...
    async fn stop_session(&self, thread_id: &str) -> ProviderResult<()> {
        info!("GeminiAdapter: 停止会话 thread_id={}", thread_id);
