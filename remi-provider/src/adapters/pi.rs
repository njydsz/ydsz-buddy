//! Pi Provider 适配器实现
//!
//! 本模块实现了 Pi Provider 的适配器，通过 JSON-RPC over stdio 协议
//! 与 Inflection Pi 进程进行交互。
//!
//! # 功能特性
//!
//! - **会话管理**：支持创建、停止、列出会话
...
/// Pi Provider 适配器
///
/// 通过 JSON-RPC over stdio 与 Inflection Pi 进程通信。
/// 内部维护活跃会话列表和事件广播通道。
///
/// # 能力特性
///
/// Pi 支持会话内模型切换和运行时模型列表，
...
pub struct PiAdapter {
    ...
    /// 事件广播发送器
    ///
    /// 将 Pi 推送的通知转换为 `ProviderRuntimeEvent` 后广播给订阅者。
...
impl PiAdapter {
    ...
    /// 启动 Pi 进程并建立 JSON-RPC 连接
    ///
    /// 启动 Pi CLI 子进程，传入模型和工作目录参数，
    /// 并启动后台事件监听线程。
    ///
    /// # 参数
    ///
    ...
    async fn spawn_pi_process(
        ...
        info!("启动 Pi 进程: model={}, cwd={}", model, cwd);
    ...
    async fn listen_events(
        ...
        loop {
            match client.recv_notification().await {
                Some(notification) => {
                    debug!("收到 Pi 通知: {}", notification.method);
...
///
/// 为 PiAdapter 提供基础的 Provider 适配器接口实现。
/// Pi 不支持 Turn 转向、技能提及等高级功能。
#[async_trait]
impl ProviderAdapter for PiAdapter {
    /// 获取 Provider 类型标识
    ///
    /// 返回 `ProviderKind::Pi`，标识此适配器对应的 Provider 类型。
    ...
    async fn start_session(&self, input: ProviderSessionStartInput) -> ProviderResult<ProviderSession> {
        ...
        let client = self.spawn_pi_process(&input.model, &cwd).await?;
