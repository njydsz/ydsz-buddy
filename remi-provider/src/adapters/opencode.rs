...
//!
//! 本模块实现了 OpenCode Provider 的适配器，通过 OpenCode SDK
//! 与 OpenCode AI 服务进行交互。
//!
//! # 功能特性
//!
//! - **会话管理**：支持创建、停止、列出会话
...
//! # 当前状态
//!
//! 当前实现为占位逻辑，核心功能（如实际的 OpenCode SDK 集成）尚未实现。
//! 需要后续集成 OpenCode SDK。
//!
//! # 模块依赖
//!
...
//!
//! ```rust,ignore
//! use remi_provider::adapters::OpenCodeAdapter;
...
/// OpenCode 适配器
///
/// 实现 `ProviderAdapter` trait，提供与 OpenCode AI 服务的交互能力。
/// 内部维护会话列表和事件广播通道。
///
/// # 线程安全
///
...
impl ProviderAdapter for OpenCodeAdapter {
    ...
    /// 获取适配器能力声明
    ///
    /// OpenCode 适配器当前不支持高级特性（如模型切换、技能发现等）。
    ...
    /// 启动新的会话
    ///
    /// 创建并初始化一个新的 OpenCode 会话。当前实现为占位逻辑，
    /// 需要后续集成 OpenCode SDK 实现实际的会话启动。
    ///
    /// # 参数
    ///
    ...
    /// 发送 Turn（对话轮次）
    ///
    /// 将用户消息发送到 OpenCode Provider，启动一个新的对话轮次。
    /// 当前实现为占位逻辑。
    ///
    /// # 参数
    ///
    ...
    /// # TODO
    ///
    /// 需要实现 OpenCode Turn 发送逻辑
    async fn send_turn(&self, input: TurnInput) -> ProviderResult<ProviderTurnStartResult> {
        info!("OpenCodeAdapter: 发送 Turn thread_id={}", input.thread_id);

        // TODO: 实现 OpenCode Turn 发送逻辑
    ...
    /// # TODO
    ///
    /// 需要实现 OpenCode Turn 中断逻辑
    async fn interrupt_turn(&self, thread_id: &str, turn_id: Option<&str>) -> ProviderResult<()> {
        info!("OpenCodeAdapter: 中断 Turn thread_id={}, turn_id={:?}", thread_id, turn_id);

        // TODO: 实现 OpenCode Turn 中断逻辑
        Ok(())
    }
    ...
    async fn stop_session(&self, thread_id: &str) -> ProviderResult<()> {
        info!("OpenCodeAdapter: 停止会话 thread_id={}", thread_id);
