...
//! # 核心组件
//!
//! - **[`ProviderSessionReaper`]**: 会话清理服务，负责定期扫描和清理过期会话
//!
//! # 清理策略
//!
//! - **扫描间隔**：每 5 分钟执行一次清理扫描
...
//!        ↓
//! ┌─────────────┐
//! │ reap_sessions│ ← 定期执行清理（每 5 分钟）
//! └──────┬──────┘
//!        ↓
//! ┌─────────────┐
//! │ shutdown    │ ← 收到关闭信号，优雅退出
//! └─────────────┘
//! ```
//!
//! # 使用场景
//!
//! - **长时间运行的服务**：自动清理不再使用的会话，释放资源
//! - **资源受限环境**：防止会话积累导致内存溢出
//! - **生产环境部署**：配合监控系统，确保系统稳定运行
//!
//! # 模块依赖
//!
//! - 依赖 [`crate::service::ProviderService`] 查询和停止会话
//! - 依赖 `tokio` 异步运行时提供定时器和信号处理
//! - 依赖 `chrono` 库计算会话存活时间
//!
//! # 使用示例
//!
