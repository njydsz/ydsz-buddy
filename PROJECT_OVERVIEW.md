...
> 基于 Peak Code 全面迁移至 Rust 技术栈的 AI 编程助手桌面应用
...
- 📦 **桌面应用**：基于 Tauri 2.x 构建，支持 Windows、macOS、Linux
...
Remi Code 的迁移分为 10 个阶段，按模块逐步推进：
...
- ✅ 环境检测适配（Electron → Tauri）
- ✅ Tauri 桥接层实现（tauri-bridge.ts）
- ✅ 路由历史模式调整
- ✅ 原生 API 适配
- ⏳ 组件迁移与适配
- ⏳ 状态管理适配
...
- [后端迁移全案](./BACKEND_MIGRATION_FULL_PLAN.md)
- [前端迁移与迭代计划](./MIGRATION_AND_ITERATION_PLAN.md)
