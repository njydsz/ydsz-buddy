...
//! ┌─────────────────.?//! .? EventStore     .?//! .? (事件存储)     .?//! └────────┬────────.?//!          .
ead_events()
//!          .?//! ┌─────────────────.?//! .?  Projector     .?//! .? ┌───────────.? .?//! .? .?轮询循环  .? .?//! .? .?(100ms)   .? .?//! .? └─────┬─────.? .?//! .?       .?       .?//! .? ┌───────────.? .?//! .? │apply_event.? .?//! .? └─────┬─────.? .?//! └────────┼────────.?//!          .?//!          .?//! ┌─────────────────.?//! .
rojectionRepo  �./! �.投影仓库)      �./! └─────────────────�./! ```
//!
//! ## 断点续传机制
//!
...
//! ```text
//! 启动 .?get_projection_state() .
ast_sequence
//!   .?//! 循环 .?read_events(last_sequence, 100) .
vents
//!   .?//! 处理 .
pply_event() for each event
//!   .?//! 更新 .
ast_sequence = max(event.sequence)
//!   .?//! 持久.?.
pdate_projection_state(last_sequence)
//! ```
//!
//! ## 性能参数
//!
...
///
    /// # 运行流程
    ///
    /// 1. 从投影仓库恢复上次处理的序列号（断点续传.?    /// 2. 进入主循环，.?100ms 轮询一次事件存.?    /// 3. 批量读取新事件（每次最.
00 条）
    /// 4. 逐个应用事件到投影仓.?    /// 5. 更新投影器处理进.?    /// 6. 收到关闭信号后退出循.
...
///
    /// # 处理流程
    ///
    /// 1. 从事件存储读取新事件（从 `last_sequence` 开始，最.
00 条）
    /// 2. 如果没有新事件，直接返回
    /// 3. 遍历事件列表，逐个反序列化并应.?    /// 4. 更新 `last_sequence` 为最新处理的序列.
/// 5. 将投影器进度持久化到投影仓库
...
///
   ...
   async fn apply_event(&self, event: &OrchestrationEvent) -> OrchestrationResult<()> {
        ...
        match event {
            ...
            // 线程事件
            ThreadCreated(e) => {
                // 注意：此处使用默认值初始化线程，而非从事件中提取完整字段.?                // 这是因为 Projector 的投影逻辑.?Engine 内的投影逻辑不同.?                // Engine 内投影会从事件中提取完整字段，.?Projector 使用默认值.?                // 后续事件（如 ThreadRuntimeModeSet 等）会逐步更新这些字段.
            ...
            ThreadReverted(e) => {
                if let Some(mut thread) = self.projection_repo.get_thread(e.thread_id)? {
                    // 回滚时清空所有可变状态，恢复到初始状.?                    // 注意：这.?Engine 内投影的处理不同，Engine 仅移除指定检查点之后的数.
