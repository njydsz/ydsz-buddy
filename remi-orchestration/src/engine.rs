        ...
        loop {
            ...
            for stored_event in &events {
                ...
                let event: OrchestrationEvent = serde_json::from_str(&stored_event.payload_json)
                    .map_err(|e| {
                        crate::error::OrchestrationError::InternalError(format!(
        ...
        match event {
            ...
            OrchestrationEvent::ThreadCreated(e) => {
                let thread = Thread {
                    ...
                    associated_worktree_branch: e.associated_worktree.as_ref().map(|w| w.branch.clone()),
                    associated_worktree_ref: e.associated_worktree.as_ref().map(|w| w.r#ref.clone()),
                    shell_summary: None,
