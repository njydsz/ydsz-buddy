...
impl OrchestrationEngine {
    ...
    pub fn new(db: Arc<Database>, provider_registry: Arc<ProviderRegistry>) -> Self {
        ...
        // Generous broadcast capacity: orchestration events can spike during
        // streaming and we don't want backpressure to slow down the
        // engine. Late subscribers can resync via `replay_events`.
    ...
    /// Build a new engine with the default reactor registry wired up.
    pub fn with_default_reactors(
        db: Arc<Database>,
        provider_registry: Arc<ProviderRegistry>,
    ) -> Self {
        let engine = Self::new(db, provider_registry);
        // Re-wire the broadcast channel and reactor registry so that
        // notifications emitted by reactors (and the public subscribers)
    ...
    /// Spawn the background event loop that pumps events into the reactor
    /// registry. The returned [`tokio::task::JoinHandle`] can be aborted to
    /// stop processing.
    ...
    async fn persist_and_project(&self, event: &OrchestrationEvent) -> Result<()> {
        ...
        // Fan out the event to reactors and broadcast subscribers. We
        // intentionally do not fail the command if a reactor errors —
        // reactors are best-effort side effects.
        self.reactors.fan_out(event).await;

        // Broadcast to external subscribers. `send` returns an error only
