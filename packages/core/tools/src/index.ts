 ...
 * execution pipeline.
 * @module @njydsz/ydb-tools
 */

import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@njydsz/ydb-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@njydsz/ydb-scope'
import type { CallId, ContentBlock, ToolSchema } from '@njydsz/ydb-llm'
import { assertNever, deepFreeze, HarnessError } from '@njydsz/ydb-llm'
import type { Agent } from '@njydsz/ydb-agent'
import { snapshotJsonValue } from '@njydsz/ydb-session'
import type { JsonValue, UserMessage } from '@njydsz/ydb-session'
import type { ToolProviderResult } from '@njydsz/ydb-system-prompt'
import type { CodeRuntime } from '@njydsz/ydb-code-runtime'
...
export type { JsonValue } from '@njydsz/ydb-session'
...
// lives in its own UI-facing module; re-export it so `@njydsz/ydb-tools`
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@njydsz/ydb-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@njydsz/ydb-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@njydsz/ydb-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@njydsz/ydb-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@njydsz/ydb-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @njydsz/ydb-code-runtime-worker-thread) or set tools mode to "native"`)
    }
