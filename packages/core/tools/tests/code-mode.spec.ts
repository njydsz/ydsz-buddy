...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@njydsz/ydb-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@njydsz/ydb-tools'
import type { Agent } from '@njydsz/ydb-agent'
import { Session, SessionId } from '@njydsz/ydb-session'
import type { JsonValue, SessionEventMap } from '@njydsz/ydb-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@njydsz/ydb-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
