/** Local durable attachment backend rooted below `YDB_HOME`. @module @njydsz/ydb-attachment-local */
...
export interface Config {
  /** Explicit harness home; omitted follows `YDB_HOME`, then `~/.dsh`. */
  dshHome?: string
...
export class LocalAttachmentStore extends AttachmentStore {
  ...
  constructor(ctx: Context, config: Config) {
    super(ctx)
    this.root = resolve(join(resolveYdbHome(config.dshHome), 'attachments', 'v1'))
