...
import * as GoalSessionInvariant from '@njydsz/ydb-goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@njydsz/ydb-goal-round-driver'
import InvariantRegistry, { InvariantError } from '@njydsz/ydb-invariants'
import SessionStore, { SessionId, type Session } from '@njydsz/ydb-session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@njydsz/ydb-goal-round-driver',
    }))
  })
