 ...
 * @module @njydsz/ydb-jobs-local
 */

import { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import type { Agent } from '@njydsz/ydb-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@njydsz/ydb-scope'
import type { ScopeLayer } from '@njydsz/ydb-scope'
import { deadline, timeoutOf } from '@njydsz/ydb-timeout'
import { JobRegistry, JobId } from '@njydsz/ydb-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@njydsz/ydb-jobs'
 ...
 * `@njydsz/ydb-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @njydsz/ydb-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @njydsz/ydb-agent)')
    }
