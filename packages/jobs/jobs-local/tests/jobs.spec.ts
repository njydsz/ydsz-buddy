...
import { bindScopeParent, createScope, scopeOf } from '@njydsz/ydb-scope'
import type { ScopeKey } from '@njydsz/ydb-scope'
import { JobId } from '@njydsz/ydb-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@njydsz/ydb-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @njydsz/ydb-tool-jobs in its composition)')
  })
