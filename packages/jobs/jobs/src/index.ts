 ...
 * process-local registry lives in `@njydsz/ydb-jobs-local`.
 * @module @njydsz/ydb-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@njydsz/ydb-jobs is the abstract job registry seam; load an implementation such as @njydsz/ydb-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
