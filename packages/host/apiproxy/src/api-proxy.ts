...
import { installModelSelection } from '@njydsz/ydb-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@njydsz/ydb-agent'
import type {} from '@njydsz/ydb-agent-presets/types'
import { AttachmentError } from '@njydsz/ydb-attachment'
import type { ImageAttachmentRef } from '@njydsz/ydb-attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@njydsz/ydb-llm'
import { errorChain } from '@njydsz/ydb-llm'
import type { ContentBlock, MessageSource } from '@njydsz/ydb-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@njydsz/ydb-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@njydsz/ydb-session'
import type { SessionPersistence } from '@njydsz/ydb-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@njydsz/ydb-session-query'
import { SubagentError } from '@njydsz/ydb-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@njydsz/ydb-subagent'
import { isUserInvocable } from '@njydsz/ydb-skill'
import type { Workspace, WorkspaceRecord } from '@njydsz/ydb-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@njydsz/ydb-workspace'
...
import type { PresetBearingSession } from '@njydsz/ydb-agent-presets'
import type {} from '@njydsz/ydb-tools'
import type {
...
import type {} from '@njydsz/ydb-session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@njydsz/ydb-jobs'
import type { JobSnapshot } from '@njydsz/ydb-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@njydsz/ydb-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@njydsz/ydb-goal'
import type { GoalRef as CoreGoalRef } from '@njydsz/ydb-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@njydsz/ydb-commands'
...
import type {} from '@njydsz/ydb-cordis-host-runner/types'
import type {} from '@njydsz/ydb-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@njydsz/ydb-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@njydsz/ydb-settings'
import { credentialRef } from '@njydsz/ydb-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@njydsz/ydb-session-title'
import type { CallId } from '@njydsz/ydb-llm/brand'
import type { ScopeKey } from '@njydsz/ydb-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@njydsz/ydb-user-approval'
...
} from '@njydsz/ydb-user-questions'
import { UserQuestionError } from '@njydsz/ydb-user-questions'
import { DirectoryPickerError } from '@njydsz/ydb-host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @njydsz/ydb-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @njydsz/ydb-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @njydsz/ydb-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @njydsz/ydb-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @njydsz/ydb-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @njydsz/ydb-skill', details: {} })
        }
