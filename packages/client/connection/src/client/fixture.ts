...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@njydsz/ydb-llm/message'
import { CallId } from '@njydsz/ydb-llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@njydsz/ydb-attachment'
import type {
  SessionEvent,
  SessionId,
  TodoItem,
} from '@njydsz/ydb-session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@njydsz/ydb-commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@njydsz/ydb-commands/types'
import { deriveEventMessage, foldSurface } from '@njydsz/ydb-session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@njydsz/ydb-host-apiproxy/api'
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@njydsz/ydb-tool-bash'\n" }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@njydsz/ydb-tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@njydsz/ydb-tool-read'\n" }],
  ])
