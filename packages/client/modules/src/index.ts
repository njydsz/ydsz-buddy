/**
 * Node half of the client module system (`ydb.client` dual-face package): scans
 * the host Loader's entries for packages declaring `ydb.client`, composes the
 ...
 * @module @njydsz/ydb-client-modules
 */
...
/** package.json `ydb.client` declaration fields, validated one by one after reading the file. */
interface YdbClientDeclaration {
  inject?: string[]
  platform: string
...
/** Resolved package metadata for one `ydb.client` package (cached per name, never expires). */
interface PkgMeta {
  clientPath: string
  inject?: string[]
  immediately: boolean
...
/** Narrow an unknown parsed JSON value to the `ydb.client` declaration, throwing on malformed fields. */
function parseYdbClient(pkgName: string, value: unknown): YdbClientDeclaration | undefined {
  if (value === undefined) return undefined
  if (typeof value !== 'object' || value === null) {
    throw new Error(`client-modules: ${pkgName} has a non-object ydb.client declaration`)
  }
  const decl = value as Record<string, unknown>
  if (typeof decl.platform !== 'string') {
    throw new Error(`client-modules: ${pkgName} ydb.client.platform must be a string`)
  }
  if (decl.inject !== undefined && (!Array.isArray(decl.inject) || decl.inject.some(i => typeof i !== 'string'))) {
    throw new Error(`client-modules: ${pkgName} ydb.client.inject must be a string array`)
  }
  if (decl.immediately !== undefined && typeof decl.immediately !== 'boolean') {
    throw new Error(`client-modules: ${pkgName} ydb.client.immediately must be a boolean`)
  }
  return {
...
/**
 * The web plugin table service: incremental `ydb.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
  ...
  // subpath rows — or a package without a web `ydb.client` declaration) are
  ...
  private resolveMeta(pkgName: string): PkgMeta | null {
    ...
    const ydb = pkg.ydb
    const decl = parseYdbClient(
      pkgName,
      ydb !== null && typeof ydb === 'object' ? (ydb as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${pkgName} declares ydb.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
