 ...
 */
export const WORKTREE_BRANCH_PREFIX = "remi-code";
...
const REMI_CODE_BRANCH_FALLBACK = "update";
...
export function buildRemiCodeBranchName(preferredBranch?: string | null): string {
  const normalizedExisting = preferredBranch?.trim().replace(/^(codex|remi-code)\//i, "") ?? "";
  return `${WORKTREE_BRANCH_PREFIX}/${sanitizeBranchFragment(
    normalizedExisting || REMI_CODE_BRANCH_FALLBACK,
  )}`;
}
