...
import { assembleContextFor, type Agent } from '@njydsz/ydb-agent'
...
import type { SubagentStartRequest } from '@njydsz/ydb-subagent'
...
describe('dsh-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @njydsz/ydb-jobs')
  })
