...
import { agentEvents } from '@njydsz/ydb-agent'
import { TOOL_ORDER_REST } from '@njydsz/ydb-system-prompt'
import type { Message } from '@njydsz/ydb-llm'
import { SessionId } from '@njydsz/ydb-session'
import * as acpAgent from '../src/index.ts'

/**
 * In-process unit coverage for the @njydsz/ydb-acp-demo composition:
...
async function withIsolatedSkillHomes<T>(run: () => Promise<T>): Promise<T> {
  ...
  try {
    ...
    if (oldDshHome === undefined) {
      ...
      process.env.YDB_HOME = oldDshHome
    }
