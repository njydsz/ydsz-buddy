/**
 * File-backed credentials provider over `$YDB_HOME/.credentials.yaml`, layered
 * against the environment by how much each layer is trusted:
 *
 * ```text
 * inherited process environment      (read-only, wins)
 * > $YDB_HOME/.credentials.yaml      (provider-managed, writable)
 * > <invocation cwd>/.env            (read-only fallback)
 * > $YDB_HOME/.env                   (read-only fallback)
 * ```
 *
 ...
 * @module @njydsz/ydb-credentials-local
 */
...
import type { LaunchEnvironmentEntry } from '@njydsz/ydb-launch-environment'
...
export interface Config {
  ...
  path?: string
  /** Harness home used when `path` is omitted; defaults to `$YDB_HOME` or `~/.dsh`. */
  dshHome?: string
...
/** File-backed credentials provider (`$YDB_HOME/.credentials.yaml`). */
