...
} from '@njydsz/ydb-typert-registry'
import type {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@njydsz/ydb-tools', 'ToolInput')).toBe('@njydsz/ydb-tools#ToolInput')
    expect(typertPackageKey('@njydsz/ydb-tools', 'host')).toBe('@njydsz/ydb-tools#host')
    expect(ctx.typert.get('@njydsz/ydb-tools#ToolInput')).toMatchObject({
      package: '@njydsz/ydb-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@njydsz/ydb-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@njydsz/ydb-tools', 'host')).toMatchObject({
      key: '@njydsz/ydb-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@njydsz/ydb-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@njydsz/ydb-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@njydsz/ydb-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@njydsz/ydb-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@njydsz/ydb-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@njydsz/ydb-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@njydsz/ydb-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@njydsz/ydb-tools#ToolInput')).not.toBe(projected)
  })
