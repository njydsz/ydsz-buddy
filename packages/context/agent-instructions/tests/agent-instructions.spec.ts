...
import * as workspaceContext from '@njydsz/ydb-agent-instructions'
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@njydsz/ydb-llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@njydsz/ydb-session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@njydsz/ydb-agent'
import AgentLoop from '@njydsz/ydb-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@njydsz/ydb-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import ToolRuntime, { defineContentToolFixture } from '@njydsz/ydb-tools'
import type {
  ToolExecution,
  ToolExecutionToken,
...
} from '@njydsz/ydb-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('defaults dshHome and uses cwd itself as root when no project marker exists', async () => {
    ...
    // Isolate the default-home fallback: blank YDB_HOME is treated as unset, and
  ...
  it('honors YDB_HOME when dshHome is not configured explicitly', async () => {
    const root = await tempRepo()
    const envHome = await tempRepo()
    try {
      await write(join(envHome, 'AGENTS.md'), 'env global rule')
      vi.stubEnv('YDB_HOME', envHome)

      const files = await discoverBaselineInstructionFiles({ cwd: root })

      expect(files).toEqual([{ absolutePath: join(envHome, 'AGENTS.md'), displayPath: '$YDB_HOME/AGENTS.md' }])
    } finally {
      vi.unstubAllEnvs()
      await rm(root, { recursive: true, force: true })
  ...
  it('labels the default DSH home as ~/.dsh when HOME points at the configured default', async () => {
    ...
    try {
      ...
      // A set YDB_HOME would override the homedir default and relabel the home.
      vi.stubEnv('YDB_HOME', '')
      vi.resetModules()
      vi.doMock('node:os', () => ({ homedir: () => home }))
      const isolated = await import('@njydsz/ydb-agent-instructions')
  ...
  it('deduplicates user-global instructions when dshHome points at the project root', async () => {
    ...
    try {
      ...
      expect(files).toEqual([{ absolutePath: join(root, 'AGENTS.md'), displayPath: '$YDB_HOME/AGENTS.md' }])
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
...
describe('workspace context request injection', () => {
  ...
  it('labels a custom dshHome as YDB_HOME instead of pretending it is ~/.dsh', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      expect(files.map(file => file.displayPath)).toEqual(['$YDB_HOME/AGENTS.md'])
    } finally {
      await rm(root, { recursive: true, force: true })
      await rm(home, { recursive: true, force: true })
    }
