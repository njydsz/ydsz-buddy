...
it('isolates replay skill discovery from every ambient host root', async () => {
  ...
  try {
    ...
    try {
      ...
      if (originalDshHome === undefined) delete process.env.YDB_HOME
      else process.env.YDB_HOME = originalDshHome
