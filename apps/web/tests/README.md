...
## Do not import `@njydsz/ydb-client-*` here
...
`@njydsz/ydb-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@njydsz/ydb-client-web` and the boot-manifest
type from `@njydsz/ydb-client-modules/client`: booting the real shell is what
...
`@njydsz/ydb-client-runtime/client` because `client/runtime` is reachable
