...
describe("storageKeyMigration", () => {
  ...
  it("does not modify existing remi-code keys", async () => {
    globalThis.localStorage.setItem("remi-code:renderer-state:v8", '{"projectNamesByCwd":{}}');

    await importMigrationFresh();

    expect(globalThis.localStorage.getItem("remi-code:renderer-state:v8")).toBe(
      '{"projectNamesByCwd":{}}',
    );
  });
