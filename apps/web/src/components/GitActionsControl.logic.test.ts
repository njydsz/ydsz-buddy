...
describe("resolveDefaultCreateBranchName", () => {
  it("uses remi-code as the default namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "fix toast copy");
    assert.equal(branch, "remi-code/fix-toast-copy");
  });

  it("keeps an existing remi-code namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "remi-code/refine-toolbar-actions");
    assert.equal(branch, "remi-code/refine-toolbar-actions");
  });

  it("preserves nested namespaces under remi-code", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "feature/refine-toolbar-actions");
    assert.equal(branch, "remi-code/feature/refine-toolbar-actions");
  });
...
describe("resolveLiveThreadBranchUpdate", () => {
  it("does not regress a semantic thread branch back to a temporary worktree branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      ...
      gitStatus: status({ branch: "remi-code/deadbeef" }),
    });
...
describe("shouldOfferCreateBranchPrompt", () => {
  const temporaryBranch = "remi-code/deadbeef";
