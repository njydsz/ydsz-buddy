...
import type { BrowserExecuteCdpInput, ThreadBrowserState, ThreadId } from "@remi-code/contracts";
...
const BROWSER_USE_PIPE_NAME_PREFIX = "remi-code-iab";
export const REMI_CODE_BROWSER_USE_PIPE_ENV = "REMI_CODE_BROWSER_USE_PIPE_PATH";
...
export function resolveConfiguredBrowserUsePipePath(
  ...
  const configured =
    env[REMI_CODE_BROWSER_USE_PIPE_ENV]?.trim() || env[REMI_CODE_BROWSER_USE_PIPE_ENV]?.trim();
  return configured || resolveDefaultBrowserUsePipePath(platform);
}

export const REMI_CODE_BROWSER_USE_PIPE_PATH = resolveConfiguredBrowserUsePipePath();
...
export class BrowserUsePipeServer {
  ...
  constructor(
    ...
    options: BrowserUsePipeServerOptions | string = REMI_CODE_BROWSER_USE_PIPE_PATH,
  ) {
    this.pipePath =
      typeof options === "string" ? options : (options.pipePath ?? REMI_CODE_BROWSER_USE_PIPE_PATH);
  ...
  private async createTabForSession(sessionId: string): Promise<{
    ...
    if (!snapshot) {
      throw new Error("No active Remi Code browser pane available");
    }
