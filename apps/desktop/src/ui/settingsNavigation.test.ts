...
describe("settingsNavigation", () => {
  ...
  it("exposes localized group labels", () => {
    expect(buildSettingsNavGroups(MESSAGES.en).map((g) => g.label)).toEqual(["App", "Remi Code"]);
    expect(buildSettingsNavGroups(MESSAGES.zh).map((g) => g.label)).toEqual(["应用", "Remi Code"]);
  });
});
