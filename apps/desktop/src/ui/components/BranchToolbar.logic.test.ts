...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.remi-code/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a branch already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.remi-code/worktrees/feature-a",
        currentWorktreePath: null,
  ...
  it("syncs stale local thread metadata to the concrete git checkout", () => {
    expect(
      shouldSyncLocalThreadBranch({
        envMode: "local",
        activeWorktreePath: null,
        activeThreadBranch: "remi-code/pi",
        currentGitBranch: "main",
  ...
  it("preserves associated worktree metadata during local branch-only syncs", () => {
    expect(
      resolveAssociatedWorktreeMetadataAfterWorkspacePatch({
        branch: "main",
        worktreePath: null,
        existingAssociatedWorktreePath: "/repo/.worktrees/remi-code-pi",
        existingAssociatedWorktreeBranch: "remi-code/pi",
        existingAssociatedWorktreeRef: "remi-code/pi",
      }),
    ).toEqual({
      associatedWorktreePath: "/repo/.worktrees/remi-code-pi",
      associatedWorktreeBranch: "remi-code/pi",
      associatedWorktreeRef: "remi-code/pi",
    });
  });
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        activeWorktreePath: "/repo/.remi-code/worktrees/feature-a",
        branch: {
          isDefault: false,
          worktreePath: "/repo/.remi-code/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.remi-code/worktrees/feature-b",
      nextWorktreePath: "/repo/.remi-code/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default branches", () => {
    expect(
      ...
      checkoutCwd: "/repo/.remi-code/worktrees/feature-a",
      nextWorktreePath: "/repo/.remi-code/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
