...
// Purpose: Renders the in-app browser chrome and mirrors the native Tauri view.
...
const BROWSER_WEBVIEW_PARTITION = "persist:remi-code-browser";
const BROWSER_BLANK_URL = "about:blank";
const BROWSER_PERF_SAMPLE_INTERVAL_MS = 5_000;
const REMI_CODE_BROWSER_LABEL = "Remi Code browser";
const PANEL_RESIZE_OVERLAY_SYNC_EVENT = "remi-code:panel-resize-overlay-sync";
...
function isBrowserPerfLoggingEnabled(): boolean {
  ...
  try {
    return window.localStorage.getItem("remi-code:browser-perf") === "1";
  } catch {
    return false;
  }
}
...
export function BrowserPanel({ mode, threadId, onClosePanel }: BrowserPanelProps) {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      console.info(`[${REMI_CODE_BROWSER_LABEL} panel perf]`, {
        threadId,
