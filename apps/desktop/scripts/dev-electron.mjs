...
function cleanupStaleDevApps() {
  ...
  spawnSync("pkill", ["-f", "--", `--remi-code-dev-root=${desktopDir}`], { stdio: "ignore" });
}
...
function listStaleComputerUsePids() {
  ...
  const result = spawnSync("pgrep", ["-fal", "Remi Code \\(Dev\\).*(computerUseMcp\\.mjs mcp)"], {
    encoding: "utf8",
  });
...
function cleanupStaleComputerUseApps() {
  ...
  console.error(
    `[desktop-dev] Cleaning up ${stalePids.length} stale Remi Code (Dev) Computer Use helper process${stalePids.length === 1 ? "" : "es"} from other worktrees.`,
  );
...
function warnIfAlphaAppRunning() {
  ...
  const result = spawnSync(
    "pgrep",
    ["-fal", "/Applications/Remi Code \\(Alpha\\)\\.app/Contents/MacOS/Remi Code \\(Alpha\\)"],
    { encoding: "utf8" },
  );
  const output = typeof result.stdout === "string" ? result.stdout.trim() : "";
  if (!output) {
  ...
  console.error(
    "[desktop-dev] Remi Code (Alpha) is still running. Close it before testing voice in Remi Code (Dev), or you may be looking at the wrong app/runtime.",
  );
...
function startApp() {
  ...
  const app = spawn(
    resolveElectronPath(),
    [`--remi-code-dev-root=${desktopDir}`, "dist-electron/main.js"],
    {
      cwd: desktopDir,
      env: {
        ...childEnv,
