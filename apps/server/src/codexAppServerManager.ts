...
import { getModelSelectionBooleanOptionValue, normalizeModelSlug } from "@remi-code/shared/model";
...
// Maps Remi Code's simple runtime toggle to Codex thread-level permission overrides.
...
export function ensureIsolatedScratchWorkspace(threadId: ThreadId): string {
  const workspaceRoot = path.join(tmpdir(), "remi-code-codex-workspaces");
...
export class CodexAppServerManager extends EventEmitter<CodexAppServerManagerEvents> {
  ...
  private shouldSuppressChildConversationNotification(method: string): boolean {
    ...
    // card advance ("1 out of 5" â†?"2 out of 5" ...) and render streaming plan text;
  ...
  private parseModelListResponse(response: unknown): ProviderListModelsResult["models"] {
    ...
    return rawModels.flatMap((value) => {
      ...
      // Accept both Remi Code's legacy string array and Remodex-style reasoning objects.
