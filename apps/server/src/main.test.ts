...
it.layer(testLayer)("server CLI command", (it) => {
  ...
  it.effect("prefers --mode over REMI_CODE_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over REMI_CODE_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        REMI_CODE_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/provider-log/websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        REMI_CODE_MODE: "desktop",
        REMI_CODE_LOG_PROVIDER_EVENTS: "true",
        REMI_CODE_LOG_WS_EVENTS: "false",
        REMI_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        REMI_CODE_NO_BROWSER: "true",
      });
