...
// Purpose: Builds the exact environment used when Remi Code launches Codex subprocesses.
...
import { readActiveCodexProviderEnvKey } from "@remi-code/shared/codexConfig";
import {
  readEnvironmentFromLoginShell,
  resolveLoginShell,
  type ShellEnvironmentReader,
} from "@remi-code/shared/shell";
...
const REMI_CODE_BROWSER_PLUGIN_CONFIG_HEADER = '[plugins."remi-code-browser@local"]';
...
export function resolveCodexBrowserUsePipePath(
  ...
  const configured =
    env.REMI_CODE_BROWSER_USE_PIPE_PATH?.trim() || env.REMI_CODE_BROWSER_USE_PIPE_PATH_LEGACY?.trim();
  if (configured) {
    return configured;
  }
...
export function disableRemiCodeBrowserPluginInCodexConfig(config: string): string {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("[") && trimmed.endsWith("]")) {
      closeTargetSection();
      inTargetSection = trimmed === REMI_CODE_BROWSER_PLUGIN_CONFIG_HEADER;
  ...
  if (!sawTargetSection) {
    ...
    output.push(REMI_CODE_BROWSER_PLUGIN_CONFIG_HEADER, "enabled = false");
  }

  return output.join("\n");
}

function prepareRemiCodeCodexHomeOverlay(input: {
  readonly env: NodeJS.ProcessEnv;
  readonly homePath?: string;
}): string | undefined {
  const sourceHomePath = resolveBaseCodexHomePath(input.env, input.homePath);
  const overlayHomePath = resolveRemiCodeCodexHomeOverlayPath(input.env, sourceHomePath);
  ...
  writeFileSync(
    ...
    disableRemiCodeBrowserPluginInCodexConfig(sourceConfig),
    "utf8",
  );
...
export function buildCodexProcessEnv(
  ...
  const overlayHomePath = shouldDisableRemiCodeBrowserPlugin(baseEnv)
    ? prepareRemiCodeCodexHomeOverlay({
        env: baseEnv,
