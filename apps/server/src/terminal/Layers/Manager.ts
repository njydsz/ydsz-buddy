...
function shouldStripOscSequence(content: string): boolean {
  return (
    /^(10|11|12);(?:\?|rgb:)/.test(content) || content.startsWith(REMI_CODE_TERMINAL_HOOK_OSC_PREFIX)
  );
}
...
function extractOscHookEvent(content: string): TerminalAgentHookEventType | null {
  if (!content.startsWith(REMI_CODE_TERMINAL_HOOK_OSC_PREFIX)) {
    return null;
  }
  const eventType = content.slice(REMI_CODE_TERMINAL_HOOK_OSC_PREFIX.length).trim();
...
function shouldExcludeTerminalEnvKey(key: string): boolean {
  ...
  if (normalizedKey.startsWith("REMI_CODE_")) {
    return true;
  }
...
function cliKindFromRuntimeEnv(
  ...
  return terminalCliKindFromValue(runtimeEnv?.[REMI_CODE_TERMINAL_CLI_KIND_ENV_KEY]);
}
...
export class TerminalManagerRuntime extends EventEmitter<TerminalManagerEvents> {
  ...
  private onProcessData(session: TerminalSessionState, data: string): void {
    ...
    if (session.pendingOutputLength >= OUTPUT_BATCH_SIZE_LIMIT) {
      // Large burst â€?flush immediately to avoid excessive latency.
      this.flushOutputBuffer(session);
        ...
        runningSessions.map(async (session) => {
          ...
          try {
            ...
            if (session.managedAgentObserved) {
              // Hooks have fired â€?trust them as the sole source of truth (superset model).
              ...
              // No hooks observed â€?fall back to process-tree + output heuristic.
