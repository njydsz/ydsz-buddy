...
describe("TerminalManager", () => {
  ...
  function makeManager(
    ...
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "remi-code-terminal-"));
  ...
  it("retries with fallback shells when preferred shell spawn fails", async () => {
    ...
    if (process.platform === "win32") {
      ...
      const fallbackShells = ["/bin/zsh", "/bin/bash", "/bin/sh", "zsh", "bash", "sh"];
      const requestedShell = ptyAdapter.spawnInputs[0]?.shell;
      const hasUsableShell = ptyAdapter.spawnInputs.some(
        (input) =>
          input.shell !== requestedShell &&
          (fallbackShells.includes(input.shell) || input.shell.length > 0),
      );
      expect(hasUsableShell).toBe(true);
    }
  ...
  it("filters app runtime env variables from terminal sessions", async () => {
    ...
    try {
      ...
      expect(spawnInput.env.REMI_CODE_PORT).toBeUndefined();
  ...
  it("injects runtime env overrides into spawned terminals", async () => {
    ...
    await manager.open(
      openInput({
        env: {
          REMI_CODE_PROJECT_ROOT: "/repo",
          REMI_CODE_WORKTREE_PATH: "/repo/worktree-a",
          CUSTOM_FLAG: "1",
        },
      }),
    );
    ...
    expect(spawnInput.env.REMI_CODE_PROJECT_ROOT).toBe("/repo");
    expect(spawnInput.env.REMI_CODE_WORKTREE_PATH).toBe("/repo/worktree-a");
