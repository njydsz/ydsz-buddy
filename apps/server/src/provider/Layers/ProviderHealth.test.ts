...
import { REMI_CODE_CODEX_HOME_OVERLAY_DIR } from "../../codexHomePaths";
import {
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.acquireRelease(
      Effect.sync(() => {
        ...
        const originalRemiCodeHome = process.env.REMI_CODE_HOME;
        const originalPortkeyApiKey = process.env.PORTKEY_API_KEY;
        process.env.CODEX_HOME = tmpDir;
        process.env.REMI_CODE_HOME = runtimeDir;
        process.env.PORTKEY_API_KEY ??= "test-portkey-key";
        return { originalCodexHome, originalRemiCodeHome, originalPortkeyApiKey };
      }),
      ({ originalCodexHome, originalRemiCodeHome, originalPortkeyApiKey }) =>
        Effect.sync(() => {
          ...
          if (originalRemiCodeHome !== undefined) {
            process.env.REMI_CODE_HOME = originalRemiCodeHome;
          } else {
  ...
  describe("checkCodexProviderStatus", () => {
    ...
    it.effect("uses configured codex home for version, config, and auth probes", () => {
      ...
      return Effect.gen(function* () {
        ...
        expectedCodexHome = path.join(runtimeDir, REMI_CODE_CODEX_HOME_OVERLAY_DIR);
