...
import { ServerProviderUpdateError } from "@remi-code/contracts";
import { parseCodexConfigModelProvider } from "@remi-code/shared/codexConfig";
import { decodeJsonResult } from "@remi-code/shared/schemaJson";
        ...
        const finalProviders = yield* setProviderUpdateState(
          ...
          makeUpdateState({
            ...
            message: stillOutdated
              ? "Update command completed, but Remi Code still detects an outdated provider version."
              : "Provider updated.",
            output: output ? output.slice(0, UPDATE_OUTPUT_MAX_BYTES) : null,
          }),
        );
