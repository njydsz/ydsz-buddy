...
import type { ProviderRuntimeEvent } from "@remi-code/contracts";
...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  const previousRemiCodeHome = process.env.REMI_CODE_HOME;
  const previousLegacyHome = process.env.REMI_CODE_HOME_LEGACY;
  const previousDisableFlag = process.env.REMI_CODE_DISABLE_CODEX_REMI_CODE_BROWSER_PLUGIN;

  afterEach(() => {
    if (previousRemiCodeHome === undefined) delete process.env.REMI_CODE_HOME;
    else process.env.REMI_CODE_HOME = previousRemiCodeHome;
    if (previousLegacyHome === undefined) delete process.env.REMI_CODE_HOME_LEGACY;
    else process.env.REMI_CODE_HOME_LEGACY = previousLegacyHome;
    if (previousDisableFlag === undefined)
      delete process.env.REMI_CODE_DISABLE_CODEX_REMI_CODE_BROWSER_PLUGIN;
    else process.env.REMI_CODE_DISABLE_CODEX_REMI_CODE_BROWSER_PLUGIN = previousDisableFlag;
  });

  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.REMI_CODE_HOME = "/remi-code-test/runtime";
    delete process.env.REMI_CODE_DISABLE_CODEX_REMI_CODE_BROWSER_PLUGIN;
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/remi-code-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });

  it("returns the source generated_images directory when the remi-code-browser plugin is enabled", () => {
    process.env.REMI_CODE_HOME = "/remi-code-test/runtime";
  ...
  it("collapses to a single root when overlay equals source", () => {
    delete process.env.REMI_CODE_HOME;
    delete process.env.REMI_CODE_HOME_LEGACY;
    // The overlay falls under `<dirname(source)>/.remi-code/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.remi-code/runtime/codex-home-overlay";
