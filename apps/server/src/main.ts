...
const CliEnvConfig = Config.all({
  mode: Config.string("REMI_CODE_MODE").pipe(
    Config.option,
    Config.map(
  ...
  noBrowser: Config.boolean("REMI_CODE_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  authToken: Config.string("REMI_CODE_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("REMI_CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logProviderEvents: Config.boolean("REMI_CODE_LOG_PROVIDER_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("REMI_CODE_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("Remi Code running", {
      ...safeConfig,
...
const logProviderEventsFlag = Flag.boolean("log-provider-events").pipe(
  Flag.withDescription(
    "Emit native/canonical provider NDJSON logs for debugging (equivalent to REMI_CODE_LOG_PROVIDER_EVENTS).",
  ),
  Flag.optional,
);
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to REMI_CODE_LOG_WS_EVENTS).",
  ),
...
export const t3Cli = Command.make("t3", {
  ...
  Command.withDescription("Run the Remi Code server."),
