...
describe("auth utils", () => {
  it("resolves stable web and port-scoped desktop cookie names", () => {
    expect(resolveSessionCookieName({ mode: "web", port: 3773 })).toBe("remi_code_session");
    expect(resolveSessionCookieName({ mode: "desktop", port: 3773 })).toBe("remi_code_session_3773");
  });
