...
const snapshotSqliteDatabase = (sourcePath: string, targetPath: string) =>
  Effect.tryPromise({
    try: async () => {
      ...
      new HomeMigrationError({
        message: `Failed to snapshot legacy sqlite database from ${sourcePath} to ${targetPath}. Close other Remi Code processes and retry.`,
        cause,
      }),
  });
...
export const migrateLegacyHomeIfNeeded = Effect.fn(function* (input: LegacyHomeMigrationInput) {
  const fs = yield* FileSystem.FileSystem;
  const path = yield* Path.Path;
  const canonicalTargetBaseDir = path.resolve(path.join(input.homeDir, REMI_CODE_HOME_DIRNAME));
  ...
  const stagingBaseDir = path.join(
    input.homeDir,
    `.${REMI_CODE_HOME_DIRNAME.slice(1)}-migration-${process.pid}-${Date.now()}`,
  );
