...
import { ApprovalRequestId, ThreadId } from "@remi-code/contracts";

import {
  buildCodexProcessEnv,
  disableRemiCodeBrowserPluginInCodexConfig,
...
describe("buildCodexProcessEnv", () => {
  ...
  it("allows the configured desktop browser-use socket in the Codex sandbox", () => {
    const env = buildCodexProcessEnv({
      env: {
        REMI_CODE_BROWSER_USE_PIPE_PATH: "/tmp/codex-browser-use/remi-code.sock",
    ...
    expect(env.NODE_REPL_SANDBOX_ALLOWED_UNIX_SOCKETS).toBe(
      "/tmp/existing.sock,/tmp/codex-browser-use/remi-code.sock",
    );
  });
    ...
    try {
      writeFileSync(
        ...
        [
          ...
          "enabled = true",
          "",
          '[plugins."remi-code-browser@local"]',
          "enabled = true",
        ].join("\n"),
        "utf8",
      );
      ...
      const env = buildCodexProcessEnv({
        env: { REMI_CODE_HOME: runtimeHome },
        homePath: tempDir,
        platform: "darwin",
      });
  ...
  it("adds a disabled remi-code-browser plugin section when Codex config does not contain one", () => {
    expect(disableRemiCodeBrowserPluginInCodexConfig('model = "gpt-5.5"')).toContain(
      '[plugins."remi-code-browser@local"]\nenabled = false',
    );
  });
});
...
describe("startSession", () => {
  it("enables Codex experimental api capabilities during initialize", () => {
    expect(buildCodexInitializeParams()).toEqual({
      clientInfo: {
        name: "remi-code_desktop",
        title: "Remi Code Desktop",
        version: "0.1.0",
      },
      capabilities: {
  ...
  it("uses an isolated scratch workspace path when no cwd is provided", () => {
    ...
    expect(cwd).toContain(`${path.sep}remi-code-codex-workspaces${path.sep}thread-1`);
  });
  ...
  it("fails fast with an upgrade message when codex is below the minimum supported version", async () => {
    ...
    const versionCheck = vi
      ...
      .mockImplementation(() => {
        throw new Error(
          "Codex CLI v0.36.0 is too old for Remi Code. Upgrade to v0.37.0 or newer and restart Remi Code.",
        );
      });
