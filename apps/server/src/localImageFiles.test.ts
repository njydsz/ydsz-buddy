...
describe("resolveAllowedLocalImageFile", () => {
  it("allows images inside the current workspace", async () => {
    const workspace = makeTempDir("remi-code-image-workspace-");
  ...
  it("allows images inside Codex generated_images without a cwd", async () => {
    const codexHome = makeTempDir("remi-code-codex-home-");
  ...
  it("allows images written to the REMI_CODE_HOME codex-home-overlay generated_images root", async () => {
    // Codex app-server is launched with CODEX_HOME pointing at a Remi Code overlay
    // directory (see resolveRemiCodeCodexHomeOverlayPath). Generated images therefore
    // live under <REMI_CODE_HOME>/codex-home-overlay/generated_images/<thread>/<call>.png,
    ...
    const remi-codeHome = path.join(fakeRoot, "remi-code", "runtime");
    ...
    process.env.REMI_CODE_HOME = remi-codeHome;
    try {
      const result = await resolveAllowedLocalImageFile({
        requestedPath: imagePath,
        cwd: null,
      ...
      if (previousRemiCodeHome === undefined) {
        delete process.env.REMI_CODE_HOME;
      } else {
        process.env.REMI_CODE_HOME = previousRemiCodeHome;
      }
