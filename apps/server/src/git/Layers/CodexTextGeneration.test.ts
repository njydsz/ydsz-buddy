...
const CodexTextGenerationTestLayer = CodexTextGenerationLive.pipe(
  Layer.provideMerge(
    ServerConfig.layerTest(process.cwd(), {
      prefix: "remi-code-codex-text-generation-test-",
    }),
  ),
  ...
  return Effect.gen(function* () {
    ...
    yield* fs.writeFileString(
      ...
      [
        ...
        "done",
        'stdin_content="$(cat)"',
        'if [ "$REMI_CODE_FAKE_CODEX_REQUIRE_IMAGE" = "1" ] && [ "$seen_image" != "1" ]; then',
        '  printf "%s\\n" "missing --image input" >&2',
        "  exit 2",
        "fi",
        'if [ "$REMI_CODE_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK" = "1" ] && [ "$seen_skip_git_repo_check" != "1" ]; then',
        '  printf "%s\\n" "missing --skip-git-repo-check" >&2',
        "  exit 9",
        "fi",
        'if [ "$REMI_CODE_FAKE_CODEX_REQUIRE_APPROVAL_NEVER" = "1" ] && [ "$seen_approval_never" != "1" ]; then',
        '  printf "%s\\n" "missing approval_policy=never" >&2',
        "  exit 10",
        "fi",
        'if [ -n "$REMI_CODE_FAKE_CODEX_STDIN_MUST_CONTAIN" ]; then',
        '  printf "%s" "$stdin_content" | grep -F -- "$REMI_CODE_FAKE_CODEX_STDIN_MUST_CONTAIN" >/dev/null || {',
        '    printf "%s\\n" "stdin missing expected content" >&2',
        "    exit 3",
        "  }",
        "fi",
        'if [ -n "$REMI_CODE_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN" ]; then',
        '  if printf "%s" "$stdin_content" | grep -F -- "$REMI_CODE_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN" >/dev/null; then',
        '    printf "%s\\n" "stdin contained forbidden content" >&2',
        "    exit 4",
        "  fi",
        "fi",
        'if [ "$REMI_CODE_FAKE_CODEX_REQUIRE_CODEX_HOME" = "1" ] && [ -z "$CODEX_HOME" ]; then',
        '  printf "%s\\n" "missing CODEX_HOME" >&2',
        "  exit 5",
        "fi",
        'if [ "$REMI_CODE_FAKE_CODEX_REQUIRE_AUTH_JSON" = "1" ] && [ ! -f "$CODEX_HOME/auth.json" ]; then',
        '  printf "%s\\n" "missing auth.json in CODEX_HOME" >&2',
        "  exit 6",
        "fi",
        'if [ -n "$REMI_CODE_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN" ]; then',
        '  grep -F -- "$REMI_CODE_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN" "$CODEX_HOME/config.toml" >/dev/null || {',
        '    printf "%s\\n" "CODEX_HOME config missing expected content" >&2',
        "    exit 7",
        "  }",
        "fi",
        'if [ -n "$REMI_CODE_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN" ]; then',
        '  if grep -F -- "$REMI_CODE_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN" "$CODEX_HOME/config.toml" >/dev/null; then',
        '    printf "%s\\n" "CODEX_HOME config contained forbidden content" >&2',
        "    exit 8",
        "  fi",
        "fi",
        'if [ -n "$REMI_CODE_FAKE_CODEX_STDERR" ]; then',
        '  printf "%s\\n" "$REMI_CODE_FAKE_CODEX_STDERR" >&2',
        "fi",
        'if [ -n "$output_path" ]; then',
        '  node -e \'const fs=require("node:fs"); const value=process.argv[2] ?? ""; fs.writeFileSync(process.argv[1], Buffer.from(value, "base64"));\' "$output_path" "${REMI_CODE_FAKE_CODEX_OUTPUT_B64:-e30=}"',
        "fi",
        'exit "${REMI_CODE_FAKE_CODEX_EXIT_CODE:-0}"',
        "",
      ].join("\n"),
    );
...
function withFakeCodexEnv<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      const fs = yield* FileSystem.FileSystem;
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "remi-code-codex-text-" });
      const binDir = yield* makeFakeCodexBinary(tempDir);
      const previousPath = process.env.PATH;
      const previousOutput = process.env.REMI_CODE_FAKE_CODEX_OUTPUT_B64;
      const previousExitCode = process.env.REMI_CODE_FAKE_CODEX_EXIT_CODE;
      const previousStderr = process.env.REMI_CODE_FAKE_CODEX_STDERR;
      const previousRequireImage = process.env.REMI_CODE_FAKE_CODEX_REQUIRE_IMAGE;
      const previousStdinMustContain = process.env.REMI_CODE_FAKE_CODEX_STDIN_MUST_CONTAIN;
      const previousStdinMustNotContain = process.env.REMI_CODE_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN;
      const previousRequireCodexHome = process.env.REMI_CODE_FAKE_CODEX_REQUIRE_CODEX_HOME;
      const previousRequireAuthJson = process.env.REMI_CODE_FAKE_CODEX_REQUIRE_AUTH_JSON;
      const previousRequireSkipGitRepoCheck = process.env.REMI_CODE_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK;
      const previousRequireApprovalNever = process.env.REMI_CODE_FAKE_CODEX_REQUIRE_APPROVAL_NEVER;
      const previousCodexHomeConfigMustContain =
        process.env.REMI_CODE_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN;
      const previousCodexHomeConfigMustNotContain =
        process.env.REMI_CODE_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN;
      const previousDisableBrowserPlugin =
        process.env.REMI_CODE_DISABLE_CODEX_REMI_CODE_BROWSER_PLUGIN;

      yield* Effect.sync(() => {
        process.env.PATH = `${binDir}:${previousPath ?? ""}`;
        process.env.REMI_CODE_FAKE_CODEX_OUTPUT_B64 = Buffer.from(input.output, "utf8").toString("base64");
        process.env.REMI_CODE_DISABLE_CODEX_REMI_CODE_BROWSER_PLUGIN = "0";

        if (input.exitCode !== undefined) {
          process.env.REMI_CODE_FAKE_CODEX_EXIT_CODE = String(input.exitCode);
        } else {
          delete process.env.REMI_CODE_FAKE_CODEX_EXIT_CODE;
        }

        if (input.stderr !== undefined) {
          process.env.REMI_CODE_FAKE_CODEX_STDERR = input.stderr;
        } else {
          delete process.env.REMI_CODE_FAKE_CODEX_STDERR;
        }

        if (input.requireImage) {
          process.env.REMI_CODE_FAKE_CODEX_REQUIRE_IMAGE = "1";
        } else {
          delete process.env.REMI_CODE_FAKE_CODEX_REQUIRE_IMAGE;
        }

        if (input.stdinMustContain !== undefined) {
          process.env.REMI_CODE_FAKE_CODEX_STDIN_MUST_CONTAIN = input.stdinMustContain;
        } else {
          delete process.env.REMI_CODE_FAKE_CODEX_STDIN_MUST_CONTAIN;
        }

        if (input.stdinMustNotContain !== undefined) {
          process.env.REMI_CODE_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN = input.stdinMustNotContain;
        } else {
          delete process.env.REMI_CODE_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN;
        }

        if (input.requireCodexHome) {
          process.env.REMI_CODE_FAKE_CODEX_REQUIRE_CODEX_HOME = "1";
        } else {
          delete process.env.REMI_CODE_FAKE_CODEX_REQUIRE_CODEX_HOME;
        }

        if (input.requireAuthJson) {
          process.env.REMI_CODE_FAKE_CODEX_REQUIRE_AUTH_JSON = "1";
        } else {
          delete process.env.REMI_CODE_FAKE_CODEX_REQUIRE_AUTH_JSON;
        }

        if (input.requireSkipGitRepoCheck) {
          process.env.REMI_CODE_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK = "1";
        } else {
          delete process.env.REMI_CODE_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK;
        }

        if (input.requireApprovalNever) {
          process.env.REMI_CODE_FAKE_CODEX_REQUIRE_APPROVAL_NEVER = "1";
        } else {
          delete process.env.REMI_CODE_FAKE_CODEX_REQUIRE_APPROVAL_NEVER;
        }
        ...
        if (input.codexHomeConfigMustContain !== undefined) {
          ...
          delete process.env.REMI_CODE_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_CONTAIN;
        }
        ...
        if (input.codexHomeConfigMustNotContain !== undefined) {
          ...
          delete process.env.REMI_CODE_FAKE_CODEX_CODEX_HOME_CONFIG_MUST_NOT_CONTAIN;
        }
      });
    ...
    (previous) =>
      Effect.sync(() => {
        ...
        if (previous.previousOutput === undefined) {
          delete process.env.REMI_CODE_FAKE_CODEX_OUTPUT_B64;
        } else {
          process.env.REMI_CODE_FAKE_CODEX_OUTPUT_B64 = previous.previousOutput;
        }

        if (previous.previousExitCode === undefined) {
          delete process.env.REMI_CODE_FAKE_CODEX_EXIT_CODE;
        } else {
          process.env.REMI_CODE_FAKE_CODEX_EXIT_CODE = previous.previousExitCode;
        }

        if (previous.previousStderr === undefined) {
          delete process.env.REMI_CODE_FAKE_CODEX_STDERR;
        } else {
          process.env.REMI_CODE_FAKE_CODEX_STDERR = previous.previousStderr;
        }

        if (previous.previousRequireImage === undefined) {
          delete process.env.REMI_CODE_FAKE_CODEX_REQUIRE_IMAGE;
        } else {
          process.env.REMI_CODE_FAKE_CODEX_REQUIRE_IMAGE = previous.previousRequireImage;
        }

        if (previous.previousStdinMustContain === undefined) {
          delete process.env.REMI_CODE_FAKE_CODEX_STDIN_MUST_CONTAIN;
        } else {
          process.env.REMI_CODE_FAKE_CODEX_STDIN_MUST_CONTAIN = previous.previousStdinMustContain;
        }

        if (previous.previousStdinMustNotContain === undefined) {
          delete process.env.REMI_CODE_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN;
        } else {
          process.env.REMI_CODE_FAKE_CODEX_STDIN_MUST_NOT_CONTAIN = previous.previousStdinMustNotContain;
        }

        if (previous.previousRequireCodexHome === undefined) {
          delete process.env.REMI_CODE_FAKE_CODEX_REQUIRE_CODEX_HOME;
        } else {
          process.env.REMI_CODE_FAKE_CODEX_REQUIRE_CODEX_HOME = previous.previousRequireCodexHome;
        }

        if (previous.previousRequireAuthJson === undefined) {
          delete process.env.REMI_CODE_FAKE_CODEX_REQUIRE_AUTH_JSON;
        } else {
          process.env.REMI_CODE_FAKE_CODEX_REQUIRE_AUTH_JSON = previous.previousRequireAuthJson;
        }

        if (previous.previousRequireSkipGitRepoCheck === undefined) {
          delete process.env.REMI_CODE_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK;
        } else {
          process.env.REMI_CODE_FAKE_CODEX_REQUIRE_SKIP_GIT_REPO_CHECK =
        ...
        if (previous.previousRequireApprovalNever === undefined) {
          ...
          process.env.REMI_CODE_FAKE_CODEX_REQUIRE_APPROVAL_NEVER = previous.previousRequireApprovalNever;
        }
        ...
        if (previous.previousDisableBrowserPlugin === undefined) {
          delete process.env.REMI_CODE_DISABLE_CODEX_REMI_CODE_BROWSER_PLUGIN;
        } else {
          process.env.REMI_CODE_DISABLE_CODEX_REMI_CODE_BROWSER_PLUGIN =
    ...
    withFakeCodexEnv(
      ...
      Effect.gen(function* () {
        ...
        const wrongCodexHome = yield* fs.makeTempDirectoryScoped({
          prefix: "remi-code-wrong-codex-",
        });
        const customCodexHome = yield* fs.makeTempDirectoryScoped({
          prefix: "remi-code-custom-codex-",
        });
