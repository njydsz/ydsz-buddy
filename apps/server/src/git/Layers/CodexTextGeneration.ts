...
import { DEFAULT_GIT_TEXT_GENERATION_MODEL } from "@remi-code/contracts";
import { sanitizeGeneratedThreadTitle } from "@remi-code/shared/chatThreads";
import { resolveCodexHome } from "@remi-code/shared/codexConfig";
import { sanitizeBranchFragment, sanitizeFeatureBranchName } from "@remi-code/shared/git";
...
const makeCodexTextGeneration = Effect.gen(function* () {
  ...
  const writeTempFile = (
    ...
    const filePath = path.join(tempDir, `remi-code-${prefix}-${process.pid}-${randomUUID()}.tmp`);
  ...
  const prepareIsolatedCodexHome = (
    ...
    Effect.gen(function* () {
      ...
      const isolatedHomePath = path.join(
        tempDir,
        `remi-code-codex-home-${process.pid}-${randomUUID()}`,
      );
