...
const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), { prefix: "remi-code-git-core-test-" });
  ...
  describe("stashAndCheckout", () => {
    ...
    it.effect("drops only the temporary stash it created after a successful reapply", () =>
      Effect.gen(function* () {
        ...
        expect(stashList).not.toContain("remi-code: stash before switching to feature");
        expect(yield* readTextFile(path.join(tmp, "README.md"))).toBe("dirty changes\n");
      }),
    );
    ...
    it.effect("keeps the stash when reapplying dirty changes conflicts", () =>
      Effect.gen(function* () {
        ...
        expect(yield* git(tmp, ["stash", "list"])).toContain(
          "remi-code: stash before switching to conflicting",
        );
      }),
    );
  });
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "remi-code/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "remi-code/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "remi-code/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "remi-code/tmp-working",
          newBranch: "remi-code/feat/session",
        });

        expect(renamed.branch).toBe("remi-code/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "remi-code/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "remi-code/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("remi-code/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "remi-code/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "remi-code/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "remi-code/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "remi-code/tmp-working" });
        ...
        expect(renamed.branch).toBe("remi-code/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:pingdotgg/remi-code.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:pingdotgg/remi-code.git/",
          });
