...
const AUTO_DETACHED_WORKTREE_DIRNAME = "remi-code";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `remi-code-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
      ...
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const tempIndexDir = yield* fileSystem.makeTempDirectoryScoped({
            prefix: `remi-code-git-status-index-${process.pid}-`,
          });
    ...
    const stashAndCheckout: GitCoreShape["stashAndCheckout"] = (input) =>
      Effect.gen(function* () {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["stash", "push", "-u", "-m", `remi-code: stash before switching to ${input.branch}`],
          {
            timeoutMs: 30_000,
