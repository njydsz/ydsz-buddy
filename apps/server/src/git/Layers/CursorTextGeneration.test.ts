...
function withFakeAcpAgent<A, E, R>(
  ...
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const tempDir = mkdtempSync(path.join(os.tmpdir(), "remi-code-cursor-text-acp-"));
      return {
        tempDir,
...
it.layer(CursorTextGenerationTestLayer)("CursorTextGenerationLive", (it) => {
  it.effect("uses ACP model config options instead of raw CLI model ids", () => {
    const requestLogDir = mkdtempSync(path.join(os.tmpdir(), "remi-code-cursor-text-log-"));
    const requestLogPath = path.join(requestLogDir, "requests.ndjson");

    return withFakeAcpAgent(
      {
        REMI_CODE_ACP_REQUEST_LOG_PATH: requestLogPath,
        REMI_CODE_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
  ...
  it.effect("accepts json objects with extra assistant text around them", () =>
    withFakeAcpAgent(
      {
        REMI_CODE_ACP_PROMPT_RESPONSE_TEXT:
  ...
  it.effect("closes the ACP child process after text generation completes", () => {
    const exitLogDir = mkdtempSync(path.join(os.tmpdir(), "remi-code-cursor-text-exit-log-"));
    const exitLogPath = path.join(exitLogDir, "exit.log");

    return withFakeAcpAgent(
      {
        REMI_CODE_ACP_EXIT_LOG_PATH: exitLogPath,
        REMI_CODE_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({
