...
import type { ModelSelection, ProviderRuntimeEvent, ProviderSession } from "@remi-code/contracts";
import {
  ApprovalRequestId,
  CommandId,
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const baseDir = input?.baseDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "remi-code-reactor-"));
  ...
  it("renames temporary worktree branches and keeps associated worktree metadata in sync", async () => {
    ...
    await waitFor(async () => {
      ...
      return (
        thread?.branch === "remi-code/app-startup-crash" &&
        thread.associatedWorktreeBranch === "remi-code/app-startup-crash" &&
        thread.associatedWorktreeRef === "remi-code/app-startup-crash"
      );
    });
    ...
    expect(thread).toMatchObject({
      branch: "remi-code/app-startup-crash",
      worktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreePath: "/tmp/provider-project/.worktrees/cb661f0d",
      associatedWorktreeBranch: "remi-code/app-startup-crash",
      associatedWorktreeRef: "remi-code/app-startup-crash",
    });
  });
  ...
  it("falls back to prompt-based worktree branch names when the provider cannot generate one", async () => {
    ...
    expect(harness.renameBranch.mock.calls[0]?.[0]).toMatchObject({
      oldBranch: "remi-code/cb661f0d",
      newBranch: "remi-code/fix-provider-startup-timeouts",
    });
    ...
    await waitFor(async () => {
      ...
      return thread?.branch === "remi-code/fix-provider-startup-timeouts";
    });
  });
