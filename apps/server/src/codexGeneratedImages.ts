...
import { isSupportedLocalImagePath as isSupportedLocalImagePathShared } from "@remi-code/shared/localImage";
 ...
 * writes images under for the current process env. When Remi Code wraps Codex
 * with the remi-code-browser overlay (production default), this is the overlay
 * home â€?not the user's `~/.codex`.
 */
 ...
 * overlay `<REMI_CODE_HOME>/codex-home-overlay/generated_images` so we serve
