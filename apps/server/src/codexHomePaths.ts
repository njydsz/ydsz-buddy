...
export const REMI_CODE_CODEX_HOME_OVERLAY_DIR = "codex-home-overlay";
export const REMI_CODE_DISABLE_CODEX_REMI_CODE_BROWSER_PLUGIN_ENV =
  "REMI_CODE_DISABLE_CODEX_REMI_CODE_BROWSER_PLUGIN";
...
export function shouldDisableRemiCodeBrowserPlugin(env: NodeJS.ProcessEnv): boolean {
  // The plugin is disabled by default; the only way to opt out is the explicit "0" sentinel.
  return env[REMI_CODE_DISABLE_CODEX_REMI_CODE_BROWSER_PLUGIN_ENV] !== "0";
}

export function resolveRemiCodeCodexHomeOverlayPath(
  env: NodeJS.ProcessEnv,
  sourceHomePath: string,
): string {
  const runtimeHome = env.REMI_CODE_HOME?.trim() || env.REMI_CODE_HOME_LEGACY?.trim();
  const overlayRoot =
    runtimeHome || path.join(path.dirname(sourceHomePath), ".remi-code", "runtime");
  return path.join(overlayRoot, REMI_CODE_CODEX_HOME_OVERLAY_DIR);
}

/**
 * Returns the home directory that the codex app-server child process actually
 * writes under. This is the overlay home when Remi Code wraps Codex with the
 * remi-code-browser plugin disabled (the production default), otherwise the
...
export function resolveActiveCodexHomeWritePath(input: CodexHomePathsInput = {}): string {
  ...
  if (!shouldDisableRemiCodeBrowserPlugin(env)) {
    return source;
  }
  const overlay = resolveRemiCodeCodexHomeOverlayPath(env, source);
