...
import { startMockLlmServer } from '@njydsz/ydb-llm-mock-server'
 ...
 * ydb.profile.bundles, no dsh-base — proving out-of-box composition machinery without
...
function createProfileLifecycleFixture(): ProfileLifecycleFixture {
  ...
  writeFileSync(join(bundleDir, 'plugin.mjs'), [
    ...
    "  writeFileSync(join(process.env.YDB_HOME, 'config-echo'), String(config.generation ?? 'bundle-default'))",
...
function createEnvironmentProbeProfile(home: string, project: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    ...
    private: true,
    dependencies: {},
    dsh: { profile: { bundles: ['@njydsz/ydb-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- insert:',
 ...
 * `@njydsz/ydb-cmdline` and `commander` through the profile module
...
function createStartupFixture(): StartupFixture {
  ...
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    ...
    "import { parseCmdline } from '@njydsz/ydb-cmdline'",
  ...
  writeFileSync(join(bundleDir, 'witness.mjs'), [
    ...
    'export function apply(ctx, config = {}) {',
    "  writeFileSync(join(process.env.YDB_HOME, 'witness'), String(config.generation ?? 'bundle-default'))",
    '}',
    '',
  ].join('\n'))
  writeFileSync(join(bundleDir, 'cordis.patch.yml'), [
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    ...
    try {
      const result = await runBuiltBin(['--profile', 'nope'], { YDB_HOME: home })
      expect(result.code).toBe(1)
  ...
  it('fully settles a custom profile, hot-reloads its patch layer with removal reverting, and disposes on a signal', async () => {
    ...
    try {
      ...
      // The home-level user layer ($YDB_HOME/cordis.patch.yml) is live too
  ...
  it('activates a dependency that gained ydb.bundle in a later update', async () => {
    ...
    // version declares ydb.bundle. Simulated without a registry: hand-place
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      const { stdout, code, stderr } = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { YDB_HOME: home })
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@njydsz/ydb-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @njydsz/ydb-base')
      expect(stdout).toContain("name: '@njydsz/ydb-host-webserver'")
    }, 30_000)
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@njydsz/ydb-headless'")
      expect(stdout).not.toMatch(/name: '@deepseek-ai\/dsh-host-/)
      expect(stdout).not.toContain("name: '@njydsz/ydb-web-app'")
    ...
    it('composes the profile user layer and a --patch overlay in order', async () => {
      ...
      const init = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { YDB_HOME: home })
