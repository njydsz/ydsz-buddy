...
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@njydsz/ydb-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    ...
    initProfile(join(home, PROFILES_DIR, 'web'), ['@njydsz/ydb-base', '@njydsz/ydb-web-app'])
    ...
    for (const name of ['@njydsz/ydb-pwsh-sandbox', '@njydsz/ydb-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'dsh-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@njydsz/ydb-base'])
