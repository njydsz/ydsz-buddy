...
import { boot, healProfilesModuleFallback, loadOverlayPatches } from '@njydsz/ydb-app-boot'
...
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@njydsz/ydb-agent-presets'
import { applyChildComposition, childSessionMeta } from '@njydsz/ydb-subagent'
import { CallId } from '@njydsz/ydb-llm'
import type {} from '@njydsz/ydb-compaction-basic'
...
import type {} from '@njydsz/ydb-token-meter'
...
async function bootWeb(
  ...
  const patches: PatchOptions[] = [
    ...
    // The settings row defaults to `$YDB_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $YDB_HOME. Unpinned, this
...
describe('product subagent rows in user presets', () => {
  ...
  beforeAll(async () => {
    ...
    productCtx = await bootWeb(settingsFile, [
      { insert: [
        { id: 'subagent-codex', name: '@njydsz/ydb-subagent-codex' },
        { id: 'subagent-claude-code', name: '@njydsz/ydb-subagent-claude-code' },
      ] },
      {
        id: 'agent-presets',
        config: {
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<dshHome>/.agent-presets` itself. `$YDB_HOME` is pointed
  ...
  beforeAll(async () => {
    ...
    previousHome = process.env.YDB_HOME
    process.env.YDB_HOME = home
  ...
  afterAll(async () => {
    if (previousHome === undefined) delete process.env.YDB_HOME
    else process.env.YDB_HOME = previousHome
