...
use ydsz_provider::service::ProviderService;
...
/// # 字段说明
///
/// - `cwd`: Git 工作目录路径，必须是有效的 Git 仓库
/// - `text_type`: 要生成的文本类型（提交消息、PR 标题等）
/// - `context`: 可选的上下文信息，如预处理的 diff 内容
///
/// # 使用示例
///
/// ```rust,ignore
/// let input = GenerateTextInput {
///     cwd: "/path/to/repo".to_string(),
///     text_type: TextGenerationType::CommitMessage,
///     context: None,
/// };
/// ```
#[derive(Debug, Clone)]
...
/// 文本生成类型
///
/// 定义需要 AI 生成的文本类型，不同类型使用不同的提示词模板。
///
/// # 类型说明
///
/// | 类型 | 用途 | 格式要求 |
/// |------|------|---------|
/// | `CommitMessage` | Git 提交消息 | Conventional Commits 格式 |
/// | `PrTitle` | PR 标题 | 简洁描述性，最多 72 字符 |
/// | `PrDescription` | PR 描述 | Markdown 格式，包含变更摘要 |
/// | `CodeReviewSuggestion` | 代码审查建议 | 结构化建议格式 |
...
///
/// - `text`: 生成的文本内容
/// - `provider`: 使用的 Provider 名称（如 "ClaudeAgent"），未使用 AI 时为 `None`
///
/// # 示例
///
/// ```rust,ignore
/// let result = GenerateTextResult {
///     text: "feat(auth): add OAuth2 support".to_string(),
...
/// # 功能特性
///
/// - **AI 驱动**：通过 ProviderService 调用 AI 模型生成高质量文本
/// - **降级机制**：AI 生成失败时自动降级到模板化生成
/// - **多种类型**：支持提交消息、PR 标题、PR 描述等
/// - **并发安全**：内部使用 `Arc` 实现多线程共享
///
/// # 架构设计
///
/// ```text
/// ┌─────────────────────────────────────┐
/// │   GitTextGenerationService          │
/// │  ┌────────────┐  ┌───────────────┐  │
/// │  │  GitCore   │  │ ProviderService│  │
/// │  │ (diff获取) │  │ (AI生成)      │  │
/// │  └─────┬──────┘  └──────┬────────┘  │
/// └────────┼─────────────────┼──────────┘
///          │                 │
