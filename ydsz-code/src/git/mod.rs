//! # 云顶数字 Git 模块
//!
//! 本模块是 ydsz 工作区中负责 Git 版本控制操作的核心服务层，提供对 Git 命令的高层封装。
//!
//! ## 模块职责
//!
...
//! | [`executor`] | Git 命令执行器抽象（`GitExecutor` trait + `LocalGitExecutor` / `SshGitExecutor`） |
...
//! async fn main() {
//! use std::sync::Arc;
//! use ydsz_git::{GitCore, GitManager, GitStatusBroadcaster};
//! 
