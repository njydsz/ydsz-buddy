...
use super::executor::{GitExecutor, LocalGitExecutor};
...
/// `git worktree list --porcelain` 解析出的单个 worktree 记录
///
/// 用于启动对账：扫描 git 自身登记的 worktree，与 `ManagedWorktreeService`
/// 内存注册表比对，发现磁盘孤儿或悬空注册表项。
///
/// # 字段说明
///
/// - `path`: worktree 在文件系统中的绝对路径
/// - `head`: worktree 当前 HEAD 的 commit SHA（可能为空，detached HEAD 时仍有值）
/// - `branch`: worktree 检出的分支引用（如 `refs/heads/main`），detached 时为 `None`
/// - `detached`: 是否为 detached HEAD 状态
...
/// ```
///
/// - `worktree <path>`: 必有，标记一条记录的开始
/// - `HEAD <sha>`: 必有
/// - `branch <ref>`: 可选，detached 时不存在
/// - `detached`: 可选，标记 detached HEAD
///
/// # 参数
///
/// - `output`: `git worktree list --porcelain` 的完整 stdout
///
/// # 返回值
///
/// 返回解析后的 `Vec<WorktreeListEntry>`。空行和无法识别的行会被跳过。
        ...
        if trimmed.is_empty() {
            ...
            if let Some(path) = current_path.take() {
                entries.push(WorktreeListEntry {
                    ...
                    detached: std::mem::take(&mut current_detached),
                });
            }
            continue;
        }
...
/// }
pub struct GitCore {
    /// Git 命令执行器（本地 LocalGitExecutor 或远端 SshGitExecutor）
    ///
    /// 通过 `with_executor` 注入 SshGitExecutor 后，所有经 `execute()` 的 Git 操作
    /// 都自动走 SSH 远端。`apply_patch` 因需要 stdin 传 patch，暂保留本地实现。
    executor: Arc<dyn GitExecutor>,
}

impl GitCore {
    /// 创建新的 Git 核心服务实例（本地模式）
    ///
    /// # 返回值
    ///
    /// 返回一个新的 `GitCore` 实例，内部使用 `LocalGitExecutor`。
    ///
    /// # 使用示例
    ///
    ///```rust,ignore
    ...
    pub fn new() -> Self {
        Self {
            executor: Arc::new(LocalGitExecutor::new()),
        }
    }

    /// 使用指定 GitExecutor 构造（本地或远端）
    ///
    /// - `Arc::new(LocalGitExecutor::new())` → 本地 git
    /// - `Arc::new(SshGitExecutor::new(conn))` → SSH 远端 git
    pub fn with_executor(executor: Arc<dyn GitExecutor>) -> Self {
        Self { executor }
    }
    ...
    pub async fn execute(&self, input: ExecuteGitInput) -> GitResult<ExecuteGitResult> {
        // 委托给 executor（LocalGitExecutor 或 SshGitExecutor）
        self.executor.execute(input).await
    }

    /// 通过 stdin 将 patch 应用到工作区
    ///
    /// 将统一 diff 格式的 patch 内容通过 stdin 传给 `git apply`，
    /// 支持选择性地应用部分 hunk 变更。
    ///
    /// # 参数
    ///
    /// - `cwd`: 仓库工作目录的绝对路径
    /// - `patch`: 统一 diff 格式的 patch 字符串
    /// - `cached`: 若为 true 则使用 `--cached` 仅更新索引区
    ///
    /// # 返回值
    ///
    /// - `Ok(())`: patch 应用成功
    /// - `Err(GitError)`: 应用失败（冲突或格式错误）
    pub async fn apply_patch(&self, cwd: &str, patch: &str, cached: bool) -> GitResult<()> {
        debug!("应用 patch ({} bytes) 到 {}", patch.len(), cwd);
        ...
        let mut child = cmd.spawn().map_err(|e| {
            GitError::CommandError(format!("启动 git apply 失败: {}", e))
        })?;
        ...
        if let Some(stdin) = child.stdin.take() {
            ...
            tokio::spawn(async move {
                ...
                let mut stdin = stdin;
                let _ = stdin.write_all(&patch_bytes).await;
            });
        }

        let output = child.wait_with_output().await.map_err(|e| {
            GitError::CommandError(format!("等待 git apply 失败: {}", e))
        })?;
        ...
        if code != 0 {
            warn!("git apply 失败 (exit code {}): {}", code, stderr);
    ...
    pub async fn status(&self, cwd: &str) -> GitResult<GitStatusResult> {
        ...
        let pr = if let Some(ref branch) = current_branch {
            self.get_pr_info(cwd, branch).await.ok().flatten()
        } else {
            None
        };
    ...
    async fn get_pr_info(&self, cwd: &str, branch: &str) -> GitResult<Option<PullRequestInfo>> {
        ...
        match output {
            Ok(out) if out.status.success() => {
                ...
                let prs: Vec<PullRequestInfo> = serde_json::from_str(&stdout)
                    .map_err(|e| GitError::InternalError(format!("parse pr json: {e}")))?;
    ...
    /// 将已有分支挂载到新 worktree（不创建新分支）
    ///
    /// 与 [`create_worktree`](Self::create_worktree) 的区别:
    /// - `create_worktree` 总是用 `-b <name>` 创建一个新分支
    /// - `attach_worktree` 直接 `git worktree add <path> <existing_branch>` 挂载到已有分支
    ///
    /// # 参数
    ///
    /// - `cwd`: 主仓库工作目录
    /// - `worktree_path`: 要创建的 worktree 路径
    /// - `branch_name`: 要挂载的已存在分支名
    ///
    /// # 错误
    ///
    /// - `branch_name` 不存在 → `git worktree add` 返回非零退出码
    /// - `worktree_path` 已被另一个 worktree 占用 → 同样失败
    pub async fn attach_worktree(
        ...
        self.execute(ExecuteGitInput {
            operation: "worktree add (attach)".to_string(),
            cwd: cwd.to_string(),
            args,
            env: vec![],
            allow_non_zero_exit: false,
    ...
    /// 列出仓库的所有 worktree（porcelain 格式）
    ///
    /// 执行 `git worktree list --porcelain`，解析每个 worktree 的路径、HEAD 和分支。
    /// 本方法只读取 git 自身登记的 worktree，不依赖 ManagedWorktreeService 注册表，
    /// 因此可用于启动对账：发现磁盘上存在但注册表缺失的 worktree（孤儿）。
    ///
    /// # 参数
    ///
    /// - `cwd`: 主仓库工作目录（可以是仓库内任意子目录）
    ///
    /// # 返回值
    ///
    /// - `Ok(Vec<WorktreeListEntry>)`: worktree 列表（包含主仓库自身）
    /// - `Err(GitError::CommandError)`: 命令执行失败（非 git 仓库等）
    ///
    /// # 输出格式示例
    ///
    /// ```text
    ...
    /// ```
    pub async fn list_worktrees_porcelain(
        &self,
        cwd: &str,
    ) -> GitResult<Vec<WorktreeListEntry>> {
        let result = self
            .execute(ExecuteGitInput {
                operation: "worktree list --porcelain".to_string(),
                cwd: cwd.to_string(),
                args: vec![
                    "worktree".to_string(),
                    "list".to_string(),
        ...
        Ok(parse_worktree_list_porcelain(&result.stdout))
    }
    ...
    pub async fn remove_index_lock(&self, cwd: &str) -> GitResult<()> {
        ...
        if lock_path.exists() {
            std::fs::remove_file(&lock_path).map_err(|e| {
                super::error::GitError::CommandError(format!(
...
mod tests {
    ...
    #[test]
    fn parse_worktree_list_porcelain_empty() {
        assert!(parse_worktree_list_porcelain("").is_empty());
        assert!(parse_worktree_list_porcelain("  \n  \n").is_empty());
    }

    #[test]
    fn parse_worktree_list_porcelain_single_main() {
    ...
    #[test]
    fn parse_worktree_list_porcelain_detached() {
        let output = "worktree /tmp/preview\nHEAD def789\nbare\n";
        let entries = parse_worktree_list_porcelain(output);
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].path, "/tmp/preview");
        assert_eq!(entries[0].head, "def789");
        assert_eq!(entries[0].branch, None);
    }

    #[test]
    fn parse_worktree_list_porcelain_multiple() {
        let output = "worktree /repo\nHEAD a1\nbranch refs/heads/main\n\nworktree /repo/.ydsz-worktrees/feature\nHEAD b2\nbranch refs/heads/feature/foo\n";
        let entries = parse_worktree_list_porcelain(output);
        assert_eq!(entries.len(), 2);
        assert_eq!(entries[0].path, "/repo");
        assert_eq!(entries[1].path, "/repo/.ydsz-worktrees/feature");
        assert_eq!(entries[1].branch.as_deref(), Some("refs/heads/feature/foo"));
    }

    #[test]
    fn parse_worktree_list_porcelain_no_trailing_newline() {
        let output = "worktree /repo\nHEAD a1\nbranch refs/heads/main";
        let entries = parse_worktree_list_porcelain(output);
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].path, "/repo");
        assert_eq!(entries[0].head, "a1");
    }

    #[test]
    fn parse_worktree_list_porcelain_ignores_locked() {
        let output = "worktree /repo\nHEAD a1\nbranch refs/heads/main\nlocked\n";
        let entries = parse_worktree_list_porcelain(output);
        assert_eq!(entries.len(), 1);
        assert_eq!(entries[0].path, "/repo");
